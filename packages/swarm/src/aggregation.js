...
export function aggregate(results, algorithm, strategy = 'union', workerIds) {
...
function asDfg(result) {
    if (!result || typeof result !== 'object')
        return null;
    return result;
}
function unionDfgResults(results) {
...
function intersectDfgResults(results) {
    if (results.length === 0)
        return { nodes: [], edges: [] };
...
function averageDfgResults(results) {
    ...
    for (const r of results) {
        ...
        if (!dfg)
            continue;
        for (const n of dfg.nodes ?? []) {
            const key = JSON.stringify(n?.id ?? n);
            nodeSet.add(key);
 ...
 */
function ensembleMlResults(algorithm, results) {
    if (results.length === 0)
        return null;
 ...
 */
function majorityVoteMl(results) {
    const allPredictions = [];
    for (const r of results) {
        const res = r;
        const preds = (res.predictions ?? res.assignments ?? []);
        for (const p of preds) {
            ...
            if (caseId)
                allPredictions.push({ caseId, predicted, confidence });
        }
    }
    // Group by caseId, take majority vote
    const byCase = new Map();
    for (const p of allPredictions) {
        const votes = byCase.get(p.caseId) ?? new Map();
    ...
    const method = results[0]?.method ?? 'ensemble';
    return {
 ...
 */
function averageNumericMl(results) {
    ...
    if (forecasts.length > 0) {
        ...
        const avg = Array.from({ length: maxLen }, (_, i) => {
        ...
        return {
            ...
            ensembleSize: results.length,
            seriesLength: results[0]?.seriesLength,
        };
    }
 ...
 */
function unionAnomalies(results) {
    const allPeaks = new Set();
    const peakValues = new Map();
    for (const r of results) {
        const obj = r;
        const peaks = (obj.peakIndices ?? []);
        const values = (obj.peakValues ?? []);
