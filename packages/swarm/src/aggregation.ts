...
export function aggregate(
  ...
  switch (strategy) {
    ...
    case 'ml_ensemble': {
      ...
      aggregate = ensembleMlResults(
        algorithm,
        relevant.map((r) => r.result)
      );
      break;
    }
...
function intersectDfgResults(results: unknown[]): unknown {
  ...
  for (const r of results.slice(1)) {
    ...
    for (const k of nodeKeys) {
      if (!rNodeKeys.has(k)) nodeKeys.delete(k);
    }
    for (const k of edgeKeys) {
      if (!rEdgeKeys.has(k)) edgeKeys.delete(k);
    }
  }
...
function majorityVoteMl(results: unknown[]): unknown {
  ...
  const finalPredictions = Array.from(byCase.entries()).map(([caseId, votes]) => {
    ...
    return {
      caseId,
      predicted: dominant,
      confidence: maxVotes / allPredictions.filter((p) => p.caseId === caseId).length,
    };
  });
