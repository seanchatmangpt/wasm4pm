    ...
    async dispatchAlgorithm(algorithmId, eventLogHandle, activityKey, params) {
        switch (algorithmId) {
            ...
            case 'compute_simplicity': {
                const result = this.wasm.wasm_compute_simplicity(params.places ?? 0, params.transitions ?? 0, params.arcs ?? 0);
            ...
            case 'ml_classify':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml classify ...' instead.`);
            case 'ml_cluster':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml cluster ...' instead.`);
            case 'ml_forecast':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml forecast ...' instead.`);
            case 'ml_anomaly':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml anomaly ...' instead.`);
            case 'ml_regress':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml regress ...' instead.`);
            case 'ml_pca':
                throw new Error(`ML algorithm '${algorithmId}' requires the @wasm4pm/ml package. Run 'wasm4pm ml pca ...' instead.`);
            default:
