...
use wasm4pm::reinforcement::{
    QLearning, SARSAAgent, DoubleQLearning, ExpectedSARSAAgent, ReinforceAgent,
...
fn test_qlearning_update_instrumentation() {
    ...
    let state = TestState { health_level: 0, cycle_count: 1 };
    let next_state = TestState { health_level: 1, cycle_count: 2 };
...
fn test_qlearning_action_selection_instrumentation() {
    ...
    assert_eq!(action1, action2, "Same seed should produce same action on fresh agents");
}
...
fn test_sarsa_update_instrumentation() {
    ...
    agent.update(&state, &TestAction::Continue, 1.0, &next_state, &next_action);
...
fn test_double_qlearning_update_instrumentation() {
    let agent: DoubleQLearning<TestState, TestAction> = DoubleQLearning::new_with_seed(0.1, 0.99, 42);
    let state = TestState { health_level: 0, cycle_count: 1 };
    let next_state = TestState { health_level: 1, cycle_count: 2 };
...
fn test_expected_sarsa_update_instrumentation() {
    let agent: ExpectedSARSAAgent<TestState, TestAction> = ExpectedSARSAAgent::new_with_seed(0.1, 0.99, 42);
    let state = TestState { health_level: 0, cycle_count: 1 };
    let next_state = TestState { health_level: 1, cycle_count: 2 };
...
fn test_reinforce_action_selection_instrumentation() {
    let agent: ReinforceAgent<TestState, TestAction> = ReinforceAgent::new_with_seed(0.01, 0.99, 42);
    let state = TestState { health_level: 0, cycle_count: 1 };
...
fn test_reinforce_trajectory_update_instrumentation() {
    let agent: ReinforceAgent<TestState, TestAction> = ReinforceAgent::new_with_seed(0.01, 0.99, 42);
    let state1 = TestState { health_level: 0, cycle_count: 1 };
    let state2 = TestState { health_level: 1, cycle_count: 2 };
    let state3 = TestState { health_level: 2, cycle_count: 3 };
...
fn test_exploration_decay_instrumentation() {
    ...
    assert!(final_eps.is_finite(), "Exploration rate should remain finite");
}
...
fn test_exploration_rate_setter_instrumentation() {
    ...
    assert_eq!(agent.get_exploration_rate(), 0.05, "Exploration rate should update");
}
...
fn test_weight_norm_convergence_tracking() {
    ...
    assert!(norm_final > norm0, "Weight norm should increase with learning");
...
fn test_determinism_with_seeded_rng() {
    ...
    let agent3: DoubleQLearning<TestState, TestAction> = DoubleQLearning::new_with_seed(0.1, 0.99, 42);
    let agent4: DoubleQLearning<TestState, TestAction> = DoubleQLearning::new_with_seed(0.1, 0.99, 42);
...
fn test_bellman_correctness() {
    ...
    assert!((q_after - 0.1).abs() < 1e-5, "Q-value should be ~0.1 after single update");
}
