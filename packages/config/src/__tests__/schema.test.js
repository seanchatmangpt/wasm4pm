...
import { validate, validatePartial, toJsonSchema, SCHEMA_VERSION, } from '../schema.js';
describe('Schema', () => {
    const minimal = {
        version: '26.4.5',
        source: { kind: 'file' },
    ...
    describe('validate', () => {
        ...
        it('accepts all otel exporters', () => {
            for (const exporter of ['otlp', 'console', 'none']) {
                expect(() => validate({
                    ...minimal,
    ...
    describe('toJsonSchema', () => {
        ...
        it('includes enum values for source.kind', () => {
            const schema = toJsonSchema();
            const sourceKind = schema.properties.source.properties.kind;
