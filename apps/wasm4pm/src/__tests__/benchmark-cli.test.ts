...
describe('wpm benchmark — benchmark corpus verification CLI', () => {
  ...
  describe('benchmark (base command)', () => {
    // `--help` for a bridged verb (nouns/lab/benchmark.ts) is generated by
    // the noun-verb framework from the verb's own citty registration (which
    // declares no args other than the generic `--human`/`--introspect`) —
    // it no longer echoes the legacy command's own subcommand listing.
    it('--help now shows the generic bridged-verb summary, not the legacy subcommand list', async () => {
      const result = await runCli(['lab', 'benchmark', '--help']);
      expect(result.stdout).toMatch(/build \| replay \| verify \| export/i);
      expect(result.stdout).toMatch(/--human/);
    });

    it('should show usage and list subcommands when invoked without subcommand or --help', async () => {
      // Omitting BOTH the subcommand and --help lets the legacy command's own
      // run() execute — it prints its own subcommand help text on stdout,
      // which the bridge (having no JSON to parse) surfaces as { raw: text }.
      ...
      expect(result.stdout).toMatch(/build|replay|verify|export|calibrate|gate/i);
    });

    it('should reject unknown subcommand', async () => {
      const result = await runCli(['lab', 'benchmark', 'invalid']);
      // citty's own "Unknown command" rejection is uncaught by the legacy
      // command and normalizes to the framework's generic EXECUTION_ERROR
      // (see NounVerbError.from()'s fallback) — always 3, not 1/2.
  ...
  describe('benchmark build', () => {
    it('should require corpus argument', async () => {
      const result = await runCli(['lab', 'benchmark', 'build']);
      // citty's own "missing required argument" rejection (declared
      // `required: true` on the legacy command's own arg schema) is
      // likewise uncaught and normalizes to EXECUTION_ERROR (3) through the
      // framework's generic catch — not the legacy command's own 1/2.
      expect(result.exitCode).toBe(EXECUTION_ERROR);
      // The diagnostic is on stdout's { error: { message } } now, not
      // stderr (stderr only carries the [experimental] banner) — check
      // both streams concatenated rather than `||` (which would pick the
      // non-empty banner and never look at stdout).
      expect(result.stdout + result.stderr).toMatch(/corpus|argument|required/i);
    });
    ...
    it('should validate JSONL corpus with valid traces', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'build', '--corpus', corpusPath]);
      expect(result.exitCode).toBe(SUCCESS);
    ...
    it('should fail when corpus file not found', async () => {
      const result = await runCli(['lab', 'benchmark', 'build', '--corpus', '/nonexistent/corpus.jsonl']);
    ...
    it('should support --format flag (human/json)', async () => {
      ...
      const jsonResult = await runCli(['lab', 'benchmark', 'build', '--corpus', corpusPath, '--format', 'json']);
    ...
    it('should support --quiet flag to suppress verbose output', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'build', '--corpus', corpusPath, '--quiet']);
  ...
  describe('benchmark replay', () => {
    ...
    it('should replay custom corpus and show per-trace results', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'replay', '--corpus', corpusPath]);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should filter to single trace via --trace flag', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'replay', '--corpus', corpusPath, '--trace', 'trace-1']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should support --verbose flag for detailed failure reasons', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'replay', '--corpus', corpusPath, '--verbose']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should support JSON output format', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'replay', '--corpus', corpusPath, '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
  ...
  describe('benchmark verify', () => {
    it('should exit 0 when all traces pass', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'verify', '--corpus', corpusPath]);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should run built-in suite when no corpus provided', async () => {
      const result = await runCli(['lab', 'benchmark', 'verify']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should support --format sarif for CI integration', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'verify', '--corpus', corpusPath, '--format', 'sarif']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    ...
    it('should support --format json', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'verify', '--corpus', corpusPath, '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should treat missing built-in corpus as execution error (not source error)', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'verify', '--corpus', '/nonexistent/builtin.jsonl']);
      expect(result.exitCode).toBe(EXECUTION_ERROR);
    });
    ...
    it('should support --verbose to show failure details', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'verify', '--corpus', corpusPath, '--verbose']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
  });
  ...
  describe('benchmark export', () => {
    it('should export results as SARIF by default', async () => {
      // NOTE: when --format is omitted entirely, nouns/_bridge.ts's
      // stripLegacyOutputFlags() has nothing to "keep" as a domain format,
      // so it forces --format=json — the legacy command's own 'sarif'
      // default is never reached through the bridge. This test's assertion
      // was already format-agnostic (parseable JSON, not a SARIF-shape
      // check), so it still holds; it just no longer proves SARIF-by-default
      // specifically. See "SARIF output should be valid JSON with results"
      // below for the real (--format sarif explicit) SARIF path.
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath]);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
      if (result.stdout.trim()) {
        expect(() => JSON.parse(result.stdout)).not.toThrow();
      }
    ...
    it('should export results as JSON with --format json', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
      if (result.stdout.trim()) {
        expect(() => JSON.parse(result.stdout)).not.toThrow();
      }
    ...
    it('should export results as CSV with --format csv', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'csv']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
      if (result.exitCode === SUCCESS) {
    ...
    it('should reject unknown export format', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'xml']);
      // Legacy config_error(1) is reclassified through the bridge's
      // classifyLegacyFailure() to INVALID_INPUT -> wpm's source_error(2).
      expect(result.exitCode).toBe(SOURCE_ERROR);
      // NOTE: the legacy command's own "Unknown format: xml..." diagnostic
      // is lost through the bridge for this specific path — its
      // emitResult() call hardcodes `{format:'human'}` (a pre-existing bug,
      // predating the rebuild) and the human-error renderer's output does
      // not survive nouns/_bridge.ts's stdio capture (see task tracker
      // "Bridge loses human-format ERROR diagnostic text..."). Only the
      // exit code is a reliable signal for this case now.
    });
    ...
    it('should support --quiet flag', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--quiet']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });

    it('should export built-in suite when no corpus provided', async () => {
      const result = await runCli(['lab', 'benchmark', 'export', '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
  });
  ...
  describe('benchmark calibrate', () => {
    it('should measure performance and exit with success', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '3']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });

    it('should enforce minimum 3 runs even if lower value provided', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '1']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
      // Implementation enforces Math.max(3, runs)
    });

    it('should use default 7 runs when --runs not specified', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });

    it('should support --format human (default)', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '3', '--format', 'human']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });

    it('should support --format json', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '3', '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });

    it('should measure WASM loading and discovery performance', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '3', '--quiet']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
    ...
    it('should support --quiet flag to suppress progress output', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--quiet']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
  });
  ...
  describe('exit codes', () => {
    ...
    it('benchmark build: source_error (2) when file not found', async () => {
      const result = await runCli(['lab', 'benchmark', 'build', '--corpus', '/nonexistent.jsonl']);
      expect(result.exitCode).toBe(SOURCE_ERROR);
    });
    ...
    it('benchmark export: source_error (2) on invalid format (was config_error=1 pre-rebuild)', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'invalid']);
      // Legacy config_error(1) is reclassified through the bridge's
      // classifyLegacyFailure() to INVALID_INPUT -> wpm's source_error(2).
      expect(result.exitCode).toBe(SOURCE_ERROR);
    });
  ...
  describe('output validation', () => {
    ...
    it('CSV output should have header row', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'csv']);
      if (result.exitCode === SUCCESS) {
        // The legacy command writes raw CSV straight to stdout (bypassing
        // emitResult()); nouns/_bridge.ts's always-JSON-on-stdout contract
        // wraps unparseable-as-JSON-but-non-empty stdout as `{ raw: text }`
        // instead of dropping it — so the CSV text is inside `raw`, not the
        // top-level stdout, and its first line is the CSV header.
        const parsed = JSON.parse(result.stdout) as { raw: string };
        const lines = parsed.raw.trim().split('\n');
    ...
    it('SARIF output should be valid JSON with results', async () => {
      ...
      const result = await runCli(['lab', 'benchmark', 'export', '--corpus', corpusPath, '--format', 'sarif']);
  ...
  describe('performance assertions', () => {
    ...
    it('calibrate command should report performance metrics', async () => {
      const result = await runCli(['lab', 'benchmark', 'calibrate', '--runs', '2', '--format', 'json']);
      expect([SUCCESS, EXECUTION_ERROR]).toContain(result.exitCode);
    });
  });
});
