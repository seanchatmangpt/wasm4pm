 ...
 */
export function selectEngineByPriority(
  ...
  if (budget.mode === 'research') {
    if (pythonAvailable) return 'pm4py';
 ...
 */
export function shouldPromoteJob(
  ...
  const qualityThreshold: Record<QualityTier, number> = {
    fast: 0.7,
    balanced: 0.8,
    quality: 0.85,
    research: 0.9,
 ...
 */
export function shouldDegradeAlgorithm(
  priorLatencyMs: number,
 ...
 */
export function profileToExecutionMode(
  ...
  switch (profile) {
    ...
    case 'balanced': {
      // balanced → online or near-online (by log size: >50K events → near-online)
      if (eventCount && eventCount > 50_000) {
        return 'near-online';
      }
      return 'online';
    }
 ...
 */
export function profileToLatencyBudget(
  profile: 'fast' | 'balanced' | 'quality' | 'stream'
): LatencyClass {
  switch (profile) {
    case 'fast':
 ...
 */
export function profileToQualityFloor(
  profile: 'fast' | 'balanced' | 'quality' | 'stream'
): QualityTier {
  switch (profile) {
    case 'fast':
