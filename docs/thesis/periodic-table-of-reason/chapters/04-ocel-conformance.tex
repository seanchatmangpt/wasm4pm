...
$= 1.0$ for MYCIN, Prolog, and STRIPS; full alignment fitness via planned external validation (pm4py replay)
...
\begin{remark}[PTIME decidability]
Choke-point enforcement escapes Rice's theorem (Axiom~\ref{ax:rice} in Chapter~\ref{chap:imported-axioms}): it does not decide a semantic property over all programs, but checks conformance of one concrete finite trace against a fixed, finite OCPN. Token replay on a trace of length $n$ against a net with $p$ places runs in $O(np)$ time---strictly PTIME, with no halting-problem contact. The decidable-subclass license of Axiom~0.5 applies directly.
\end{remark}
