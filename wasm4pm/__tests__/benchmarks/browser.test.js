...
function generateXES(numCases, numActivities = 12, avgEvents = 15, noiseFactor = 0.1) {
    ...
    for (let caseIdx = 0; caseIdx < numCases; caseIdx++) {
        ...
        for (let evtIdx = 0; evtIdx < numEvents; evtIdx++) {
            ...
            lines.push(`      <date key="time:timestamp" value="2024-01-${String(day).padStart(2, '0')}T${String(hour).padStart(2, '0')}:${String(min).padStart(2, '0')}:00.000+00:00"/>`);
            lines.push('    </event>');
        }
        lines.push('  </trace>');
    }
...
function callAlgorithm(name, handle, params = {}) {
    ...
    switch (name) {
        ...
        case 'discover_heuristic_miner':
            return pm.discover_heuristic_miner(handle, params.activityKey || ACTIVITY_KEY, params.threshold ?? 0.5);
        case 'discover_alpha_plus_plus':
            return pm.discover_alpha_plus_plus(handle, params.activityKey || ACTIVITY_KEY, params.minSupport ?? 0.1);
        ...
        case 'extract_process_skeleton':
            return pm.extract_process_skeleton(handle, params.activityKey || ACTIVITY_KEY, params.minFreq ?? 2);
        case 'discover_astar':
            return pm.discover_astar(handle, params.activityKey || ACTIVITY_KEY, params.maxIter ?? 500);
        case 'discover_simulated_annealing':
            return pm.discover_simulated_annealing(handle, params.activityKey || ACTIVITY_KEY, params.temp ?? 100.0, params.cooling ?? 0.95);
        case 'discover_ant_colony':
            return pm.discover_ant_colony(handle, params.activityKey || ACTIVITY_KEY, params.ants ?? 20, params.iterations ?? 10);
...
describe('WASM4PM Browser Benchmarks', () => {
    ...
    describe.each(BENCHMARK_TASKS)('$algorithm', (task) => {
        it.skip.each(task.sizes)(`should benchmark with %s cases`, (size) => {
            // Generate log
            ...
            console.log(`${task.algorithm.padEnd(30)} [${String(size).padEnd(5)} cases] ${result.medianMs.toFixed(2).padEnd(8)} ms`);
    ...
    describe('Summary', () => {
        it.skip('should report all benchmark results', () => {
            ...
            const sorted = [...BENCHMARK_RESULTS].sort((a, b) => a.algorithm.localeCompare(b.algorithm) || a.size - b.size);
