...
if (typeof ProgressEvent === 'undefined') {
    globalThis.ProgressEvent = class MockProgressEvent {
        constructor(type, _init) {
            this.type = type;
        }
    };
}
if (typeof StorageEvent === 'undefined') {
    globalThis.StorageEvent = class MockStorageEvent {
...
describe('process_mining_wasm Browser Integration', () => {
    ...
    describe('CORS and Security', () => {
        test('should validate content security', () => {
            const validateSecurity = (content) => {
                if (content.includes('<script>') || content.includes('javascript:')) {
                    throw new Error('Unsafe content detected');
                }
            };
            expect(() => validateSecurity('safe')).not.toThrow();
            expect(() => validateSecurity('<script>alert(1)</script>')).toThrow('Unsafe content detected');
        });
...
//# sourceMappingURL=browser-mocks.test.js.map