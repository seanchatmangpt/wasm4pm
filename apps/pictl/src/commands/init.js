 ...
 */
async function safeWriteFile(filepath, content, force, formatter, outputFormat) {
    if (existsSync(filepath) && !force) {
        if (outputFormat === 'human') {
            formatter.warn(`File already exists: ${filepath} (use --force to overwrite)`);
        }
        return false;
    }
...
async function validateConfigFiles(dirpath, formatter, outputFormat) {
    ...
    if (existsSync(tomlPath)) {
        try {
            ...
            throw new Error(`Configuration validation failed for ${tomlPath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    if (existsSync(jsonPath)) {
        try {
            ...
            // Config validation error is FATAL — user must fix it
            throw new Error(`Configuration validation failed for ${jsonPath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
export const init = defineCommand({
    ...
    async run(ctx) {
        const outputFormat = ctx.args.format;
        ...
        try {
            ...
            const configFormat = (ctx.args.configFormat || 'toml').toLowerCase();
