...
```bash
wpm explain --config config.toml --mode brief
```
...
```bash
WASM4PM_DEBUG=1 wpm run --config config.toml --verbose
```
...
```bash
wpm explain --config config.toml --expand-env
```
...
```bash
wpm explain --config config.toml --show-provenance
```
...
```bash
wpm run --config config.toml --dry-run
```
