 ...
 *   3. CLI     — wasm4pm run --algorithm X exits 0 or 3 (never 1=config or 2=source)
 *   4. CLI     — wasm4pm compare with all 14 IDs comma-joined exits 0 or 3
 *
...
import { wasm4pm, createCliTestEnv, EXIT_CODES } from '@wasm4pm/testing';
...
describe('all algorithms: config layer', () => {
    ...
    it('resolveConfig rejects an algorithm not in the registry', async () => {
        await expect(resolveConfig({ cliOverrides: { algorithm: 'made_up_algo' }, configSearchPaths: [] })).rejects.toThrow(/validation|invalid/i);
    });
});
...
describe('all algorithms: planner layer', () => {
    ...
    for (const id of ALGORITHM_IDS) {
        it(`plan() with algorithm="${id}" produces steps including a discover step`, () => {
            if (!plan)
                return;
            const p = plan({ version: '1.0', source: { kind: 'file', format: 'xes' }, execution: { profile: 'balanced' }, algorithm: { name: id, parameters: {} } });
    ...
    it('all 14 algorithms produce distinct plan hashes', () => {
        if (!plan)
            return;
        const hashes = ALGORITHM_IDS.map((id) => plan({ version: '1.0', source: { kind: 'file', format: 'xes' }, execution: { profile: 'balanced' }, algorithm: { name: id, parameters: {} } }).hash);
...
// ── 3. CLI: wasm4pm run --algorithm X exits 0 or 3, never 1 or 2 ────────────────
...
// ── 4. CLI: wasm4pm compare with all algorithms ────────────────────────────────
describe('all algorithms: CLI compare layer', () => {
    it('wasm4pm compare accepts all 14 algorithm IDs comma-joined', async () => {
    ...
    it('wasm4pm compare with unknown algorithm exits 2', async () => {
        const result = await wasm4pm(['compare', 'dfg,ghost_algo', '-i', xesPath]);
        expect(result.exitCode).toBe(EXIT_CODES.SOURCE_ERROR);
    }, 20000);
});
//# sourceMappingURL=08-all-algorithms.js.map