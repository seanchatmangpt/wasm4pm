 ...
 */
export async function runSwarm(config) {
    ...
    const workerSpecs = buildWorkerSpecs(config);
    for (let ep = 0; ep < maxEpisodes; ep++) {
        const episodeId = `swarm-ep-${Date.now()}-${ep}`;
        // Fan-out: run all workers in parallel
        const workerResults = await Promise.all(workerSpecs.map(spec => runWorker(spec, config)));
        ...
        const { converged, stableWorkers, unstableWorkers, agreementRate } = checkSwarmConvergence(workerResults, hashHistory, convergenceRuns);
...
// ── Internal helpers ─────────────────────────────────────────────────────────
function buildWorkerSpecs(config) {
...
async function runWorker(spec, config) {
...
function buildArtifact(episodes) {
    const lastEpisode = episodes[episodes.length - 1];
    return {
