...
function trendArrow(trend) {
    if (trend === 'rising')
        return `${RED}↑ rising${RESET}`;
    if (trend === 'falling')
        return `${GREEN}↓ falling${RESET}`;
...
export const driftWatch = defineCommand({
    ...
    async run(ctx) {
        ...
        const activityKey = ctx.args['activity-key'] || DEFAULT_ACTIVITY_KEY;
        const rawWindow = ctx.args.window;
        ...
        const rawInterval = ctx.args.interval;
        ...
        const jsonMode = ctx.args.json === true;
        const enhancedMode = ctx.args.enhanced === true;
        ...
        const MAX_DISTANCE_HISTORY = 10000;
        if (!jsonMode) {
            console.log(`${BOLD}[drift-watch]${RESET} Streaming EWMA drift monitor started`);
            console.log(`  file=${inputPath}  activity-key=${activityKey}  window=${windowSize}  interval=${intervalMs}ms  α=${ewmaAlpha}  threshold=${driftThreshold}`);
        ...
        const tick = async () => {
            ...
            let driftResult;
            try {
                const raw = wasm.detect_drift(logHandle, activityKey, windowSize);
                driftResult = JSON.parse(raw);
            }
            catch (err) {
            ...
            let ewmaResult;
            try {
                const raw = wasm.compute_ewma(JSON.stringify(distanceHistory), ewmaAlpha);
                ewmaResult = JSON.parse(raw);
            }
            catch (err) {
            ...
            if (jsonMode) {
                ...
                const statusLine = `${CYAN}[${ts}]${RESET} ` +
                ...
                if (newDriftCount > 0) {
                    ...
                    const alertLine = `${BOLD}${RED}  ⚠  ALERT${RESET} — ${newDriftCount} new drift point${newDriftCount !== 1 ? 's' : ''} ` +
            ...
            if (enhancedMode && distanceHistory.length >= 10) {
                try {
                    ...
                    const peakIndices = anomalyResult.peakIndices;
                    ...
                    if (jsonMode) {
                        const anomalyLine = {
                            ...
                            original_length: anomalyResult.originalLength,
                        };
                        process.stdout.write(JSON.stringify(anomalyLine) + '\n');
                    }
                    else if (peakCount > 0) {
                        console.log(`${BOLD}${YELLOW}  ML Anomaly${RESET} — ${peakCount} peak${peakCount !== 1 ? 's' : ''} detected in drift signal`);
                    }
                }
                catch (err) {
                    console.error(`[drift-watch] Enhanced anomaly detection failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        };
...
//# sourceMappingURL=drift-watch.js.map