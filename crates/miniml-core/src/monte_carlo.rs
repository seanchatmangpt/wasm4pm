...
impl MonteCarloBootstrapResult {
    ...
    pub fn n_bootstrap(&self) -> usize {
        self.n_bootstrap
    }

    #[wasm_bindgen(getter, js_name = "statisticName")]
    pub fn statistic_name(&self) -> String {
        self.statistic_name.clone()
    }
}
...
pub fn mc_bootstrap_impl(
    ...
    bootstrap_stats.sort_unstable_by(f64::total_cmp);
...
pub fn mc_expected_value(a: f64, b: f64, n_samples: usize, seed: u64) -> MonteCarloResult {
...
mod tests {
    ...
    fn test_mc_estimate_pi_accuracy() {
        ...
        assert!(
            (result.estimate - std::f64::consts::PI).abs() < 0.1,
            "pi estimate {} is too far from {}",
            result.estimate,
            std::f64::consts::PI
        );
    ...
    fn test_mc_integrate_constant() {
        ...
        assert!(
            (result.estimate - 5.0).abs() < 0.1,
            "integral of 5 should be ~5, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_integrate_linear() {
        ...
        assert!(
            (result.estimate - 1.0).abs() < 0.1,
            "integral of 2x should be ~1, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_integrate_sin() {
        ...
        assert!(
            (result.estimate - 2.0).abs() < 0.1,
            "integral of sin over [0,pi] should be ~2, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_integrate_multidim() {
        ...
        let upper = [1.0, 1.0];
        let result =
            mc_integrate_multidim_impl(|p| p[0] + p[1], &lower, &upper, 50_000, 42).unwrap();
        assert!(
            (result.estimate - 1.0).abs() < 0.1,
            "integral of x+y over unit square should be ~1, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_integrate_multidim_volume() {
        ...
        let lower = [0.0, 0.0];
        let upper = [2.0, 3.0];
        let result = mc_integrate_multidim_impl(|_| 1.0, &lower, &upper, 10_000, 42).unwrap();
        assert!(
            (result.estimate - 6.0).abs() < 0.1,
            "volume of [0,2]x[0,3] should be ~6, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_bootstrap_mean() {
        ...
        // Mean of 1..100 = 50.5
        assert!(
            (result.estimate - 50.5).abs() < 1.0,
            "bootstrap mean should be ~50.5, got {}",
            result.estimate
        );
    ...
    fn test_mc_bootstrap_median() {
        ...
        assert!(
            (result.estimate - 50.5).abs() < 1.0,
            "bootstrap median should be ~50.5, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_bootstrap_std() {
        ...
        // Std of 1..100 ≈ 29.01
        assert!(
            (result.estimate - 29.01).abs() < 1.0,
            "bootstrap std should be ~29, got {}",
            result.estimate
        );
    }
    ...
    fn test_mc_convergence_flag() {
        ...
        assert!(
            result.converged,
            "1M samples should converge within 0.01 of pi"
        );
    }
}
