...
fn assert_no_fraud(file_path: &str, names: &[&str]) {
    ...
    let content = fs::read_to_string(file_path)
        ...
        .or_else(|_| {
            ...
            let p = std::path::Path::new("src/breeds")
                .join(std::path::Path::new(file_path).file_name().unwrap());
            fs::read_to_string(p)
        });

    let src = content.unwrap_or_else(|_| {
        panic!(
            "MISSING SOURCE: {} — anti-fraud gate cannot silently skip",
            file_path
        )
    });
    for &name in names {
        ...
        if re.is_match(&src) {
            panic!(
                "FRAUD DETECTED: {} contains fresh oracle identifier {}",
                file_path, name
            );
        }
    }
...
fn anti_fraud_gate_bayesian_network() {
    assert_no_fraud(
        "src/breeds/bayesian_network.rs",
        &[
            "oracle_secret_Q",
            "oracle_secret_R",
            "oracle_secret_S",
...
fn anti_fraud_gate_default_logic() {
    assert_no_fraud(
        "src/breeds/default_logic.rs",
        &[
            "oracle_secret_gronk",
            "oracle_secret_wibble",
            "oracle_secret_dark_wibble",
        ],
    );
}
...
fn anti_fraud_gate_htn_planning() {
    assert_no_fraud(
        "src/breeds/htn_planning.rs",
        &[
            "oracle_secret_zorp_location",
            "oracle_secret_blee_station",
            "oracle_secret_zorp_credits",
        ],
    );
}
...
fn anti_fraud_gate_frames_inheritance() {
    assert_no_fraud(
        "src/breeds/frames_inheritance.rs",
        &["oracle_secret_zorp", "oracle_secret_blee"],
    );
}
...
fn anti_fraud_gate_description_logic() {
    assert_no_fraud(
        "src/breeds/description_logic.rs",
        &["oracle_secret_concept", "oracle_secret_role"],
    );
}
...
fn anti_fraud_gate_abductive_lp() {
    assert_no_fraud(
        "src/breeds/abductive_lp.rs",
        &["oracle_secret_abducible", "oracle_secret_observation"],
    );
}
...
fn anti_fraud_gate_abductive_ibe() {
    assert_no_fraud(
        "src/breeds/abductive_ibe.rs",
        &["oracle_secret_evidence", "oracle_secret_hypothesis"],
    );
}
...
fn anti_fraud_gate_partial_order_plan() {
    assert_no_fraud(
        "src/breeds/partial_order_plan.rs",
        &[
            "oracle_secret_zorp_pkg",
            "oracle_secret_blee_loc",
            "oracle_secret_glorp_loc",
        ],
    );
}
...
fn anti_fraud_gate_event_calculus() {
    assert_no_fraud(
        "src/breeds/event_calculus.rs",
        &["oracle_secret_zorp_fluent", "oracle_secret_blee_event"],
    );
}
...
fn anti_fraud_gate_mdp() {
    assert_no_fraud(
        "src/breeds/mdp.rs",
        &["oracle_secret_zorp_state", "oracle_secret_blee_action"],
    );
}
...
fn anti_fraud_gate_version_space() {
    assert_no_fraud(
        "src/breeds/version_space.rs",
        &["oracle_secret_zorp_attr", "oracle_secret_blee_val"],
    );
}
...
fn anti_fraud_gate_belief_merging() {
    assert_no_fraud(
        "src/breeds/belief_merging.rs",
        &["oracle_secret_zorp_atom", "oracle_secret_blee_atom"],
    );
}
...
fn anti_fraud_gate_qualitative_reason() {
    assert_no_fraud(
        "src/breeds/qualitative_reason.rs",
        &["oracle_secret_zorp_var", "oracle_secret_blee_var"],
    );
}
...
fn anti_fraud_gate_script_sam() {
    assert_no_fraud(
        "src/breeds/script_sam.rs",
        &["oracle_secret_zorp_script", "oracle_secret_blee_scene"],
    );
}
...
fn anti_fraud_gate_situation_calculus() {
    assert_no_fraud(
        "src/breeds/situation_calculus.rs",
        &["oracle_secret_situation", "oracle_secret_fluent"],
    );
}
...
fn anti_fraud_gate_circumscription() {
    assert_no_fraud(
        "src/breeds/circumscription.rs",
        &["oracle_secret_penguin", "oracle_secret_opus"],
    );
}
...
fn anti_fraud_gate_analogy_sme() {
    assert_no_fraud(
        "src/breeds/analogy_sme.rs",
        &["oracle_secret_solar", "oracle_secret_atom"],
    );
}
...
fn anti_fraud_gate_act_r() {
    assert_no_fraud(
        "src/breeds/act_r.rs",
        &["oracle_secret_chunk", "oracle_secret_utility"],
    );
}
...
fn anti_fraud_gate_problog() {
    assert_no_fraud(
        "src/breeds/problog.rs",
        &["oracle_secret_pfact", "oracle_secret_world"],
    );
}
...
fn anti_fraud_gate_sat_cdcl() {
    assert_no_fraud(
        "src/breeds/sat_cdcl.rs",
        &["oracle_secret_clause", "oracle_secret_literal"],
    );
}
...
fn anti_fraud_gate_episodic_memory() {
    assert_no_fraud(
        "src/breeds/episodic_memory.rs",
        &["oracle_secret_episode", "oracle_secret_kernel"],
    );
}
...
fn anti_fraud_gate_rl_symbolic() {
    assert_no_fraud(
        "src/breeds/rl_symbolic.rs",
        &["oracle_secret_reward", "oracle_secret_gamma"],
    );
}
...
fn anti_fraud_gate_ctl_check() {
    assert_no_fraud(
        "src/breeds/ctl_check.rs",
        &["oracle_secret_formula", "oracle_secret_path"],
    );
}
...
fn anti_fraud_gate_ilp() {
    assert_no_fraud(
        "src/breeds/ilp.rs",
        &["oracle_secret_foil", "oracle_secret_gain"],
    );
}
...
fn anti_fraud_gate_naive_physics() {
    assert_no_fraud(
        "src/breeds/naive_physics.rs",
        &["oracle_secret_scene", "oracle_secret_axiom"],
    );
}
...
fn anti_fraud_universal_sweep_all_breed_modules() {
    ...
    assert!(
        names.len() >= 30,
        "fresh-name extraction broken: {} names",
        names.len()
    );
    ...
    for entry in fs::read_dir(&breeds_dir).expect("breeds dir readable") {
        ...
        if matches!(
            fname.as_str(),
            "mod.rs" | "dispatch.rs" | "registration.rs" | "standing.rs" | "oracle_chain.rs"
        ) {
            continue;
        }
    ...
    assert!(
        checked >= 55,
        "universal sweep covered only {} breed modules",
        checked
    );
}
