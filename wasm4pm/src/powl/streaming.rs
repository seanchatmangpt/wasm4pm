...
impl Default for AlertConfig {
    fn default() -> Self { Self { fitness_threshold: 0.8, perfect_rate_threshold: 0.5, missing_tokens_threshold: 5.0 } }
}
...
impl StreamingConformance {
    pub fn new(net: PowlPetriNet, initial_marking: PowlMarking, final_marking: PowlMarking) -> Self {
        Self {
            net, initial_marking, final_marking, alert_config: AlertConfig::default(),
            total_produced: 0, total_consumed: 0, total_missing: 0, total_remaining: 0,
            traces_seen: 0, perfect_traces: 0, trace_fitness_sum: 0.0,
            window_size: 100, window: std::collections::VecDeque::new(),
        }
    }

    pub fn from_powl(arena: &PowlArena, root: u32) -> Result<Self, String> {
        let pn_result = to_petri_net::apply(arena, root);
        Ok(Self::new(pn_result.net, pn_result.initial_marking, pn_result.final_marking))
    }

    pub fn set_alert_config(&mut self, config: AlertConfig) { self.alert_config = config; }
    pub fn set_window_size(&mut self, n: usize) { self.window_size = n.max(1); }
    ...
    pub fn push_trace(&mut self, trace: &Trace) -> (TraceReplayResult, Vec<Alert>) {
        ...
        if result.is_perfect() { self.perfect_traces += 1; }
    ...
    pub fn push_all<'a>(&mut self, traces: impl IntoIterator<Item = &'a Trace>) -> ConformanceSnapshot {
        for trace in traces { self.push_trace(trace); }
        self.snapshot()
    }

    pub fn fitness(&self) -> f64 {
        if self.total_produced == 0 && self.total_consumed == 0 { return 1.0; }
        let c = self.total_consumed as f64; let p = self.total_produced as f64;
        let m = self.total_missing as f64; let r = self.total_remaining as f64;
        (0.5 * (1.0 - m / c) + 0.5 * (1.0 - r / p)).clamp(0.0, 1.0)
    }

    pub fn windowed_fitness(&self) -> f64 {
        if self.window.is_empty() { return 1.0; }
    ...
    pub fn snapshot(&self) -> ConformanceSnapshot {
        ...
        let perfect_rate = if self.traces_seen == 0 { 1.0 } else { self.perfect_traces as f64 / self.traces_seen as f64 };
        let avg_trace_fitness = if self.traces_seen == 0 { 1.0 } else { self.trace_fitness_sum / self.traces_seen as f64 };
        let avg_missing = if self.traces_seen == 0 { 0.0 } else { self.total_missing as f64 / self.traces_seen as f64 };
        ConformanceSnapshot {
            fitness, avg_trace_fitness, perfect_rate,
            traces_seen: self.traces_seen, perfect_traces: self.perfect_traces,
            total_missing: self.total_missing, total_remaining: self.total_remaining,
            total_produced: self.total_produced, total_consumed: self.total_consumed,
            avg_missing_per_trace: avg_missing, alerts: self.check_alerts(),
        }
    }

    pub fn reset(&mut self) {
        self.total_produced = 0; self.total_consumed = 0; self.total_missing = 0;
        self.total_remaining = 0; self.traces_seen = 0; self.perfect_traces = 0;
        self.trace_fitness_sum = 0.0; self.window.clear();
    }
    ...
    fn check_alerts(&self) -> Vec<Alert> {
        ...
        if fitness < self.alert_config.fitness_threshold {
            alerts.push(Alert::FitnessBelowThreshold { current: fitness, threshold: self.alert_config.fitness_threshold });
        }
        if perfect_rate < self.alert_config.perfect_rate_threshold {
            alerts.push(Alert::PerfectRateBelow { current: perfect_rate, threshold: self.alert_config.perfect_rate_threshold });
        }
        if avg_missing > self.alert_config.missing_tokens_threshold {
            alerts.push(Alert::MissingTokensExceeded { current: avg_missing, threshold: self.alert_config.missing_tokens_threshold });
        }
        alerts
    }
}
...
mod tests {
    ...
    fn make_trace(case_id: &str, acts: &[&str]) -> Trace {
        Trace {
            ...
            events: acts.iter().map(|&a| Event { name: a.to_string(), timestamp: None, lifecycle: None, attributes: HashMap::new() }).collect(),
        }
    }
    ...
    fn alert_fires_on_low_fitness() {
        let (arena, root) = parse("PO=(nodes={A, B}, order={A-->B})");
        let mut sc = StreamingConformance::from_powl(&arena, root).unwrap();
        sc.set_alert_config(AlertConfig { fitness_threshold: 0.99, perfect_rate_threshold: 0.0, missing_tokens_threshold: 100.0 });
