 ...
 * process mining tool. This scenario validates that `wpm social` exists, is callable,
...
describe.sequential('Social Command Alive', () => {
    it('wasm4pm social command exists and is callable — Rank 2: domain contract', async () => {
        // JTBD: "The social command must exist in the CLI and be callable"
        const result = await wasm4pm(['social', '--help']);
        // Command should show help without crashing
        expect([0, 1, 2]).toContain(result.exitCode);
    });
    it('wasm4pm social accepts input file without crashing — Rank 2: domain contract', async () => {
        // JTBD: "The social command must handle input files gracefully"
        const result = await wasm4pm(['social', RUNNING_EXAMPLE, '--format', 'json']);
    ...
    it('wasm4pm social returns valid JSON output — Rank 2: domain contract', async () => {
    ...
    it('wasm4pm social on success returns handover network with nodes — Rank 1: mathematical invariant', async () => {
        // JTBD: "When social succeeds, handover network must have nodes array"
        const result = await wasm4pm(['social', RUNNING_EXAMPLE, '--format', 'json', '--metric', 'handover']);
    ...
    it('wasm4pm social --metric working-together returns valid network — Rank 1: mathematical invariant', async () => {
        // JTBD: "Working-together metric must produce valid network output"
        const result = await wasm4pm(['social', RUNNING_EXAMPLE, '--format', 'json', '--metric', 'working-together']);
        if (result.exitCode === 0) {
            const output = extractJson(result.stdout);
            expect(output.status).toBe('success');
    ...
    it('wasm4pm social --metric similar-task returns graceful response — Rank 2: domain contract', async () => {
        // JTBD: "Similar-task metric must not crash, even if not fully supported"
        const result = await wasm4pm(['social', RUNNING_EXAMPLE, '--format', 'json', '--metric', 'similar-task']);
