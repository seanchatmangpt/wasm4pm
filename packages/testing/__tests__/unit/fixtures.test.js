...
import { MINIMAL_CONFIG, BALANCED_ALPHA_CONFIG, QUALITY_GENETIC_CONFIG, STREAM_WATCH_CONFIG, HTTP_OTEL_CONFIG, PIPELINE_DAG_CONFIG, INLINE_CONTENT_CONFIG, BARE_MINIMUM_CONFIG, MAX_RESOURCES_CONFIG, INVALID_CONFIGS, ALL_VALID_CONFIGS, } from '../../src/fixtures/configs.js';
import { SIMPLE_SEQUENTIAL, PARALLEL_SPLIT, EXCLUSIVE_CHOICE, LOOP_PROCESS, EMPTY_LOG, SINGLE_EVENT, generateSyntheticLog, SAMPLE_XES, } from '../../src/fixtures/events.js';
import { SUCCESS_RECEIPT, PARTIAL_RECEIPT, FAILED_RECEIPT, validateReceiptShape, validateExplainOutput, } from '../../src/fixtures/expected-outputs.js';
describe('Config Fixtures', () => {
    ...
    describe('Invalid Configs', () => {
        ...
        it('invalidProfile has bad profile value', () => {
            expect(INVALID_CONFIGS.invalidProfile.execution.profile).toBe('invalid');
        });
        it('negativeTimeout has negative timeout', () => {
            expect(INVALID_CONFIGS.negativeTimeout.execution.timeout).toBeLessThan(0);
        });
        it('nullSource has null source', () => {
            expect(INVALID_CONFIGS.nullSource.source).toBeNull();
        });
        it('emptyPipeline has zero pipeline steps', () => {
            expect(INVALID_CONFIGS.emptyPipeline.pipeline).toHaveLength(0);
        });
        it('cyclicPipeline has circular dependencies', () => {
            const pipeline = INVALID_CONFIGS.cyclicPipeline.pipeline;
