...
- Simulate what an end user does after `npm install wasm4pm`
...
```
tests/
  nodejs.test.ts       # WASM library API surface (require('wasm4pm'))
...
```bash
cd lab
npm install           # installs wasm4pm from npm (not workspace source)
npm test              # 124 tests across 3 files
```

## Dependency

`lab/` depends on `"wasm4pm": "^26.4.5"` — the published npm package, not `workspace:*`.
