...
import { fingerprintConfig, diffConfigs } from '../hash.js';
describe('Resolution', () => {
    let tmpDir;
    beforeEach(async () => {
    ...
    describe('resolution order: CLI > TOML > JSON > ENV > defaults', () => {
        ...
        it('JSON file overrides env', async () => {
            await fs.writeFile(path.join(tmpDir, 'wasm4pm.json'), JSON.stringify({ version: '26.4.5', source: { kind: 'file' }, execution: { profile: 'fast' } }));
            const cfg = await resolveConfig({
                configSearchPaths: [tmpDir],
                env: { WASM4PM_PROFILE: 'stream' },
            });
    ...
    describe('source, sink, algorithm', () => {
        it('loads source config from TOML', async () => {
            await fs.writeFile(path.join(tmpDir, 'pictl.toml'), `version = "1.0.0"\n[source]\nkind = "http"\nurl = "http://localhost:9000/events"`);
            const cfg = await resolveConfig({ configSearchPaths: [tmpDir] });
