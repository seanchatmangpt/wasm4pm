...
describe('autoprocess command (Aalst methodology)', () => {
    ...
    it('completes successfully (exit code 0) with valid structure', async () => {
        ...
        const cycleResult = json['cycle_result'];
    ...
    it('processes BPI 2020 real-scale dataset (20MB) with valid result', async () => {
        ...
        expect(result.durationMs).toBeLessThan(90000);
        ...
        expect(eventCount).toBeGreaterThanOrEqual(1000);
    });
    /**
    ...
    it('human format output shows phase execution sequence', async () => {
        ...
        const hasPerception = combined.includes('Perception') || combined.includes('perception');
        const hasEvents = combined.includes('Events') || combined.includes('events');
        const hasDecision = combined.includes('Decision') || combined.includes('decision');
        ...
        const hasError = combined.includes('AutoProcess failed') || combined.includes('Error');
...
//# sourceMappingURL=20-autoprocess-aalst.js.map