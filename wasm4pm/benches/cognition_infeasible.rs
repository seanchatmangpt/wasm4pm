...
fn real_observation() -> Observation {
    ...
    Observation { traces, activities: 96 }
}
...
fn precedence_closure(obs: &Observation) -> Vec<Vec<u64>> {
    ...
    for k in 0..obs.activities {
        ...
        for row in &mut closure {
            if row[kw] & km != 0 {
                for (dst, src) in row.iter_mut().zip(&row_k) { *dst |= *src; }
            }
        }
    }
    closure
...
fn policy_frontier(obs: &Observation, closure: &[Vec<u64>], dimensions: usize) -> (usize, [u8; 32]) {
    ...
    for trace in &obs.traces {
        ...
        for &a in trace { h.update(a.to_le_bytes()); }
        let d = h.finalize();
        for dim in 0..dimensions {
            ...
            frontier.entry(key).and_modify(|old| {
                if score.0 >= old.0 && score.1 <= old.1 { *old = score; }
            }).or_insert(score);
        }
    }
    let mut receipt = Sha256::new();
    for (k, v) in &frontier {
        receipt.update(k.0.to_le_bytes()); receipt.update(k.1.to_le_bytes()); receipt.update(k.2.to_le_bytes());
        receipt.update(v.0.to_le_bytes()); receipt.update(v.1.to_le_bytes());
    }
    (frontier.len(), receipt.finalize().into())
}

fn counterfactual_blast_radius(obs: &Observation, closure: &[Vec<u64>], changes: usize) -> ([u64; 4], [u8; 32]) {
    ...
    for i in 0..changes {
        let seed = obs.traces[i % obs.traces.len()][i % obs.traces[i % obs.traces.len()].len()] as usize;
        affected[seed / 64] |= 1 << (seed % 64);
        for (dst, src) in affected.iter_mut().zip(&closure[seed]) { *dst |= *src; }
    }
    let mut h = Sha256::new();
    for w in affected { h.update(w.to_le_bytes()); }
    (affected, h.finalize().into())
}

fn adversarial_ambiguity(obs: &Observation, closure: &[Vec<u64>], hypotheses: usize) -> (u64, [u8; 32]) {
    ...
    for trace in &obs.traces {
        for i in 0..hypotheses {
            ...
            let reach = closure[a].iter().map(|w| w.count_ones() as u64).sum::<u64>();
            signatures.insert((a as u16, (reach % 257) as u16, trace.len() as u16));
        }
    }
    let mut h = Sha256::new();
    for s in &signatures { h.update(s.0.to_le_bytes()); h.update(s.1.to_le_bytes()); h.update(s.2.to_le_bytes()); }
...
fn bench_cognition(c: &mut Criterion) {
    ...
    for &dims in &[24usize, 32, 40] {
        g.bench_with_input(BenchmarkId::new("policy_frontier_5_pow_d", dims), &dims, |b, &d| {
            b.iter(|| black_box(policy_frontier(black_box(&obs), black_box(&closure), d)))
        });
    }

    for &changes in &[8usize, 16, 32] {
        g.bench_with_input(BenchmarkId::new("counterfactual_blast_radius_2_pow_96", changes), &changes, |b, &n| {
            b.iter(|| black_box(counterfactual_blast_radius(black_box(&obs), black_box(&closure), n)))
        });
    }

    for &depth in &[8usize, 12, 16] {
        g.bench_with_input(BenchmarkId::new("ambiguity_96_pow_depth", depth), &depth, |b, &d| {
            b.iter(|| black_box(adversarial_ambiguity(black_box(&obs), black_box(&closure), d)))
        });
    }
    g.finish();
}
