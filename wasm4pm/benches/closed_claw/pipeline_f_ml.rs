...
const STREAMING_ACTIVITIES: &[&str] = &[
    "Register", "Validate", "Check_Docs", "Assess_Risk",
    "Calculate_Fee", "Send_Invoice", "Confirm_Payment",
    "Approve_Basic", "Approve_Senior", "Notify_Applicant",
    "Create_Record", "Archive", "Close", "Reject",
];

fn generate_streaming_events(num_cases: usize, avg_events_per_case: usize) -> Vec<(String, String)> {
...
fn encode_streaming_to_columnar(events: &[(String, String)]) -> (Vec<u32>, Vec<usize>, Vec<String>) {
    ...
    for (case_id, activity) in events {
        ...
        case_events.entry(case_id.clone()).or_insert_with(|| {
            ...
            |b, events| {
                b.iter(|| {
                    ...
                    for (_case_id, trace) in &open_traces {
                        ...
                        for &id in trace { *node_counts.entry(id).or_insert(0) += 1; }
                        for pair in trace.windows(2) {
                            *edge_counts.entry((pair[0], pair[1])).or_insert(0) += 1;
                        }
                        *start_counts.entry(trace[0]).or_insert(0) += 1;
                        if let Some(&last) = trace.last() { *end_counts.entry(last).or_insert(0) += 1; }
                    }
...
fn bench_anomaly_detection(c: &mut Criterion) {
    ...
    for num_cases in [500usize, 2_000, 5_000] {
        let shape = LogShape { num_cases, avg_events_per_case: 15, num_activities: 14, noise_factor: 0.1 };
        let log = generate_event_log(&shape);
        let mut edge_freq: HashMap<(String, String), usize> = HashMap::new();
        let mut total_edges: usize = 0;
        for trace in &log.traces {
            let activities: Vec<&str> = trace.events.iter()
                .filter_map(|e| e.attributes.get(ACTIVITY_KEY).and_then(|v| v.as_string()))
                .collect();
            for pair in activities.windows(2) {
                *edge_freq.entry((pair[0].to_string(), pair[1].to_string())).or_insert(0) += 1;
                total_edges += 1;
            }
        }
        let total_f = total_edges.max(1) as f64;
        let traces: Vec<Vec<String>> = log.traces.iter().map(|t| {
            t.events.iter()
                .filter_map(|e| e.attributes.get(ACTIVITY_KEY).and_then(|v| v.as_string()).map(|s| s.to_string()))
                .collect()
        }).collect();
            ...
            |b, (traces, edge_freq, total_f)| {
                b.iter(|| {
                    ...
                    for trace in black_box(traces).iter() {
                        if trace.len() < 2 { scores.push(0.0); continue; }
                        let steps = trace.len() - 1;
                        let mut cost_sum = 0.0_f64;
                        for i in 0..steps {
                            let freq = edge_freq.get(&(trace[i].clone(), trace[i + 1].clone())).copied().unwrap_or(0);
                            cost_sum += if freq == 0 { missing_edge_cost } else { -(freq as f64 / *black_box(total_f)).log2() };
                        }
...
fn bench_drift_detection(c: &mut Criterion) {
    ...
    for num_cases in [1_000usize, 5_000, 10_000] {
        ...
        let trace_activity_sets: Vec<HashSet<String>> = log.traces.iter().map(|t| {
            t.events.iter()
                .filter_map(|e| e.attributes.get(ACTIVITY_KEY).and_then(|v| v.as_string()).map(|s| s.to_string()))
                .collect()
        }).collect();
            ...
            |b, (trace_sets, window_size)| {
                b.iter(|| {
                    ...
                    for window in black_box(trace_sets).windows(*black_box(window_size)) {
                        ...
                        for set in window { for act in set { current.insert(act); } }
                        if !prev_activities.is_empty() {
                            ...
                            if jaccard_distance > drift_threshold { drift_count += 1; }
                        }
