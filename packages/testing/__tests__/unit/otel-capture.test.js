...
import { createOtelCapture } from '../../src/harness/otel-capture.js';
describe('OtelCapture', () => {
    ...
    describe('assertNonBlocking', () => {
        it('passes for fast spans', () => {
            ...
            capture.captureSpan(makeSpan({ startTime: now, endTime: now + 1000000 })); // 1ms
            const errors = capture.assertNonBlocking(100);
            expect(errors).toHaveLength(0);
        });
        it('fails for slow spans', () => {
            const now = Date.now() * 1000000;
            capture.captureSpan(makeSpan({ startTime: now, endTime: now + 200000000 })); // 200ms
            const errors = capture.assertNonBlocking(100);
            expect(errors).toHaveLength(1);
...
//# sourceMappingURL=otel-capture.test.js.map