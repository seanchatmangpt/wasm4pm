#!/bin/bash
# PostToolUseFailure Hook: wasm4pm Recovery Injection
#
...
if [ "$TOOL_NAME" = "Bash" ]; then
  ...
  if echo "$COMMAND" | grep -q "npm\|pnpm"; then
    ...
    RECOVERY="TypeScript check failed: try 'pnpm lint' or check 'apps/wasm4pm/tsconfig.json'"
    ...
    RECOVERY="Run 'wpm doctor' to check environment health"
  fi
fi

if [ -z "$RECOVERY" ]; then
  RECOVERY="Tool '$TOOL_NAME' failed. Run 'wpm doctor' to check environment."
fi

# Output recovery suggestion to stderr (Claude sees this as feedback)
{
  echo "[wasm4pm recovery] $TOOL_NAME failed"
