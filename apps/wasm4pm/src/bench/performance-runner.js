...
const TARGET_MIN_THROUGHPUT = 250000;
 ...
 */
class Lcg {
    constructor(seed = 0xdeadbeefcafebaben) {
        this.state = seed;
    }
    next() {
 ...
 */
function buildSyntheticXes(numCases, avgEventsPerCase, numActivities, noiseFactor) {
...
function computeStats(samplesNs) {
    if (samplesNs.length === 0)
        throw new Error('computeStats: empty sample set');
 ...
 */
function sampleThermalMacOs() {
    try {
        const raw = execSync('sudo powermetrics --samplers smc --sample-count 1 --sample-rate 1000 2>/dev/null', { timeout: 3000, encoding: 'utf-8' });
...
const ALGORITHM_DEFS = [
    ...
    { id: 'alpha_plus_plus', invoke: (w, h, k) => w['discover_alpha_plus_plus'](h, k, 0.0), maxCases: 10000 },
    { id: 'heuristic_miner', invoke: (w, h, k) => w['discover_heuristic_miner'](h, k, 0.5), maxCases: 10000 },
    { id: 'inductive_miner', invoke: (w, h, k) => w['discover_inductive_miner'](h, k), maxCases: 10000 },
    { id: 'hill_climbing', invoke: (w, h, k) => w['discover_hill_climbing'](h, k), maxCases: 1000 },
    { id: 'declare', invoke: (w, h, k) => w['discover_declare'](h, k), maxCases: 10000 },
    { id: 'simulated_annealing', invoke: (w, h, k) => w['discover_simulated_annealing'](h, k, 1.0, 0.95), maxCases: 1000 },
    { id: 'a_star', invoke: (w, h, k) => w['discover_astar'](h, k, 500), maxCases: 1000 },
    { id: 'ant_colony', invoke: (w, h, k) => w['discover_ant_colony'](h, k, 20, 20), maxCases: 1000 },
    { id: 'pso', invoke: (w, h, k) => w['discover_pso_algorithm'](h, k, 20, 20), maxCases: 1000 },
    { id: 'genetic_algorithm', invoke: (w, h, k) => w['discover_genetic_algorithm'](h, k, 20, 20), maxCases: 1000 },
    { id: 'ilp', invoke: (w, h, k) => w['discover_ilp_petri_net'](h, k), maxCases: 1000 },
    ...
    { id: 'ml_classify', invoke: (w, h, k) => w['ml_classify'](h, k, 'knn', 3), maxCases: 5000 },
    { id: 'ml_cluster', invoke: (w, h, k) => w['ml_cluster'](h, k, 'kmeans', 5), maxCases: 5000 },
    { id: 'ml_forecast', invoke: (w, h, k) => w['ml_forecast'](h, k, 10), maxCases: 5000 },
    { id: 'ml_anomaly', invoke: (w, h, k) => w['ml_anomaly'](h, k, 2.0), maxCases: 5000 },
    { id: 'ml_regress', invoke: (w, h, k) => w['ml_regress'](h, k, 'linear'), maxCases: 5000 },
    { id: 'ml_pca', invoke: (w, h, k) => w['ml_pca'](h, k, 2), maxCases: 5000 },
];
 ...
 */
async function benchmarkAlgorithm(wasm, def, xes, totalEvents, opts) {
    const { activityKey, samples, warmup } = opts;
    // Availability probe: load, invoke once, discard
    let invokeOk = true;
    try {
        const probeHandle = wasm['load_eventlog_from_xes'](xes);
        try {
...
function formatHumanReport(report) {
    ...
    const DIM = '\x1b[2m';
    const RESET = '\x1b[0m';
    const pass = (ok) => ok ? `${GREEN}PASS${RESET}` : `${RED}FAIL${RESET}`;
    ...
    lines.push('  Algorithm'.padEnd(26) +
    ...
    for (const r of report.algorithms) {
        ...
        lines.push('  ' + r.algorithm.padEnd(24) +
...
async function saveReport(report, overridePath) {
...
export async function runBenchmark(options = {}) {
    ...
    if (format === 'human') {
        process.stdout.write(`wasm4pm performance benchmark — ${opts.samples} samples, ${opts.numCases} cases\n`);
    ...
    if (format === 'human') {
        process.stdout.write(`Generating synthetic XES log (${opts.numCases} cases, ~${opts.avgEventsPerCase} avg events/case)...\n`);
    }
    const { xes, totalEvents } = buildSyntheticXes(opts.numCases, opts.avgEventsPerCase, opts.numActivities, opts.noiseFactor);
    if (format === 'human')
        process.stdout.write('Sampling thermal baseline...\n');
    const thermal = collectThermal();
    const gpu = collectGpu();
    const defsToRun = options.algorithms && options.algorithms.length > 0
        ? ALGORITHM_DEFS.filter(d => options.algorithms.includes(d.id))
        : ALGORITHM_DEFS;
    const results = [];
    for (const def of defsToRun) {
        ...
        if (format === 'human')
            process.stdout.write(`  Benchmarking ${def.id}...`);
        const result = await benchmarkAlgorithm(wasm, def, xes, totalEvents, opts);
        if (result == null) {
            if (format === 'human')
                process.stdout.write(' unavailable (skipped)\n');
            continue;
        }
        results.push(result);
        if (format === 'human') {
            const ok = result.meets_latency_target && result.meets_throughput_target;
            process.stdout.write(` ${ok ? 'ok' : 'FAIL'}  ${result.per_state_mean_ns.toFixed(2)} ns/state  ` +
...
const isMain = scriptName.endsWith('performance-runner.js') ||
...
if (isMain) {
    ...
    runBenchmark(cliOpts)
        ...
        const allOk = report.summary.all_latency_targets_met &&
...
//# sourceMappingURL=performance-runner.js.map