...
function mae(actual, predicted) {
    ...
    let sum = 0;
    for (let i = 0; i < n; i++)
        sum += actual[i] - predicted[i] < 0 ? predicted[i] - actual[i] : actual[i] - predicted[i];
    return sum / n;
}
...
// ---------------------------------------------------------------------------
function knnBatch(col, labels, k) {
    const { cols, n, d } = col;
    ...
    for (let qi = 0; qi < n; qi++) {
        ...
        for (let i = 0; i < n && sortedLen < k; i++) {
            ...
            let pos = sortedLen;
            while (pos > 0 && distBuf[i] < distBuf[sorted[pos - 1]])
                pos--;
            // Shift right
            for (let s = sortedLen; s > pos; s--)
                sorted[s] = sorted[s - 1];
            sorted[pos] = i;
            sortedLen++;
        }
...
function gaussianNBTrain(data, labels) {
    const n = data.length;
    const d = data[0]?.length ?? 0;
...
function gaussianNBPredictBatch(model, data) {
    const n = data.length;
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let ci = 1; ci < nClasses; ci++) {
            if (logProbs[ci] > maxLp)
                maxLp = logProbs[ci];
        }
        let expSum = 0;
        for (let ci = 0; ci < nClasses; ci++)
            expSum += Math.exp(logProbs[ci] - maxLp);
...
// ---------------------------------------------------------------------------
function logisticRegressionTrain(data, labels, maxIter = 100, lr = 0.01) {
    ...
    for (let ci = 0; ci < k; ci++)
        weights.push(new Float64Array(d + 1));
    const probs = new Float64Array(k);
    for (let iter = 0; iter < maxIter; iter++) {
        const grads = [];
        for (let ci = 0; ci < k; ci++)
            grads.push(new Float64Array(d + 1));
        for (let i = 0; i < n; i++) {
...
function logisticRegressionPredictBatch(model, data) {
    const n = data.length;
    const d = data[0]?.length ?? 0;
    const results = new Array(n);
...
function buildTree(data, labels, indices, // pre-allocated index buffer
depth, maxDepth, d, classCount) {
...
function predictTree(node, query) {
    while (node.label === -1) {
 ...
 */
export async function classifyTraces(featuresJson, options = {}) {
 ...
 */
export async function regressRemainingTime(featuresJson, options = {}) {
