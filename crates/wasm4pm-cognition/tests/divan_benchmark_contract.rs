...
fn divan_matrix_is_exactly_the_55_admitted_cognitions() {
    ...
    assert_eq!(
        unique.len(),
        55,
        "BreedId::ALL contains duplicate cognition ids"
    );
}
...
fn every_admitted_cognition_has_real_benchmark_work_and_evidence() {
    for id in BreedId::ALL {
        ...
        assert_eq!(
            raw.breed, id,
            "{}: raw output attributed to another breed",
            id
        );
        assert!(
        ...
        assert_eq!(
            full.breed, id,
            "{}: full output attributed to another breed",
            id
        );
        assert!(
        ...
        assert!(
            full.ocel_log.is_some(),
            "{}: benchmark path emitted no OCEL evidence",
            id
        );
        assert!(
