...
let checkSwarmConvergence;
let checkMlConvergence;
let aggregate;
let sendDirective;
beforeEach(async () => {
...
describe('checkConvergence', () => {
    const makeResults = (hash, count, algo = 'alpha') => Array.from({ length: count }, (_, i) => ({
        workerId: `w${i}`,
        algorithmId: algo,
        resultHash: hash,
        result: {},
...
// ─── Aggregation ────────────────────────────────────────────────────────────────
describe('aggregate', () => {
    const makeResults = (algo, hash, result) => [
    ...
    it('should aggregate with union strategy (default)', () => {
        ...
        expect(agg.aggregate.nodes).toContain('C'); // union adds C
    });
    it('should aggregate with majority_vote strategy', () => {
        ...
        expect(agg.aggregate.answer).toBe('X');
    });
    it('should aggregate ML classification with ml_ensemble', () => {
        ...
        expect(agg.workersIncluded).toBe(2);
        expect(agg.aggregate.method).toBe('ensemble');
    });
    it('should aggregate ML forecast with ml_ensemble', () => {
        ...
        const forecast = agg.aggregate.forecast;
