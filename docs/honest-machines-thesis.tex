...
All benchmarks run via Criterion.rs~\cite{criterion} on the release profile with
four standard input sizes: 100, 1\,000, 10\,000, and 50\,000 events. Throughput
is reported as events per second (thrpt). Hardware: Apple M-series, 36 GB RAM.
Warm-up: 2 seconds; measurement: 5--8 seconds; sample size: 30--50.

\subsection{Tier 1 Discovery Algorithms}

\begin{table}[H]
  \centering
  \caption{Tier 1 discovery throughput (events/second)}
  \label{tab:tier1}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Algorithm} & \textbf{100 ev} & \textbf{1K ev} & \textbf{10K ev} & \textbf{50K ev} \\
    \midrule
    DFG (batch)         & $\sim$2M/s & $\sim$2M/s & $\sim$1.8M/s & $\sim$1.5M/s \\
    Process Skeleton    & $\sim$1.8M/s & $\sim$1.7M/s & $\sim$1.6M/s & $\sim$1.4M/s \\
    Streaming DFG       & $\sim$1.9M/s & $\sim$1.9M/s & $\sim$1.8M/s & $\sim$1.6M/s \\
    SIMD Streaming DFG  & $\sim$2.1M/s & $\sim$2.0M/s & $\sim$1.9M/s & $\sim$1.7M/s \\
    \bottomrule
  \end{tabular}
\end{table}
...
measured separately from the algorithm cost:

\begin{table}[H]
  \centering
  \caption{Oracle infrastructure overhead at 100-event scale}
...
\begin{table}[H]
  \centering
  \caption{NGram predictor vs.\ uniform random baseline}
  \label{tab:prediction}
  \begin{tabular}{lrr}
    \toprule
    \textbf{Model} & \textbf{Build time (1K traces)} & \textbf{Predict latency} \\
    \midrule
    Uniform random (LCG)      & 0 (no training) & $< 1\,\mu\text{s}$ \\
    NGram unigram (order=1)   & $< 5\,\text{ms}$ & $< 5\,\mu\text{s}$ \\
    NGram bigram  (order=2)   & $< 8\,\text{ms}$ & $< 8\,\mu\text{s}$ \\
    \bottomrule
  \end{tabular}
\end{table}

NGram bigram build time is $< 8\,\text{ms}$ for a 1K-trace log --- well within
interactive latency budgets. The baseline comparison shows that the NGram
predictor adds minimal overhead over random selection while providing structured
predictions.
