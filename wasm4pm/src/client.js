 ...
 */
import { asEventLogHandleId, asOCELHandleId, asDFGHandleId, asPetriNetHandleId, asDeclareHandleId, asTemporalProfileHandleId, asNGramPredictorHandleId, asStreamingDFGHandleId, asStreamingConformanceHandleId, } from './types.js';
/**
...
export class ProcessMiningClient {
    ...
    discoverTemporalProfile(log, options = {}) {
        ...
        const handle = asTemporalProfileHandleId(this.wasmModule.discover_temporal_profile(log.getId(), activityKey, timestampKey));
    ...
    buildNGramPredictor(log, options = {}) {
        ...
        const activityKey = options.activityKey || 'concept:name';
        const n = options.n || 3;
        const handle = asNGramPredictorHandleId(this.wasmModule.build_ngram_predictor(log.getId(), activityKey, n));
    ...
    beginStreamingConformance(dfg) {
        if (!this.initialized)
            throw new Error('Client not initialized. Call init() first.');
        const handle = asStreamingConformanceHandleId(this.wasmModule.streaming_conformance_begin(dfg.getId()));
...
export class EventLogHandle {
    ...
    discoverOptimizedDFG(options = {}) {
        ...
        const result = this.wasmModule.discover_optimized_dfg(this.handle, activityKey, fitnessWeight, simplicityWeight);
        return new DFGHandle(asDFGHandleId(result.handle), this.wasmModule);
    }
    /**
    ...
    discoverGeneticAlgorithm(options = {}) {
        ...
        const result = this.wasmModule.discover_genetic_algorithm(this.handle, activityKey, populationSize, generations);
        return new DFGHandle(asDFGHandleId(result.handle), this.wasmModule);
    }
    /**
    ...
    discoverPSOAlgorithm(options = {}) {
        ...
        const result = this.wasmModule.discover_pso_algorithm(this.handle, activityKey, swarmSize, iterations);
        return new DFGHandle(asDFGHandleId(result.handle), this.wasmModule);
    }
    /**
    ...
    mineSequentialPatterns(options = {}) {
        ...
        return this.wasmModule.mine_sequential_patterns(this.handle, activityKey, minSupport, patternLength);
    }
    /**
    ...
    discoverAntColony(options = {}) {
        ...
        const result = this.wasmModule.discover_ant_colony(this.handle, activityKey, numAnts, iterations);
        return new DFGHandle(asDFGHandleId(result.handle), this.wasmModule);
    }
    /**
    ...
    discoverSimulatedAnnealing(options = {}) {
        ...
        const result = this.wasmModule.discover_simulated_annealing(this.handle, activityKey, temperature, coolingRate);
        return new DFGHandle(asDFGHandleId(result.handle), this.wasmModule);
    }
    /**
     ...
     */
    extractCaseFeatures(activityKey = 'concept:name', timestampKey = 'time:timestamp', config = { features: [], target: 'outcome' }) {
        try {
            const result = this.wasmModule.extract_case_features(this.handle, activityKey, timestampKey, JSON.stringify(config));
     ...
     */
    extractPrefixFeatures(activityKey = 'concept:name', timestampKey = 'time:timestamp', prefixLength = 5) {
        try {
            const result = this.wasmModule.extract_prefix_features(this.handle, activityKey, timestampKey, prefixLength);
            return Promise.resolve(JSON.parse(result));
        }
        catch (error) {
     ...
     */
    exportFeaturesAsCSV(activityKey = 'concept:name', timestampKey = 'time:timestamp', config = { features: [], target: 'outcome' }) {
        try {
            const featuresJson = this.wasmModule.export_features_json(this.handle, activityKey, timestampKey, JSON.stringify(config));
    ...
    analyzeResourceUtilization(resourceKey = 'org:resource', timestampKey = 'time:timestamp') {
        try {
            const result = this.wasmModule.analyze_resource_utilization(this.handle, resourceKey, timestampKey);
            return Promise.resolve(JSON.parse(result));
        }
        catch (error) {
    ...
    analyzeResourceActivityMatrix(resourceKey = 'org:resource', activityKey = 'concept:name') {
        try {
            const result = this.wasmModule.analyze_resource_activity_matrix(this.handle, resourceKey, activityKey);
            return Promise.resolve(JSON.parse(result));
        }
        catch (error) {
     ...
     */
    identifyResourceBottlenecks(resourceKey = 'org:resource', timestampKey = 'time:timestamp', activityKey = 'concept:name') {
        try {
            const result = this.wasmModule.identify_resource_bottlenecks(this.handle, resourceKey, timestampKey, activityKey);
            return Promise.resolve(JSON.parse(result));
        }
        catch (error) {
...
export class TemporalProfileHandle {
    ...
    checkConformance(log, options = {}) {
        ...
        return this.wasmModule.check_temporal_conformance(log.getId(), this.handle, activityKey, timestampKey, zeta);
    }
    delete() {
        this.wasmModule.delete_object(this.handle);
    }
