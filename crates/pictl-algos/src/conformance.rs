...
pub fn check_conformance_token_replay(log: &EventLog, model: &DFG, activity_key: &str) -> Result<ConformanceResult> {
    ...
    for trace in &log.traces {
        ...
        let trace_fitness = TokenReplayResult::calculate_fitness(produced, consumed, missing, remaining);
    ...
    Ok(ConformanceResult::new(fitness, total_traces, fitting_traces, deviating))
}
...
fn align_trace_to_model(
    ...
    TraceAlignment {
        steps,
        total_cost: if total_cost == usize::MAX { n + m } else { total_cost },
    }
}
...
mod tests {
    ...
    fn make_dfg(edges: &[(&str, &str)], starts: &[&str], ends: &[&str]) -> DFG {
        ...
        for (s, t) in edges {
            ...
            dfg.edges.push(DFGEdge::new(s.to_string(), t.to_string(), 1));
        }
        for n in nodes {
    ...
    fn test_alignment_conforming_trace_zero_cost() {
        ...
        net.places.push(PetriNetPlace { id: "source".to_string(), label: "source".to_string(), initial_marking: 1 });
        net.places.push(PetriNetPlace { id: "p1".to_string(), label: "p1".to_string(), initial_marking: 0 });
        net.places.push(PetriNetPlace { id: "sink".to_string(), label: "sink".to_string(), initial_marking: 0 });
        net.transitions.push(PetriNetTransition::new("t_A".to_string(), "A".to_string(), false));
        net.transitions.push(PetriNetTransition::new("t_B".to_string(), "B".to_string(), false));
        net.arcs.push(PetriNetArc::new("source".to_string(), "t_A".to_string(), 1));
        net.arcs.push(PetriNetArc::new("t_A".to_string(), "p1".to_string(), 1));
        net.arcs.push(PetriNetArc::new("p1".to_string(), "t_B".to_string(), 1));
        net.arcs.push(PetriNetArc::new("t_B".to_string(), "sink".to_string(), 1));

        let log = EventLog::new(
            vec![make_trace("c1", &["A", "B"])],
            HashMap::new(),
        );

        let result = check_conformance_alignment(&log, &net, "concept:name").unwrap();
        assert_eq!(result.fitting_traces, 1, "Conforming trace should have zero alignment cost");
    }
    ...
    fn test_alignment_nonconforming_trace_has_cost() {
        ...
        let result = check_conformance_alignment(&log, &net, "concept:name").unwrap();
        assert!(result.fitness < 1.0, "Non-conforming trace should have fitness < 1.0");
