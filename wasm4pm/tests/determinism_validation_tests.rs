...
fn test_compute_reward_is_pure_function() {
    ...
    let first_result = compute_reward(inputs.0, inputs.1, inputs.2, inputs.3, inputs.4);

    for _ in 0..99 {
        let result = compute_reward(inputs.0, inputs.1, inputs.2, inputs.3, inputs.4);
        assert_eq!(
            result, first_result,
    ...
    let different = compute_reward(3u8, 3u8, 0usize, true, true);
    assert_ne!(
    ...
    let improvement = compute_reward(3, 2, 0, true, true);
    let stability = compute_reward(2, 2, 0, true, true);
    assert!(
...
fn test_telemetry_accumulation_deterministic() {
    ...
    let reward = compute_reward(3, 2, 0, true, true);
    for _ in 0..100 {
        assert_eq!(
            compute_reward(3, 2, 0, true, true),
            reward,
...
fn test_run_cycle_output_reproducible() {
    ...
    let reward_a = compute_reward(3, 2, 0, true, true);
    let reward_b = compute_reward(3, 2, 0, true, true);
    assert_eq!(
        reward_a, reward_b,
