...
export interface ModelIR {
  ...
  readonly nodes: ReadonlyArray<{ id: string; label: string; type: string }>;
  readonly edges: ReadonlyArray<{ from: string; to: string; weight?: number }>;
  readonly quality?: {
    fitness?: number;
    precision?: number;
...
export interface MiningBackend {
   ...
   */
  healthCheck(): Promise<{ healthy: boolean; latency_ms: number; detail?: string }>;
}
