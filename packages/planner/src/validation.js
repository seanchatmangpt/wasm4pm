 ...
 */
export function validatePlan(executionPlan) {
    const errors = [];
    ...
    for (let i = 0; i < executionPlan.steps.length; i++) {
        ...
        // Check step type
        if (!Object.values(PlanStepType).includes(step.type)) {
            errors.push({
                path: `${path}.type`,
        ...
        if (step.type === PlanStepType.BOOTSTRAP)
            hasBootstrap = true;
        if (step.type === PlanStepType.CLEANUP)
            hasCleanup = true;
        if (step.type === PlanStepType.LOAD_SOURCE)
            hasLoadSource = true;
        if (step.type === PlanStepType.VALIDATE_SOURCE)
            hasValidate = true;
        // Check description
 ...
 */
export function assertPlanValid(executionPlan) {
 ...
 */
export function validateSourceSinkCompatibility(sourceKind, sinkKind) {
    const errors = [];
    if (!sourceKind) {
        errors.push({
            path: 'source',
