...
fn same_request_produces_identical_response() {
    assert_eq!(InterviewAssistScenario::new().run(), InterviewAssistScenario::new().run());
}

#[test]
fn canonical_json_is_stable() {
    let first = serde_json::to_string(&InterviewAssistScenario::new().run()).expect("response serializes");
    let second = serde_json::to_string(&InterviewAssistScenario::new().run()).expect("response serializes");
...
fn changed_transcript_changes_receipt() {
    ...
    let second = InterviewAssistScenario::new()
        .transcript("event-transcript-001", "Given a sorted array, use two pointers to find a target sum.")
        .run();
    assert_ne!(
