...
import { WatchMode, watchWithReconnection, } from '../src/watch.js';
...
describe('WatchMode - Streaming', () => {
    ...
    it('should emit complete event with receipt', async () => {
        ...
        const completeEvents = events.filter((e) => e.type === 'complete');
        expect(completeEvents.length).toBe(1);
        const receipt = completeEvents[0].receipt;
...
describe('WatchMode - Checkpointing', () => {
    ...
    it('should emit checkpoint events', async () => {
        ...
        checkpoints.forEach((cp) => {
            expect(cp.progress_hash).toBeDefined();
            expect(cp.progress_hash).toHaveLength(16); // SHA256 truncated
        });
    });
        ...
        try {
            for await (const event of watchWithReconnection(plan, config, watchConfig)) {
                ...
                if (event.type === 'reconnect') {
                    reconnectAttempts.push(event.attempt);
                }
            }
        }
        catch (err) {
