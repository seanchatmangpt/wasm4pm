...
import { Instrumentation, REQUIRED_FIELD_NAMES, createRequiredFields, } from '@wasm4pm/observability';
...
function makeRequiredAttrs(runId = 'playground-run-001') {
    return {
        'run.id': runId,
...
describe('otel tracing: capture bootstrap spans', () => {
    ...
    it('captureRaw captures a state_change span', () => {
        const { otelEvent } = Instrumentation.createStateChangeEvent(traceId, 'uninitialized', 'bootstrapping', requiredAttrs);
    ...
    it('captures a full bootstrap sequence (2 state changes)', () => {
        const { otelEvent: e1 } = Instrumentation.createStateChangeEvent(traceId, 'uninitialized', 'bootstrapping', requiredAttrs);
        const { otelEvent: e2 } = Instrumentation.createStateChangeEvent(traceId, 'bootstrapping', 'ready', requiredAttrs);
    ...
    it('shows how to verify a new custom attribute', () => {
        const { otelEvent } = Instrumentation.createStateChangeEvent(traceId, 'bootstrapping', 'ready', requiredAttrs);
...
describe('otel tracing: required attributes', () => {
    ...
    it('assertRequiredAttributes catches a span missing run.id', () => {
        ...
        delete brokenAttrs['run.id'];
        const { otelEvent } = Instrumentation.createStateChangeEvent(traceId, 'uninitialized', 'bootstrapping', brokenAttrs);
        capture.captureRaw(otelEvent);
        const errors = capture.assertRequiredAttributes([...REQUIRED_FIELD_NAMES]);
...
describe('otel tracing: trace tree and non-blocking timing', () => {
    it('spans form a valid parent-child tree', () => {
        ...
        capture.captureSpan({
            ...
            startTime: nowNs, endTime: nowNs + 10000000,
        ...
        capture.captureSpan({
            ...
            startTime: nowNs + 1000000, endTime: nowNs + 5000000,
            attributes: { ...attrs }, events: [],
        });
    ...
    it('assertValidTraces detects orphan spans', () => {
        ...
        capture.captureSpan({
            ...
            startTime: nowNs, endTime: nowNs + 2000000,
            attributes: { ...attrs }, events: [],
        });
        const errors = capture.assertValidTraces();
        expect(errors.length).toBeGreaterThan(0);
    ...
    it('assertNonBlocking passes for fast spans (<1000ms)', () => {
        ...
        otelEvent.end_time = otelEvent.start_time + 2000000; // 2ms in ns
        capture.captureRaw(otelEvent);
    ...
    it('assertNonBlocking catches a span exceeding 1000ms', () => {
        ...
        capture.captureSpan({
            ...
            startTime: nowNs,
            endTime: nowNs + 1500000000, // 1500ms in ns
            attributes: { ...attrs }, events: [],
        });
        const errors = capture.assertNonBlocking(1000);
        expect(errors.length).toBeGreaterThan(0);
...
//# sourceMappingURL=05-otel-tracing.js.map