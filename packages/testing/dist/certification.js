...
registerGate('contracts:schemas', async () => {
    try {
        const kernel = (await import(/* @vite-ignore */ '@wasm4pm/kernel'));
...
registerGate('parity:explain-run', async () => {
    try {
        const testing = (await import(/* @vite-ignore */ '@wasm4pm/testing'));
...
registerGate('observability:otel-optional', async () => {
    try {
        const observability = (await import(/* @vite-ignore */ '@wasm4pm/observability'));
        const instr = observability.Instrumentation;
        const hasFuncs = typeof instr.createAlgorithmStartedEvent === 'function' &&
...
registerGate('security:redaction', async () => {
    try {
        const config = (await import(/* @vite-ignore */ '@wasm4pm/config'));
...
registerGate('watch:reconnect', async () => {
    try {
        const testing = (await import(/* @vite-ignore */ '@wasm4pm/testing/harness/cli'));
...
registerGate('config:resolution', async () => {
    try {
        ...
        const cfg = config.resolveConfig({ algorithm: 'dfg' });
        const algoProvenance = cfg.metadata.provenance['algorithm'];
