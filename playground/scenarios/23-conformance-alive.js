...
describe('Conformance Command Alive', () => {
    it('wasm4pm conformance command exists and is callable — Rank 2: domain contract', async () => {
        // JTBD: "The conformance command must exist in the CLI and be callable"
        const result = await wasm4pm(['conformance', '--help']);
        // Command should show help without crashing
        expect([0, 1, 2]).toContain(result.exitCode);
    });
    it('wasm4pm conformance accepts input file without crashing — Rank 2: domain contract', async () => {
        // JTBD: "The conformance command must handle input files gracefully"
        const result = await wasm4pm(['conformance', RUNNING_EXAMPLE, '--format', 'json']);
    ...
    it('wasm4pm conformance returns valid JSON output — Rank 2: domain contract', async () => {
    ...
    it('wasm4pm conformance on success returns fitness score — Rank 1: mathematical invariant', async () => {
...
//# sourceMappingURL=23-conformance-alive.js.map