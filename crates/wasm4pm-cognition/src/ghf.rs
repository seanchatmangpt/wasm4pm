...
pub fn check_receipt_schema(valid: bool) -> ValidationResult {
    if valid {
        ...
        ValidationResult::Refuse(RefusalState8::ReceiptSchemaInvalid, "invalid schema".into())
    }
}
...
pub fn check_hash_binding(valid: bool) -> ValidationResult {
    if valid {
        ...
        ValidationResult::Refuse(RefusalState8::HashBindingFailed, "hash mismatch".into())
    }
}
...
pub fn check_replay(valid: bool) -> ValidationResult {
    if valid {
        ...
        ValidationResult::Refuse(RefusalState8::ReplayFailed, "replay failed".into())
    }
}
...
pub fn check_temporal_conformance(valid: bool) -> ValidationResult {
    if valid {
        ...
        ValidationResult::Refuse(
            RefusalState8::TemporalConformanceFailed,
            "temporal check failed".into(),
        )
    }
}
...
mod tests {
    ...
    fn test_refuses_policy_conformance_failed() {
        assert_eq!(
            check_policy_conformance(false),
            ValidationResult::Refuse(
                RefusalState8::PolicyConformanceFailed,
                "policy failed".into()
            )
    ...
    fn test_refuses_temporal_conformance_failed() {
        assert_eq!(
            check_temporal_conformance(false),
            ValidationResult::Refuse(
                RefusalState8::TemporalConformanceFailed,
                "temporal check failed".into()
            )
    ...
    fn test_refuses_external_verification_failed() {
        assert_eq!(
            check_external_verification(false),
            ValidationResult::Refuse(
                RefusalState8::ExternalVerificationFailed,
                "external verify failed".into()
            )
