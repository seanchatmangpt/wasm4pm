 ...
 */
export function generateMarkdownReport(config, results, classifications, summary) {
    const lines = [];
    ...
    const avgPrecision = validPrecision.length > 0
    ...
    if (summary.tier0.length > 0) {
        ...
        for (const algo of summary.tier0) {
            ...
            if (result) {
                lines.push(`| ${algo} | ${result.quality.fitness.toFixed(3)} | ${result.latencyMs.toFixed(1)} | ✅ |`);
            }
        }
        lines.push('');
    }
    ...
    if (summary.tier1.length > 0) {
        ...
        for (const algo of summary.tier1) {
            ...
            if (result) {
                ...
                lines.push(`| ${algo} | ${result.quality.fitness.toFixed(3)} | ${result.latencyMs.toFixed(1)} | ${reason} |`);
            }
        }
        lines.push('');
    }
    ...
    for (const result of results) {
        ...
        const fitness = result.quality.fitness > 0 ? result.quality.fitness.toFixed(3) : 'N/A';
        const precision = result.quality.precision > 0 ? result.quality.precision.toFixed(3) : 'N/A';
        const latency = result.latencyMs.toFixed(1);
        const crashed = result.crashed ? '❌' : '✅';
        lines.push(`| ${result.algorithm} | ${result.outputType} | ${fitness} | ${precision} | ${latency} | ${crashed} | ${tier} |`);
    }
    lines.push('');
    // Footer
    lines.push('---');
    ...
    lines.push('');
    lines.push('If the code says it worked but the event log cannot prove a lawful process happened, then it did not work.');
    lines.push('');
 ...
 */
export function generateHtmlReport(config, results, classifications, summary) {
