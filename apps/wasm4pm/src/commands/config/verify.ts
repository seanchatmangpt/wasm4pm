...
export const configVerify = defineCommand({
  meta: {
    name: 'verify',
    description:
      'Verify config is self-consistent: schema valid, provenance complete, zero warnings',
  },
  args: {
  ...
  async run(ctx) {
    ...
    return withSpanRaw(
      ...
      async () => {
        ...
        try {
          ...
          gates.push({
            ...
            detail: config.metadata?.hash
              ? `hash: ${config.metadata.hash.slice(0, 16)}…`
              : 'no hash',
          });
        } catch (e) {
          gates.push({
        ...
        verified = allPass;
        issueCount = gates.filter((g) => !g.pass).length;

        const result = makeResult(
          'config verify',
          { gates, all_pass: allPass },
          performance.now() - t0,
      ...
      () => ({ 'config.verified': verified, 'config.issue_count': issueCount })
    );
  },
});
