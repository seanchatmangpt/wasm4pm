...
const BASE_CONFIG = { version: '1.0', source: { kind: 'file', format: 'xes' }, execution: { profile: 'fast' } };
// ── Plan hash stability ───────────────────────────────────────────────────────
...
describe('determinism: synthetic receipt hash stability', () => {
    function makeReceipt(profile) {
        if (!planFn)
            throw new Error('planFn not initialized');
...
describe('determinism: harness self-check', () => {
    it('detects a producer that leaks Math.random() into a stable field', async () => {
        const result = await checkDeterminism(() => Promise.resolve({
            status: 'success',
...
//# sourceMappingURL=03-determinism.js.map