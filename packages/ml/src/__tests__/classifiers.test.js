...
describe('regressRemainingTime', () => {
    ...
    it('throws for insufficient data', async () => {
        await expect(regressRemainingTime([{ case_id: 'c1', trace_length: 2, remaining_time: 100 }])).rejects.toThrow('Not enough traces');
    });
});
// ---------------------------------------------------------------------------
// Edge Cases
...
describe('classifyTraces edge cases', () => {
    ...
    it('logistic regression predictions are stable across dataset sizes', async () => {
        ...
        expect(extCorrect / extendedFeatures.length).toBeGreaterThanOrEqual(baseCorrect / baseFeatures.length * 0.8);
    });
});
//# sourceMappingURL=classifiers.test.js.map