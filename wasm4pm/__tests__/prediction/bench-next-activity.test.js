...
describe('predict_next_activity', () => {
    ...
    it('sample — 1 000 calls latency', async () => {
        ...
        for (let i = 0; i < 1000; i++)
            wasm.predict_next_activity(model, prefix);
...
describe('score_trace_likelihood', () => {
    ...
    it('sample — 1 000 calls latency', async () => {
        ...
        const t = performance.now();
        for (let i = 0; i < 1000; i++)
            wasm.score_trace_likelihood(model, trace);
        const perCall = Number(((performance.now() - t) / 1000).toFixed(4));
        rows.push({
...
describe('predict_next_k', () => {
    ...
    it('sample — 1 000 calls latency', async () => {
        ...
        const t = performance.now();
        for (let i = 0; i < 1000; i++)
            wasm.predict_next_k(model, prefix, 3);
        const perCall = Number(((performance.now() - t) / 1000).toFixed(4));
        rows.push({
...
//# sourceMappingURL=bench-next-activity.test.js.map