 ...
 */
function emitAlgorithmSpan(algorithmId, status, executionTimeMs, errorMessage) {
    try {
        const span = {
            ...
            kind: 'INTERNAL',
            start_time: (Date.now() - executionTimeMs) * 1000000,
        ...
        if (typeof globalThis._wasm4pm_span_sink === 'function') {
            globalThis._wasm4pm_span_sink(span);
        }
    }
    catch {
        /* never block on OTEL */
    ...
    try {
        switch (algorithmId) {
            ...
            case 'alpha_plus_plus': {
                const minSupport = params.min_support ?? 0.0;
