...
import { measure4DQuality, summarizeQuality } from './quality-pipeline.js';
 ...
 */
export async function runAdversarialAudit(wasm, config = DEFAULT_AUDIT_CONFIG) {
    ...
    for (const meta of ALGORITHM_MANIFEST) {
        ...
        try {
            ...
            // Measure 4D quality
            const { quality, error: qualityError } = await measure4DQuality(wasm, meta.id, outputType, model, modelHandle, logHandle, config.activityKey);
            results.push({
                algorithm: meta.id,
                outputType,
                model,
            ...
            if (config.verbose) {
                const fitnessStr = quality.fitness > 0
                    ? `fitness=${quality.fitness.toFixed(3)}`
                    : `no fitness (${outputType})`;
                console.log(`✅ ${meta.id}: ${latencyMs.toFixed(1)}ms, ${fitnessStr}`);
            }
        }
        catch (e) {
            crashed = true;
 ...
 */
export async function runAdversarialAuditBatch(wasm, datasetDir = './data', outputDir = './benchmarks/adversarial/results') {
