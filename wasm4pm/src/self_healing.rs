...
mod tests {
    ...
    /// Every test gets a fresh clock at zero.
    fn setup() {
        reset_clock();
    }
    ...
    fn test_circuit_breaker_closed_to_open() {
        ...
        // Record failures until threshold (default = 5).
        for _ in 0..5 {
    ...
    fn test_circuit_breaker_half_open_to_closed() {
        ...
        // Advance clock past open_timeout (60 000 ms).
        ...
        // Record successes to close circuit (default threshold = 2).
        breaker.record_success();
        breaker.record_success();
    ...
    fn test_circuit_breaker_half_open_failure_returns_to_open() {
        ...
        // Advance past open_timeout.
        advance_clock(60_100);
        assert!(breaker.allow_request());
        assert_eq!(breaker.state(), CircuitState::HalfOpen);

        // A single failure in half-open should return to open.
        breaker.record_failure();
        assert_eq!(breaker.state(), CircuitState::Open);
    }
    ...
    fn test_retry_policy_exponential_backoff() {
        ...
        // First retry: 100 ms.
        let backoff1 = state.next_attempt(&policy);
        assert_eq!(backoff1, Some(100));

        // Second retry: 200 ms (2x).
        let backoff2 = state.next_attempt(&policy);
        assert_eq!(backoff2, Some(200));

        // Third retry: 400 ms (4x).
        let backoff3 = state.next_attempt(&policy);
        assert_eq!(backoff3, Some(400));

        // Fourth retry: 800 ms (8x).
        let backoff4 = state.next_attempt(&policy);
        assert_eq!(backoff4, Some(800));
    }
    ...
    fn test_retry_policy_max_backoff_cap() {
        ...
        // Should never exceed max_backoff_ms.
        for _ in 0..10 {
            let backoff = state.next_attempt(&policy).unwrap();
            assert!(
                backoff <= 1_000,
                "backoff {} exceeded max 1000",
                backoff
            );
        }
    }
    ...
    fn test_retry_policy_jitter_within_bounds() {
        let policy = RetryPolicy {
            ...
            backoff_multiplier: 1.0, // keep base constant so jitter bounds are stable
            max_backoff_ms: 10_000,
            jitter: true,
        };
        ...
        // multiplier = 1.0 keeps current_backoff at 1000 every call
        for _ in 0..5 {
            let backoff = state.next_attempt(&policy).unwrap();
            // Jitter is +/-25%, so [750, 1250] for base 1000.
            assert!(
    ...
    fn test_health_check_healthy_threshold() {
        ...
        assert_eq!(check.status(), HealthStatus::Healthy); // default is healthy
    ...
    fn test_health_check_recovery() {
        ...
        // Recover — first success not enough.
        check.record_result(true);
        assert_eq!(check.status(), HealthStatus::Unhealthy);

        // Second success reaches healthy threshold.
        check.record_result(true);
        assert_eq!(check.status(), HealthStatus::Healthy);
    }
    ...
    fn test_manager_execute_with_circuit_breaker_failure() {
        ...
        let result: Result<i32, SelfHealingError> =
            manager.execute_with_circuit_breaker("test_dep", || {
                Err::<i32, SelfHealingError>(
                    SelfHealingError::OperationFailed("boom".into()),
                )
            });
    ...
    fn test_manager_execute_with_circuit_breaker_open_rejects() {
        ...
        for _ in 0..5 {
            let _: Result<i32, SelfHealingError> =
                manager.execute_with_circuit_breaker("test_dep", || {
                    Err::<i32, SelfHealingError>(
                        SelfHealingError::OperationFailed("fail".into()),
                    )
                });
        }
    ...
    fn test_manager_execute_with_retry_succeeds_eventually() {
        ...
        let result: Result<i32, SelfHealingError> = manager.execute_with_retry(&policy, || {
            ...
            if n < 2 {
                Err::<i32, SelfHealingError>(
                    SelfHealingError::OperationFailed("transient".into()),
                )
            } else {
    ...
    fn test_manager_execute_with_retry_exhausted() {
        ...
        let result: Result<i32, SelfHealingError> = manager.execute_with_retry(&policy, || {
            Err::<i32, SelfHealingError>(SelfHealingError::OperationFailed(
                "permanent".into(),
            ))
        });
