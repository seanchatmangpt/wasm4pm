...
#[test]
fn initial_observation_creates_a_nonempty_hypothesis_set() {
    assert!(!first_turn().state.hypotheses.is_empty());
}

#[test]
fn hypothesis_ids_remain_stable_across_equivalent_observations() {
    let first = first_turn();
    let equivalent = turn(
        None,
        Some(observation("event-transcript-equivalent", QUESTION)),
        None,
    )
    .expect("equivalent observation admitted");
    assert_eq!(candidate_ids(&first), candidate_ids(&equivalent));
}

#[test]
fn rejecting_the_leading_hypothesis_removes_or_demotes_it() {
    let first = first_turn();
    let rejected_id = first.projection.hypotheses[0].id.clone();
    let rejected = confirm_after(&first.state, false).expect("rejection admitted");
    let hypothesis = rejected
        .projection
        .hypotheses
        .iter()
        .find(|item| item.id == rejected_id)
        .expect("rejected hypothesis remains auditable");
    assert!(hypothesis.eliminated);
    assert_eq!(hypothesis.score, 0.0);
    assert_ne!(rejected.projection.hypotheses[0].id, rejected_id);
}

#[test]
fn rejecting_one_hypothesis_does_not_remove_unrelated_candidates() {
    let first = first_turn();
    let before: BTreeSet<_> = candidate_ids(&first).into_iter().collect();
    let rejected = confirm_after(&first.state, false).expect("rejection admitted");
    let after: BTreeSet<_> = candidate_ids(&rejected).into_iter().collect();
    assert_eq!(before, after);
    assert!(after.contains("two_pointer"));
    assert!(after.contains("brute_force"));
}

#[test]
fn confirming_a_hypothesis_marks_exactly_one_track_confirmed() {
    let first = first_turn();
    let output = confirm_after(&first.state, true).expect("confirmation admitted");
    assert!(output.state.committed_track.is_some());
    let confirmed_count = domain_pack()
        .tracks
        .iter()
        .filter(|track| output.state.committed_track.as_deref() == Some(track.id.as_str()))
        .count();
    assert_eq!(confirmed_count, 1);
    assert!(output.state.rejected_tracks.is_empty());
}

#[test]
fn correction_evidence_changes_the_hypothesis_revision() {
    let first = first_turn();
    let corrected = observe_after(&first.state, "event-correction-001", TWO_POINTER_CORRECTION);
    assert_eq!(corrected.state.turn, first.state.turn + 1);
    assert_ne!(corrected.state.hypotheses, first.state.hypotheses);
    assert_ne!(corrected.state.state_hash, first.state.state_hash);
}

#[test]
fn stale_confirmation_cannot_mutate_a_newer_revision() {
    let first = first_turn();
    let newer = observe_after(&first.state, "event-correction-001", TWO_POINTER_CORRECTION);
    let stale_confirmation = confirm_after(&first.state, true);
    assert!(
        stale_confirmation.is_err(),
        "a confirmation against revision {} must not mutate revision {}",
        first.state.turn,
        newer.state.turn
    );
}

fn reject_all_tracks() -> SessionTurnOutput {
    let first = first_turn();
    let rejected_hash = confirm_after(&first.state, false).expect("reject hash lookup");
    let two_pointer = observe_after(
        &rejected_hash.state,
        "event-correction-two-pointer",
        TWO_POINTER_CORRECTION,
    );
    let rejected_two = confirm_after(&two_pointer.state, false).expect("reject two pointer");
    let brute_force = observe_after(
        &rejected_two.state,
        "event-correction-brute-force",
        BRUTE_FORCE_CORRECTION,
    );
    confirm_after(&brute_force.state, false).expect("reject brute force")
}

#[test]
fn repeated_rejection_reaches_abstention_instead_of_looping_forever() {
    let output = reject_all_tracks();
    let value = output_json(&output);
    assert_eq!(value.get("abstention").and_then(Value::as_bool), Some(true));
    assert!(output.state.pending_confirmation.is_none());
}

#[test]
fn abstention_does_not_fabricate_a_confirmed_track() {
    let output = reject_all_tracks();
    assert!(output.state.committed_track.is_none());
    assert!(output.state.hypotheses.iter().all(|item| item.eliminated));
}

#[test]
fn confidence_ordering_is_deterministic() {
    let first = first_turn();
    let second = first_turn();
    assert_eq!(first.state.hypotheses, second.state.hypotheses);
}

#[test]
fn equivalent_evidence_does_not_duplicate_a_hypothesis() {
    let first = first_turn();
    let repeated = observe_after(&first.state, "event-transcript-002", QUESTION);
    let ids: Vec<_> = repeated
        .state
        .hypotheses
        .iter()
    ...
    assert_eq!(ids.len(), domain_pack().tracks.len());
}
