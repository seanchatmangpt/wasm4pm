...
describe('forecastThroughput', () => {
    it('forecasts with upward trend', async () => {
        ...
        const timestamps = [];
        const base = 1700000000000; // ms
        let count = 1;
        for (let w = 0; w < 10; w++) {
            for (let e = 0; e < count; e++) {
                timestamps.push(base + w * 3600000 + e * 60000);
            }
            count++;
        }
        ...
        expect(result.windowSizeMs).toBe(3600000);
    });
...
describe('forecastSeries', () => {
    ...
    it('detects seasonality at correct period for sine wave', async () => {
        ...
        const series = Array.from({ length: 24 }, (_, i) => Math.round(50 + 30 * Math.sin(2 * Math.PI * i / 8)));
        const result = await forecastSeries(series, { forecastPeriods: 4 });
...
describe('forecastSeries edge cases', () => {
    ...
    it('detects strong vs weak seasonality', async () => {
        ...
        const strongSeasonal = Array.from({ length: 32 }, (_, i) => Math.round(50 + 30 * Math.sin(2 * Math.PI * i / 8)));
        ...
        const weakSeasonal = Array.from({ length: 32 }, (_, i) => Math.round(1000 + 5 * Math.sin(2 * Math.PI * i / 8) + (Math.random() - 0.5) * 200));
...
//# sourceMappingURL=forecasting.test.js.map