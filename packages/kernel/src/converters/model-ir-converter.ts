...
export interface PetriNet {
  ...
  initial_marking: Record<string, number>; // place_id -> token count
  final_markings: Array<Record<string, number>>; // Alternative final states
  quality?: QualityMetrics;
}
...
export interface PowlModel {
  ...
  index_map: Record<string, number>; // string ID -> arena index
  quality?: QualityMetrics;
}
 ...
 */
export function inferStartActivities(
 ...
 */
export function inferEndActivities(
  nodes: ReadonlyArray<ModelNode>,
  edges: ReadonlyArray<ModelEdge>
): string[] {
 ...
 * ```
 */
export function modelIrToDfg(ir: ModelIR): DirectlyFollowsGraph {
 ...
 */
function inferInitialMarking(
  places: string[],
  transitions: string[],
  arcs: ReadonlyArray<ModelEdge>
): Record<string, number> {
  const incomingToPlace = new Set<string>();
...
export function modelIrToPetriNet(ir: ModelIR): PetriNet {
  ...
  const outgoingFromPlace = new Set<string>();
  ...
  const finalMarkings: Array<Record<string, number>> = [];
 ...
 */
function allocateArenaIndices(
  nodes: ReadonlyArray<ModelNode>
): [Array<{ id: number; label: string; node_type: string }>, Record<string, number>] {
  const indexMap: Record<string, number> = {};
  const arena: Array<{ id: number; label: string; node_type: string }> = [];
 ...
 * ```
 */
export function modelIrToPowlModel(ir: ModelIR): PowlModel {
 ...
 * @returns ModelIR equivalent
 */
export function powlModelToModelIr(powl: PowlModel): ModelIR {
  // Reverse the index_map to get ID -> string
  const reverseIndexMap: Record<number, string> = {};
