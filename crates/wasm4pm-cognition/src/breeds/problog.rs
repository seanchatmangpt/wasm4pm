...
fn pfacts(input: &BreedInput) -> Result<Vec<(String, f64)>, String> {
    ...
    for f in &input.facts {
        if let Some(atom) = f.key.strip_prefix("pfact:") {
            let p: f64 = f.value.parse().map_err(|_| {
                format!("pfact '{}' has non-numeric probability '{}'", atom, f.value)
            })?;
...
mod tests {
    ...
    fn falsification_gate_exact_probability() {
        ...
        input.rules.push(Rule {
            id: "r1".to_string(),
            premise: vec!["c1".to_string()],
            conclusion: "a".to_string(),
            certainty: 1.0,
        });
        input.rules.push(Rule {
            id: "r2".to_string(),
            premise: vec!["c2".to_string()],
            conclusion: "a".to_string(),
            certainty: 1.0,
        });
    ...
    fn de_raedt_2007_wet_probability_is_0552() {
        ...
        inp.rules.push(Rule {
            id: "r-rain".into(),
            premise: vec!["rain".into()],
            conclusion: "wet".into(),
            certainty: 1.0,
        });
        inp.rules.push(Rule {
            id: "r-sprinkler".into(),
            premise: vec!["sprinkler".into()],
            conclusion: "wet".into(),
            certainty: 1.0,
        });
        inp.rules.push(Rule {
            id: "r-hose".into(),
            premise: vec!["hose".into()],
            conclusion: "wet".into(),
            certainty: 1.0,
        });
