...
import { ALGORITHM_ID_TO_STEP_TYPE, getProfileAlgorithms, ALGORITHM_DISPLAY_NAMES, } from '@pictl/contracts';
import { topologicalSort, validateDAG } from './dag.js';
import { PlanStepType, createBootstrapStep, createInitWasmStep, createLoadSourceStep, createValidateSourceStep, createAlgorithmStep, createAnalysisStep, createGenerateReportsStep, createSinkStep, createCleanupStep, } from './steps.js';
/**
 ...
 */
function getDefaultPipeline(profile) {
 ...
 */
function algorithmNameFromStepType(stepType) {
    ...
    if (entry)
        return ALGORITHM_DISPLAY_NAMES[entry[0]] ?? stepType;
...
function createBudgetEnvelopeFromConfig(config, sourceKind) {
    ...
    const memoryBudget = (config.execution.maxMemoryMB || 0) * 1024 * 1024;
...
export function plan(config) {
    ...
    if (!config || typeof config !== 'object') {
        throw new PlannerError(createError('CONFIG_INVALID', 'Configuration must be a non-null object', {
            received: typeof config,
        }));
    }
    if (config.version !== '1.0') {
        throw new PlannerError(createError('CONFIG_INVALID', `Invalid config version: expected "1.0", got "${config.version}"`, { version: config.version }));
    }
    if (!config.source || !config.source.format) {
        throw new PlannerError(createError('CONFIG_INVALID', 'Configuration must include source.format', {
            source: config.source,
        }));
    }
    if (!config.execution || !config.execution.profile) {
        throw new PlannerError(createError('CONFIG_INVALID', 'Configuration must include execution.profile', {
    ...
    if (algorithmOverride) {
        ...
        if (!overrideStepType) {
            throw new PlannerError(createError('CONFIG_INVALID', `Unknown algorithm: "${algorithmOverride}". See ALGORITHM_ID_TO_STEP_TYPE for valid IDs.`, { algorithmName: algorithmOverride }));
        }
        ...
        pipelineSteps = [overrideStepType, ...analysisOnly];
    }
    for (const algoType of pipelineSteps) {
        ...
        if (algoType.toString().includes('discover') || algoType.toString().includes('import') || algoType.toString().includes('convert') || algoType.toString().includes('export') || algoType.toString().includes('simulate')) {
            ...
            planStep = createAlgorithmStep(algoName, algoType, algoParams, true, ['validate_source'], true);
        }
        else {
            // It's an analysis step
            planStep = createAnalysisStep(algoName, algoType, config.execution.parameters || {}, ['validate_source'], true);
        }
        steps.push(planStep);
    ...
    if (config.ml?.enabled && config.ml.tasks && config.ml.tasks.length > 0) {
        ...
        for (const mlTask of config.ml.tasks) {
            ...
            const mlStep = createAnalysisStep(algorithmNameFromStepType(mlType), mlType, mlParams, ['validate_source'], true);
            steps.push(mlStep);
    ...
    const analysisStepIds = steps
        .filter((s) => s.type !== PlanStepType.BOOTSTRAP &&
    ...
    for (const step of steps) {
        ...
        if (!Array.isArray(step.dependsOn)) {
            throw new PlannerError(createError('CONFIG_INVALID', `Step "${step.id}" is missing required field: dependsOn must be an array`, { stepId: step.id, dependsOn: step.dependsOn }));
        }
    ...
    if (dagErrors.length > 0) {
        throw new PlannerError(createError('CONFIG_INVALID', `Invalid execution plan DAG: ${dagErrors.join('; ')}`, {
            dagErrors,
        }));
    }
    ...
    try {
        ...
        throw new PlannerError(createError('CONFIG_INVALID', `Execution plan contains cycles: ${err instanceof Error ? err.message : String(err)}`));
    }
 ...
 */
function computePlanHash(_planId, steps, graph, config) {
...
export function toContractsPlan(executionPlan) {
    ...
    const totalDuration = executionPlan.steps.reduce((sum, s) => sum + (s.estimatedDurationMs || 0), 0);
    return {
        schema_version: '1.0',
