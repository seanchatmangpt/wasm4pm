...
export class AgentOrchestrator {
    ...
    async analyze(context, monitor) {
        ...
        for (const agentName of agentsToRun) {
            const result = await this.executeAgent(agentName, {
                ...context,
                traces: monitor.telemetry.data.traces,
                ocel_events: monitor.process.data.events,
                receipts: monitor.execution.data.receipts,
            });
            if (!result.passed) {
     ...
     */
    async _runAgentLogic(agentName, context) {
        const agentState = this.registry.getAgent(agentName);
        if (!agentState) {
    ...
    _validateMockInterceptor(context, config) {
        const violations = [];
        const traces = context.traces || [];
    ...
    _validateReceiptChain(context, config) {
        const violations = [];
        const receipts = context.receipts || [];
    ...
    _validateGateIndependence(context, config) {
        const violations = [];
    ...
    _validateEvidenceFabrication(context, config) {
        const violations = [];
        const traces = context.traces || [];
        for (const trace of traces) {
    ...
    _validateProcessMiningSkeptic(context, config) {
        const violations = [];
        const events = context.ocel_events || [];
        if (events.length === 0) {
            return {
    ...
    _validateTheaterDetector(context, config) {
        const violations = [];
        const traces = context.traces || [];
        for (const trace of traces) {
    ...
    _validateAuthorityEscalation(context, config) {
        const violations = [];
        const events = context.ocel_events || [];
    ...
    async _applyCorrection(action, context) {
