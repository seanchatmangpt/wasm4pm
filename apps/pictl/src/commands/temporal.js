...
export const temporal = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            const threshold = parseFloat(ctx.args.threshold || '0.05');
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                printHumanTemporal(formatter, result);
            }
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.error(`Temporal analysis failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            process.exit(EXIT_CODES.execution_error);
        }
    },
});
function printHumanTemporal(formatter, result) {
    const violations = result.violations;
    const perfDfg = result.performanceDfg;
    const activityDurs = result.activityDurations;
    formatter.log('');
    formatter.success(`Temporal Analysis — ${result.input}`);
    formatter.log(`  Activity key: ${result.activityKey}`);
    formatter.log(`  Timestamp key: ${result.timestampKey}`);
    formatter.log(`  Threshold: ${result.threshold.toFixed(3)}`);
    formatter.log('');
    ...
    if (activityDurs) {
        ...
        const durations = activityDurs.durations;
        if (durations) {
