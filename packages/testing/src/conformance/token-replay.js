 ...
 */
export function tokenReplayConformance(net, eventLog, config = {}) {
    const { initialMarking = [], finalMarking = [], maxTokens = 1000, skipMissingActivities = false, } = config;
    const traceResults = [];
    let totalMissing = 0;
 ...
 */
function replayTrace(net, trace, config) {
    const deviations = [];
    let missingTokens = 0;
 ...
 */
export function createTestPetriNet() {
    return {
        places: [
            { id: 'p1', label: 'start' },
            { id: 'p2', label: 'after_A' },
 ...
 */
export function getExpectedTestResult() {
    return {
 ...
 */
export function expectCloseTo(value, delta = 0.01) {
    return value;
}
/**
 * Assert that a token replay result matches expected values.
 */
export function assertTokenReplayResult(actual, expected) {
 ...
 */
export function computeAlignment(net, trace, config = {}) {
    const { costModel = { moveOnLog: 1, moveOnModel: 1, synchronousMove: 0 }, maxStates = 10000, timeout = 30000, } = config;
 ...
 */
function extractModelSequence(net) {
    // Find initial places (no incoming arcs)
    const initialPlaces = net.places.filter(p => !net.arcs.some(a => a.target === p.id && net.transitions.some(t => t.id === a.source)));
    // Find final places (no outgoing arcs)
    const finalPlaces = net.places.filter(p => !net.arcs.some(a => a.source === p.id && net.transitions.some(t => t.id === a.target)));
    // Simple topological sort following transitions
    const sequence = [];
    const visited = new Set();
    function followFromPlace(placeId) {
 ...
 */
function canFireTransition(net, activity) {
    const transition = net.transitions.find(t => t.label === activity);
    if (!transition)
        return false;
    const inputArcs = net.arcs.filter(a => a.target === transition.id);
 ...
 */
export function formatTokenReplayResult(result) {
    const lines = [];
...
//# sourceMappingURL=token-replay.js.map