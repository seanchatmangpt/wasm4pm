...
export class PipelineResolver {
    ...
    resolve(config) {
        ...
        for (const configStep of configSteps) {
            ...
            if (!wasmFunction) {
                throw new PictlError(`No WASM binding for step type: ${configStep.type}`, ErrorCode.CONFIG_INVALID, {
    ...
    validateDependencies(steps) {
        ...
        for (const step of steps) {
            for (const dep of step.dependencies) {
                if (!stepIds.has(dep)) {
                    throw new PictlError(`Step "${step.stepId}" depends on non-existent step "${dep}"`, ErrorCode.CONFIG_INVALID, {
...
export function topologicalSort(steps) {
    ...
    function visit(stepId) {
        ...
        if (recursionStack.has(stepId)) {
            throw new PictlError(`Circular dependency detected in pipeline: ${stepId}`, ErrorCode.EXECUTION_FAILED, {
                nextAction: ErrorRecovery.RECONFIGURE,
                context: { step: stepId },
            });
        }
