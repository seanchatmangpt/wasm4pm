<!-- wasm4pm-doc-status: active; reviewed: 2026-08-02; original: .claude/worktrees/wf_99470ccd-c7b-1/docs/archive/2026-08-02/docs/implementation-status.md; source-sha256: fd828b90ad0a57e08da51dcd4aeee225b35dfb51a1e3ea0a9dea184a289fe84f; reason: tooling or agent control surface -->

# Implementation Status — PRD Gates and Workstreams

**Date:** 2026-06-21 (v26.6.12)
...
| G3 | Receipt Gate | ADMITTED | `input_hash`, `output_hash`, `ocel_hash`, `model_hash`, `wasm_hash`, `signature`, `public_key_id`, `signature_algorithm` all emitted by `cognition_run()` (v26.6.12) | Production deployment requires replacing default actor with institutionally managed keypair for legal non-repudiation |
| G4 | OCEL Gate | ADMITTED | L0 + L1 spans in all 52/52 breeds; OCPN models; `validate_ocel_alignment()` native DFA replay at fitness=1.0 for all 52/52 breeds (v26.6.12) | None |
...
|----|------|--------|-----------------|------------|
| A | Registry | PARTIAL_ALIVE | `registry.rs` dispatcher; 60 algorithms in Rust kernel; 52 PARTIAL_ALIVE breeds in cognition layer; `registry.json` and `check_registry.sh` created | No CI enforcement at build time; no schema validation at boundary |
...
| C | Adversary | PARTIAL_ALIVE | `adversarial-catalogue.test.ts` exists and runs; `oracle_hidden.rs` provides T_hidden corpus (19 hidden tests across all 52/52 breeds, v26.6.12) | Adversary classes not formally defined; catalogue is a list of cases, not a typed threat model; no boundary between adversary class and mitigation |
| D | OCEL L1 | ADMITTED | L0 + L1 spans in all 52/52 breeds; OCPN models; native DFA replay fitness=1.0 for all 52/52 breeds (v26.6.12) | None |
| E | Receipts | ADMITTED | All receipt fields present: `input_hash`, `output_hash`, `ocel_hash`, `model_hash`, `wasm_hash`, `signature`, `public_key_id` (v26.6.12) | Default actor is compile-time seed; production non-repudiation requires institutionally managed keypair |
...
| ID | Action | Unblocks |
|----|--------|---------|
| M1 | Generate `registry.json` from `registry.rs` at build time and add CI check that algorithm count equals 60 (breed count is tracked separately: 52 PARTIAL_ALIVE in cognition layer) | G1 → ADMITTED, WS-A → ADMITTED |
...
| M4 | ~~Implement L1 breed inference trace spans; derive OCEL log from spans; add replay test~~ DONE (v26.6.12) | G4 → ADMITTED ✓, WS-D → ADMITTED ✓ |
