 ...
 */
export async function computeFitness(wasm, logHandle, modelHandle, activityKey) {
    try {
 ...
 */
export async function computePrecision(wasm, logHandle, modelHandle, activityKey) {
    try {
 ...
 */
export async function computeGeneralization(wasm, logHandle, modelHandle) {
    try {
 ...
 */
export function computeSimplicity(model) {
    try {
        if (!model)
            return 0;
        let elementCount = 0;
 ...
 */
export async function measure4DQuality(wasm, algorithmName, outputType, model, modelHandle, logHandle, activityKey) {
    try {
...
export function summarizeQuality(results) {
    ...
    return {
        ...
        avgGeneralization: generalizationValues.length > 0
...
//# sourceMappingURL=quality-pipeline.js.map