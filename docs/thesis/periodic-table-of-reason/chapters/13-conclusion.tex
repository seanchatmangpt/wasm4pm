...
defining-property oracle is subsumed by any combination of the other twelve. The thirteen
breeds occupy thirteen pairwise-distinguishable points in the epistemological space
$\mathcal{E}$, as established by the oracle-hierarchy theorem
(Theorem~\ref{thm:epistemic-independence}). This pairwise distinguishability --- not
linear independence, which is ill-defined for discrete oracle predicates --- is the sense
in which the breeds form a complete and non-redundant basis for the space. The coverage is
not arbitrary. Each dimension of $\Ei$ --- uncertainty handling, deductive strength, case
memory depth, generative power, temporal sensitivity, learning rate, communication
topology, and abductive capacity --- is a fundamental axis along which reasoning systems
can be characterised, and the thirteen validated breeds together provide non-zero coverage
of every dimension.
