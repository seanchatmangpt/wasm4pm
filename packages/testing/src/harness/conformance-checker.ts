...
export interface ConformanceResult {
  conformant: boolean;
  fitness: number; // 0-1, proportion of trace fitting the model
  precision: number; // 0-1, how much model behavior matches log behavior
  violations: ConformanceViolation[];
  totalEvents: number;
  conformingEvents: number;
  violatingEvents: number;
  pathsDivergent: number; // Count of execution paths that deviate
}
...
export class ConformanceChecker {
   ...
   */
  private discoverModel(ocel: OcelEventLog): DiscoveredModel {
    const activities = new Set<string>();
    const directlyFollows = new Map<string, Set<string>>();
   ...
   */
  async checkConformance(ocel: OcelEventLog, model?: DiscoveredModel): Promise<ConformanceResult> {
    ...
    const possibleEdges = Array.from(discoveredModel.directlyFollows.values()).reduce(
      (sum, followers) => sum + followers.size,
      0
    );
