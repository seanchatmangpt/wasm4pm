...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Fitness universality:} the conformance fitness
        $\phi(\mathcal{L}_i, M_i) = 1.0$ for all 13 breeds under pm4py token-based
        replay against the declared process model $M_i$.
\end{enumerate}
\end{theorem}
...
span linearly independent directions in the epistemological space $\mathcal{V}$. The
categorical argument excludes not only exact substitution but also approximation up to
...
non-repudiation, reproducibility, and regulatory admissibility.
\end{theorem}
...
\begin{itemize}
  ...
  \item \textbf{Non-repudiation} — the \texttt{input\_hash} / \texttt{output\_hash}
        pair, emitted at the WASM boundary, cryptographically binds each inference to its
        inputs; no party can deny having performed a certified inference.
