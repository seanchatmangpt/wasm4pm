    ...
    match run_breed(&breed, &input) {
        Ok(output) => {
            ...
            for f in output
                .facts
                .iter()
                .filter(|f| f.key.starts_with("sam:role:"))
            {
