/**
 * End-to-End Test: wasm4pm autoprocess command
 *
 * Task: Verify CLI integration for AutoProcess (Perception → Decision → Protection → Optimization)
 *
 * Scenarios:
 * 1. Basic invocation: wasm4pm autoprocess <log.xes> --format json
...
describe('wpm autoprocess e2e', () => {
    ...
    describe('Scenario 2: Persistence across runs', () => {
        it('should load and increment state from previous cycle', async () => {
            ...
            // Read saved state
            const savedState = JSON.parse(await fs.readFile(stateFilePath, 'utf-8'));
            ...
            const savedState2 = JSON.parse(await fs.readFile(stateFilePath, 'utf-8'));
            if (savedState.spc_history?.snapshots &&
                savedState2.spc_history?.snapshots) {
                expect(savedState2.spc_history.snapshots.length).toBeGreaterThanOrEqual(savedState.spc_history.snapshots.length);
            }
        }, { timeout: 60000 });
    ...
    describe('Integration: Output structure validation', () => {
        ...
        it('should parse valid XES with multiple traces', async () => {
            const complexLog = path.join(__dirname, '../../../lab/fixtures/sample-logs/complex.xes');
...
//# sourceMappingURL=30-autoprocess-e2e.test.js.map