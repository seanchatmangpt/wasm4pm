 ...
 */
function extractActivities(model) {
 ...
 */
function isValidPetriNet(output) {
    if (typeof output !== 'object' || output === null) {
        return false;
    }
    const model = output;
    return (model.type === 'petrinet' &&
...
describe('Cross-Algorithm Parity: Conformance Metrics', () => {
    it('should produce sound fitness values (0 ≤ fitness ≤ 1.0)', () => {
        ...
        try {
            ...
            const conformanceRaw = wasmModule.conformance_token_replay(logHandle, 'concept:name', JSON.stringify(model));
            ...
            const fitness = conformance.fitness;
            expect(fitness).toBeGreaterThanOrEqual(0.0);
            expect(fitness).toBeLessThanOrEqual(1.0);
            // Diagnostic counts must be non-negative
            expect(conformance.traced >= 0).toBe(true);
            expect(conformance.remaining >= 0).toBe(true);
            expect(conformance.missing >= 0).toBe(true);
        }
        finally {
            try {
                wasmModule.delete_object(logHandle);
    ...
    it('should produce sound precision values (0 ≤ precision ≤ 1.0)', () => {
        ...
        try {
            ...
            try {
                const precisionRaw = wasmModule.conformance_etconformance_precision(logHandle, 'concept:name', JSON.stringify(model));
    ...
    it('should be consistent across multiple conformance checks', () => {
        ...
        try {
            ...
            const result1Raw = wasmModule.conformance_token_replay(logHandle, 'concept:name', JSON.stringify(model));
            const result1 = typeof result1Raw === 'string' ? JSON.parse(result1Raw) : result1Raw;
            const result2Raw = wasmModule.conformance_token_replay(logHandle, 'concept:name', JSON.stringify(model));
    ...
    <event>
      ...
      <date key="time:timestamp" value="2024-01-01T09:00:00Z"/>
        ...
        try {
            ...
            const dfg = typeof dfgRaw === 'string' ? JSON.parse(dfgRaw) : dfgRaw;
            expect(isValidDFG(dfg)).toBe(true);
            expect(dfg.nodes.length).toBe(1);
            expect(dfg.edges.length).toBe(0);
            // Alpha++ should work
            const alphaRaw = wasmModule.discover_alpha_plus_plus(logHandle, 'concept:name');
            const alpha = typeof alphaRaw === 'string' ? JSON.parse(alphaRaw) : alphaRaw;
            expect(isValidPetriNet(alpha)).toBe(true);
