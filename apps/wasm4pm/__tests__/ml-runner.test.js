...
describe('executeMlTask — parameter validation', () => {
    ...
    it('classify rejects negative k', async () => {
        await expect(executeMlTask(mockWasm, 'classify', logHandle, activityKey, { k: -1 })).rejects.toThrow('positive number');
    });
    ...
    it('classify rejects zero k', async () => {
        await expect(executeMlTask(mockWasm, 'classify', logHandle, activityKey, { k: 0 })).rejects.toThrow('positive number');
    });
    // ── cluster ─────────────────────────────────────────────────────────────
    it('cluster rejects negative k', async () => {
        await expect(executeMlTask(mockWasm, 'cluster', logHandle, activityKey, { k: -1 })).rejects.toThrow('positive number');
    });
...
//# sourceMappingURL=ml-runner.test.js.map