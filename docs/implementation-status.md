...
| G3 | Receipt Gate | ADMITTED | `input_hash`, `output_hash`, `ocel_hash`, `model_hash`, `wasm_hash`, `signature`, `public_key_id`, `signature_algorithm` all emitted by `cognition_run()` (v26.6.10) | Production deployment requires replacing default actor with institutionally managed keypair for legal non-repudiation |
| G4 | OCEL Gate | ADMITTED | L0 + L1 spans in all 39/52 breeds; OCPN models; `validate_ocel_alignment()` native DFA replay at fitness=1.0 for all 39/52 breeds (v26.6.10) | None |
...
| C | Adversary | PARTIAL_ALIVE | `adversarial-catalogue.test.ts` exists and runs; `oracle_hidden.rs` provides T_hidden corpus (19 hidden tests across all 39/52 breeds, v26.6.10) | Adversary classes not formally defined; catalogue is a list of cases, not a typed threat model; no boundary between adversary class and mitigation |
| D | OCEL L1 | ADMITTED | L0 + L1 spans in all 39/52 breeds; OCPN models; native DFA replay fitness=1.0 for all 39/52 breeds (v26.6.10) | None |
| E | Receipts | ADMITTED | All receipt fields present: `input_hash`, `output_hash`, `ocel_hash`, `model_hash`, `wasm_hash`, `signature`, `public_key_id` (v26.6.10) | Default actor is compile-time seed; production non-repudiation requires institutionally managed keypair |
...
| M4 | ~~Implement L1 breed inference trace spans; derive OCEL log from spans; add replay test~~ DONE (v26.6.10) | G4 → ADMITTED ✓, WS-D → ADMITTED ✓ |
