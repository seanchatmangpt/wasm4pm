# pictl Documentation

Welcome to the pictl documentation. This folder contains comprehensive guides for understanding, using, and deploying pictl.

The docs follow the **Diataxis framework** — see [INDEX.md](./INDEX.md) for the full organized index, or [DIATAXIS.md](./DIATAXIS.md) for the framework overview.

## Quick Navigation

### Getting Started
- **[INDEX.md](./INDEX.md)** - Full documentation index with role-based learning paths
- **[QUICKSTART.md](./QUICKSTART.md)** - 5-minute setup and first algorithm
- **[TUTORIAL.md](./TUTORIAL.md)** - Step-by-step examples and workflows
- **[DEPLOYMENT.md](./DEPLOYMENT.md)** - Build, test, and deploy guide

### Diataxis Sections
- **[tutorials/](./tutorials/)** - Hands-on learning guides (8 tutorials, beginner to advanced)
- **[how-to/](./how-to/)** - Task-focused recipes (26 guides)
- **[explanation/](./explanation/)** - Conceptual deep dives (17 topics)
- **[reference/](./reference/)** - Technical specifications (18 references)

### Reference
- **[API.md](./API.md)** - Complete API reference for all functions
- **[reference/algorithms.md](./reference/algorithms.md)** - 18 tools + 6 ML analysis tasks
- **[reference/cli-commands.md](./reference/cli-commands.md)** - pmctl CLI reference

### Advanced Topics
- **[FAQ.md](./FAQ.md)** - Troubleshooting and common questions
- **[TOOLS.md](./TOOLS.md)** - Tool overview and selection guide

### Project Information
- **[CHANGELOG.md](./CHANGELOG.md)** - Version history and release notes
- **[PROJECT_STATUS.md](./PROJECT_STATUS.md)** - Current project status and roadmap
- **[PREDICTION_ADDITIONS_GUIDE.md](./PREDICTION_ADDITIONS_GUIDE.md)** - Predictive process mining

### Data & Benchmarking
- **[REAL-DATASETS.md](./REAL-DATASETS.md)** - Available datasets for testing
- **[DATASETS-INDEX.md](./DATASETS-INDEX.md)** - Dataset catalog and descriptions
- **[BENCHMARK-TIERS-USAGE.md](./BENCHMARK-TIERS-USAGE.md)** - Running performance benchmarks
- **[BENCHMARK-RESULTS-INTERPRETATION.md](./BENCHMARK-RESULTS-INTERPRETATION.md)** - Understanding benchmark output
- **[BENCHMARK-REAL-DATA-PLAN.md](./BENCHMARK-REAL-DATA-PLAN.md)** - Real data benchmarking strategy
- **[BROWSER-BENCHMARKS.md](./BROWSER-BENCHMARKS.md)** - Browser performance testing with headless Chromium

### Strategic Documents
- **[HBR-BLUE-OCEAN-CHATMANGPT-VS-CELONIS.md](./HBR-BLUE-OCEAN-CHATMANGPT-VS-CELONIS.md)** - Strategic positioning
- **[HBR-BLUE-OCEAN-INNOVATION-WASM4PM-VS-CELONIS.md](./HBR-BLUE-OCEAN-INNOVATION-WASM4PM-VS-CELONIS.md)** - Innovation analysis
- **[COMPETITIVE-ECONOMICS-WASM4PM-VS-CELONIS.md](./COMPETITIVE-ECONOMICS-WASM4PM-VS-CELONIS.md)** - Competitive economics analysis

## Choose Your Path

**I want to...**

- **Get started quickly** → Read [QUICKSTART.md](./QUICKSTART.md)
- **Learn by example** → Follow [TUTORIAL.md](./TUTORIAL.md) or [tutorials/](./tutorials/)
- **Deploy to production** → Follow [DEPLOYMENT.md](./DEPLOYMENT.md) or [how-to/docker-deploy.md](./how-to/docker-deploy.md)
- **Look up a function** → Check [API.md](./API.md)
- **Understand algorithms** → Read [reference/algorithms.md](./reference/algorithms.md)
- **Use the CLI** → See [reference/cli-commands.md](./reference/cli-commands.md)
- **Troubleshoot issues** → Check [FAQ.md](./FAQ.md)
- **Run benchmarks** → Follow [BENCHMARK-TIERS-USAGE.md](./BENCHMARK-TIERS-USAGE.md)
- **Test in browser** → See [BROWSER-BENCHMARKS.md](./BROWSER-BENCHMARKS.md)
- **Predict outcomes** → See [PREDICTION_ADDITIONS_GUIDE.md](./PREDICTION_ADDITIONS_GUIDE.md)

## Document Summaries

| Document | Description |
|----------|-------------|
| [INDEX.md](./INDEX.md) | Full Diataxis-organized index with 6 role-based learning paths |
| [QUICKSTART.md](./QUICKSTART.md) | 5-minute setup and first algorithm |
| [TUTORIAL.md](./TUTORIAL.md) | Step-by-step examples with complete code samples |
| [DEPLOYMENT.md](./DEPLOYMENT.md) | Build, test, deploy, and publish guide |
| [API.md](./API.md) | Complete API reference (35+ functions) |
| [reference/algorithms.md](./reference/algorithms.md) | All 14+ discovery algorithms with parameters |
| [FAQ.md](./FAQ.md) | Troubleshooting and common questions |
| [CHANGELOG.md](./CHANGELOG.md) | Version history and release notes |
| [PROJECT_STATUS.md](./PROJECT_STATUS.md) | Current status and roadmap |
| [PREDICTION_ADDITIONS_GUIDE.md](./PREDICTION_ADDITIONS_GUIDE.md) | Predictive process mining (18 algorithms) |
| [BROWSER-BENCHMARKS.md](./BROWSER-BENCHMARKS.md) | Browser performance testing |
| [TOOLS.md](./TOOLS.md) | Tool overview and selection |

## 🔍 Search & Index

All documentation is cross-referenced. Look for:
- **🔗 Links** to related documents
- **📌 Bookmarks** within long documents
- **📑 Table of Contents** at the top of each guide

## 📝 Contributing

If you find unclear sections or have suggestions for improving the documentation, please open an issue or pull request.

---

**Last Updated**: April 2026
**Version**: 26.4.7
**Status**: Production Ready
