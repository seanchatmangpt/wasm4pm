        ...
        try {
            ...
            if (!ctx.args.model && !ctx.args.algorithm && !ctx.args.config) {
                if (formatter instanceof JSONFormatter) {
                    formatter.error('No model, algorithm, or config specified. Use --model, --algorithm, or --config');
                }
                else {
                    formatter.warn('No model, algorithm, or config specified. Use --model, --algorithm, or --config');
                }
                process.exit(EXIT_CODES.source_error);
            }
            // Step 2: Generate explanation content
            let explanationContent = '';
            const level = (ctx.args.level || 'detailed');
            if (ctx.args.model) {
                ...
                try {
                    ...
                    throw new Error(`Failed to explain config: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.error(`Failed to generate explanation: ${error instanceof Error ? error.message : String(error)}`);
            }
 ...
 */
function getAlgorithmExplanation(algorithm, level) {
    const explanations = {
        dfg: {
