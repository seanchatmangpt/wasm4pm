...
pub fn check_gall_conformance(ocel: &OCEL) -> GallVerdict {
    ...
    for evt in &ocel.events {
        ...
        if let Some(pr_attr) = evt.attributes.iter().find(|a| a.name == "previous_receipt") {
            ...
            // String representation might include quotes from Display, but ocel-core 
            // should provide the raw value if possible. We strip for safety.
    ...
    if chain_broken {
        return GallVerdict::Blocked { reason: "Chain Broken".to_string() };
    }
