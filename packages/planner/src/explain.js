...
export function explain(config) {
    ...
    lines.push(`- **Environment**: browser-safe=${executionPlan.budget.environment.browserSafe}, python-available=${executionPlan.budget.environment.pythonAvailable}`);
    lines.push('');
    // Execution steps
 ...
 */
export function explainBrief(config) {
    const executionPlan = plan(config);
    const lines = [];
