...
struct PetriNetLookup<'a> {
    ...
    /// Pre-computed initial marking vector (indexed by place_idx)
...
impl<'a> PetriNetLookup<'a> {
    fn build(petri_net: &'a PetriNet) -> Self {
        ...
        // Build arc adjacency maps using O(1) set lookup
...
fn replay_log<'a>(
    ...
    // Fix D — u64 bitmask fast-path when ≤64 places and all arc weights are 1.
    // Represents each token as a single bit.  Falls through to the Vec path for
    // weighted arcs or larger nets.
    if n_places <= 64 && petri_net.arcs.iter().all(|a| a.weight.unwrap_or(1) == 1) {
    ...
    for (case_id, trace) in log.traces.iter().enumerate() {
        ...
        // then clamp to [0,1] handles the all-zero identity.
        let c = consumed_tokens.max(1) as f64;
        let p = produced_tokens.max(1) as f64;
        let trace_fitness = (0.5 * (1.0 - missing_tokens as f64 / c)
            + 0.5 * (1.0 - tokens_remaining as f64 / p))
