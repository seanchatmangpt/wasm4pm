...
export const validate = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            const logFormat = ctx.args.format || 'xes';
            const activityKey = ctx.args['activity-key'] || 'concept:name';
            const caseIdKey = ctx.args['case-id-key'] || 'case:concept:name';
            const timestampKey = ctx.args['timestamp-key'] || 'time:timestamp';
            ...
            try {
                ...
                if (humanFormatter instanceof JSONFormatter) {
                    ...
                    humanFormatter.error(`Parse error: ${error}`);
                }
                process.exit(EXIT_CODES.source_error);
            }
            ...
            try {
                ...
                checks.push({
                    ...
                    message: schemaResult.valid ? 'Log schema is valid' : 'Log schema validation failed',
                    details: schemaResult,
                });
                if (!schemaResult.valid) {
                    errors.push(`Schema validation failed: ${schemaResult.message}`);
                }
            }
            catch {
                checks.push({
            ...
            try {
                ...
                const missing = attrsResult.missing ?? [];
                checks.push({
            ...
            try {
                ...
                const hasIssues = qualityResult.issues > 0;
                checks.push({
                    name: 'data_quality',
            ...
            try {
                ...
                const incompleteTraces = tracesResult.incomplete ?? 0;
                checks.push({
            ...
            try {
                ...
                const outOfOrder = orderingResult.out_of_order ?? 0;
                checks.push({
            ...
            if (humanFormatter instanceof JSONFormatter) {
                ...
                printHumanValidation(humanFormatter, validationResults);
            }
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.error(`Validation failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            process.exit(EXIT_CODES.execution_error);
        }
    },
});
function printHumanValidation(formatter, result) {
    ...
    formatter.log('');
    if (status === 'pass') {
        formatter.success(`Event Log Validation — ${result.input}`);
    }
    else if (status === 'warn') {
        formatter.warn(`Event Log Validation — ${result.input}`);
    }
    else {
        formatter.error(`Event Log Validation — ${result.input}`);
    }
    formatter.log(`  Format: ${result.format.toUpperCase()}`);
    formatter.log('');
    // Print check results
