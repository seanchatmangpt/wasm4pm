...
fn test_discovery_span_names() {
    ...
    for target in expected_targets {
        assert!(
            target.starts_with("wasm4pm.discovery."),
            "Target {} should follow naming convention",
            target
        );
        assert!(
            target.len() > "wasm4pm.discovery.".len(),
            "Target {} should have algorithm name",
            target
        );
    }
}
...
fn test_discovery_checkpoint_labels() {
    ...
    for checkpoint in expected_checkpoints {
        ...
        assert!(
            checkpoint.chars().all(|c| c.is_lowercase() || c == '_'),
            "Checkpoint {} should be snake_case",
            checkpoint
        );
    }
}
...
fn test_discovery_span_attributes() {
    ...
    for (algo, attrs) in expected_attributes_by_algorithm.iter() {
        assert!(
            !attrs.is_empty(),
            "Algorithm {} should have documented attributes",
            algo
        );
        // Each algorithm should have at least 'algorithm' and 'activity_key'
        assert!(
            attrs.contains(&"algorithm"),
            "Algorithm {} missing 'algorithm' attribute",
            algo
        );
        assert!(
            attrs.contains(&"activity_key") || algo == &"declare",
            "Algorithm {} missing 'activity_key' attribute (declare is exception)",
            algo
        );
    }
}
