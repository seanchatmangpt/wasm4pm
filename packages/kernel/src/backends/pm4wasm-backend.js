...
export class Pm4wasmBackend {
    ...
    async discover(log, algorithmId, budget) {
        ...
        try {
            ...
            const wasmResult = await this.withTimeout(this.callWasmAlgorithm(algo.wasmFunctionName, wasmLog), timeout, `${algorithmId} exceeded budget timeout of ${timeout}ms`);
    ...
    async conformance(log, model, budget) {
        ...
        try {
            ...
            if (model.model_type === 'dfg') {
                ...
                const timeout = BUDGET_TIMEOUTS[budget.latencyBudget];
                result = await this.withTimeout(this.alignmentConformance(wasmLog, wasmModel), timeout, `Conformance checking exceeded budget timeout of ${timeout}ms`);
            }
            else {
     ...
     */
    isBudgetCompatible(algorithmBudgetTier, budgetLatency) {
    ...
    withTimeout(promise, timeoutMs, timeoutMessage) {
        return Promise.race([
            promise,
            new Promise((_, reject) => setTimeout(() => reject(new Error(`timeout: ${timeoutMessage}`)), timeoutMs)),
        ]);
    }
    /**
     ...
     */
    async convertEventLogToWasm(log) {
     ...
     */
    async convertModelToWasm(model) {
     ...
     * Placeholder: actual implementation in Phase 2.
     */
    async callWasmAlgorithm(wasmFunctionName, logHandle) {
        if (!this.wasmModule) {
     ...
     * Placeholder: actual implementation in Phase 2.
     */
    parseModelOutput(wasmOutput, outputType, algorithmId) {
     ...
     */
    async tokenReplayConformance(logHandle, modelHandle) {
        if (!this.wasmModule) {
            throw new Error('WASM module not loaded');
        }
     ...
     * Placeholder: actual pm4wasm function call in Phase 2.
     */
    async alignmentConformance(logHandle, modelHandle) {
        if (!this.wasmModule) {
            throw new Error('WASM module not loaded');
        }
     ...
     */
    createProvenance(algorithmId, operationType, log, model) {
    ...
    async loadWasmModule() {
        ...
        return {
            ...
            discover_dfg: async (handle) => JSON.stringify({ nodes: [], edges: [] }),
            discover_alpha_plus_plus: async (handle) => JSON.stringify({ nodes: [], edges: [] }),
            discover_inductive_miner: async (handle) => JSON.stringify({ nodes: [], edges: [] }),
            eventlog_from_json: async (json) => `log_handle_${Date.now()}`,
            model_from_json: async (json) => `model_handle_${Date.now()}`,
            token_replay_pure: async (logHandle, modelHandle) => JSON.stringify({ fitness: 0.85, precision: 0.80, generalization: 0.75, simplicity: 100 }),
            compute_optimal_alignments: async (logHandle, modelHandle) => JSON.stringify({ fitness: 0.90, precision: 0.85, generalization: 0.80, simplicity: 100 }),
        };
    }
}
//# sourceMappingURL=pm4wasm-backend.js.map