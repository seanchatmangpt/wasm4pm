...
export const diff = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            const log1Path = ctx.args.log1;
            const log2Path = ctx.args.log2;
            const activityKey = ctx.args['activity-key'] || 'concept:name';
            // Validate both input files exist
            for (const [label, filePath] of [['log1', log1Path], ['log2', log2Path]]) {
                try {
            ...
            const handle1 = wasm.load_eventlog_from_xes(xes1);
            const handle2 = wasm.load_eventlog_from_xes(xes2);
            ...
            const dfg1 = typeof dfg1Raw === 'string' ? JSON.parse(dfg1Raw) : dfg1Raw;
            ...
            const variants1 = normalizeVariants(typeof variants1Raw === 'string' ? JSON.parse(variants1Raw) : variants1Raw);
            const variants2 = normalizeVariants(typeof variants2Raw === 'string' ? JSON.parse(variants2Raw) : variants2Raw);
            // Compute diff
 ...
 */
function computeDiff(dfg1, dfg2, variants1, variants2) {
    // --- Activities ---
    const acts1 = new Set(dfg1.nodes.map((n) => n.id));
    const acts2 = new Set(dfg2.nodes.map((n) => n.id));
 ...
 */
function printHumanDiff(formatter, log1Path, log2Path, result) {
    ...
    const line = (s) => formatter.log(s);
    // Sparkbar helper (8 chars, ▓ filled ░ empty)
    const sparkBar = (val, min, max, width = 8) => {
        if (max <= min)
            return '▓'.repeat(width);
