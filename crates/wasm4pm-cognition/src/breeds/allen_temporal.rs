                ...
                if let Some((node1, node2, rel_str)) = parsed {
                    ...
                    if mask > 0 {
                        ...
                        trace.push(TraceStep {
                            ...
                            depth: 0,
                            objects: vec![
                                ("interval".into(), node1.to_string()),
                                ("interval".into(), node2.to_string()),
                            ],
                        });
                    }
                }
                ...
                if k != i && k != j {
                    ...
                    if t != matrix[k][j] {
                        ...
                        trace.push(TraceStep {
                            ...
                            detail: format!(
                                "{},{},{}",
                                node_names[k], node_names[i], node_names[j]
                            ),
                            depth: 0,
                            objects: vec![
                                ("interval".into(), node_names[k].clone()),
                                ("interval".into(), node_names[i].clone()),
                                ("interval".into(), node_names[j].clone()),
                            ],
                        });
                    }
                }
...
mod tests {
    ...
    fn test_algebraic_property_sweep() {
        ...
        for i in 0..13 {
            ...
            assert_eq!(
                compose_mask(i_mask, eq_mask),
                i_mask,
                "r * eq = r failed for {}",
                rel_to_str(i)
            );
            assert_eq!(
                compose_mask(eq_mask, i_mask),
                i_mask,
                "eq * r = r failed for {}",
                rel_to_str(i)
            );
            ...
            assert_eq!(
                INVERSE[INVERSE[i]],
                i,
                "inverse(inverse(r)) = r failed for {}",
                rel_to_str(i)
            );
            ...
            for j in 0..13 {
                ...
                assert_eq!(
                    inv_comp_ab,
                    comp_invb_inva,
                    "inv(a*b) = inv(b)*inv(a) failed for {} * {}",
                    rel_to_str(i),
                    rel_to_str(j)
                );
            }
    ...
    fn test_path_consistency_4_intervals() {
        ...
        m[0][1] = p;
        m[1][0] = inverse_mask(p);
        m[1][2] = p;
        m[2][1] = inverse_mask(p);
        m[2][3] = p;
        m[3][2] = inverse_mask(p);
