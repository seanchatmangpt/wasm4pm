...
const ARCHETYPES: &[Archetype] = &[
    Archetype {
        ...
        required: UNAVAILABLE | DEPENDENCY_FAIL,
        contradictory: 0,
        hypotheses: 8,
        compiled: true,
    },
    ...
    Archetype {
        ...
        domain: "security",
        required: AUTH_FAIL | POLICY_DENY,
        contradictory: 0,
        hypotheses: 8,
        compiled: true,
    },
    Archetype {
        ...
        required: RESOURCE_PRESSURE | CAPACITY_FAIL,
    ...
    Archetype {
        ...
        required: CONFIG_DRIFT,
        contradictory: VERSION_DRIFT,
        hypotheses: 7,
        compiled: true,
    },
    Archetype {
        ...
        domain: "dependency",
        required: DEPENDENCY_FAIL,
        contradictory: DATA_INVALID,
        hypotheses: 9,
        compiled: true,
    },
    Archetype {
        id: "data_schema_validation",
        domain: "data",
        required: DATA_INVALID,
        contradictory: IO_FAIL,
        hypotheses: 8,
        compiled: true,
    },
    Archetype {
        ...
        domain: "software",
        required: VERSION_DRIFT,
        contradictory: CONFIG_DRIFT,
        hypotheses: 7,
        compiled: true,
    },
    Archetype {
    ...
    Archetype {
        ...
        domain: "network",
        required: NETWORK_FAIL,
        contradictory: IO_FAIL,
        hypotheses: 9,
        compiled: true,
    },
    Archetype {
        id: "build_dependency_toolchain",
        domain: "developer_tooling",
        required: BUILD_FAIL | DEPENDENCY_FAIL,
        contradictory: 0,
        hypotheses: 10,
        compiled: true,
    },
    Archetype {
        id: "policy_admission",
        domain: "governance",
        required: POLICY_DENY,
        contradictory: AUTH_FAIL,
        hypotheses: 7,
        compiled: true,
    },
    Archetype {
        id: "stuck_workflow_state",
        domain: "business_process",
        required: STATE_STUCK,
        contradictory: 0,
        hypotheses: 8,
        compiled: true,
    },
    Archetype {
        id: "unknown_or_novel_causal_topology",
        domain: "novelty",
        required: UNKNOWN_CAUSAL,
        contradictory: 0,
        hypotheses: 16,
        compiled: false,
    },
...
fn evidence_for(ordinal: u64, a: Archetype) -> u32 {
    ...
    if mode < 7 {
        ...
        evidence |= a.contradictory;
    }
    if ordinal % 13 == 0 {
        evidence |= CONFIG_DRIFT;
    }
    if ordinal % 17 == 0 {
        evidence |= DEPENDENCY_FAIL;
    }
    evidence
}
...
fn evaluate(ordinal: u64) -> Episode {
    ...
    let standing = if !a.compiled {
        Standing::Fallback
    } else if !present || contradiction {
        Standing::Refused
    } else {
        Standing::Admitted
    };
    let eliminated = if present {
        u64::from(a.hypotheses.saturating_sub(1))
    } else {
        u64::from(a.hypotheses / 2)
    };

    let mut h = blake3::Hasher::new();
    h.update(b"wasm4pm:generalized-issue-reasoning:v1");
    h.update(a.id.as_bytes());
    h.update(a.domain.as_bytes());
    h.update(&ordinal.to_le_bytes());
    h.update(&evidence.to_le_bytes());
    h.update(&[match standing {
        Standing::Admitted => 1,
        Standing::Refused => 2,
        Standing::Fallback => 3,
    }]);
...
fn run(name: &str, scale: u64) {
    ...
    for i in 0..scale {
        let e = evaluate(black_box(i));
        aggregate.update(e.receipt.as_bytes());
        match e.standing {
            Standing::Admitted => admitted += 1,
            Standing::Refused => refused += 1,
            Standing::Fallback => fallback += 1,
        }
        compiled += u64::from(e.compiled);
...
fn main() {
    let rows = [
        ...
        ("route", 100_000),
        ("route", 1_000_000),
        ("route", FLAGSHIP_SCALE),
        ("eliminate", 100_000),
        ("eliminate", 1_000_000),
        ("compiled", 100_000),
        ("compiled", 1_000_000),
        ("compiled", 5_000_000),
        ("fallback_boundary", 100_000),
        ("fallback_boundary", 1_000_000),
        ("end_to_end", 100_000),
        ("end_to_end", 1_000_000),
        ("end_to_end", 5_000_000),
    ];
