...
function buildAlgorithms() {
    const algos = [
        ...
        {
            id: 'simple_process_tree', category: 'discovery', output_type: 'tree', runs: 5, timeout_ms: 30000,
        ...
        {
            id: 'powl_decision_graph_clustering', category: 'powl', output_type: 'powl', runs: 3, timeout_ms: 120000,
        ...
        {
            id: 'powl_decision_graph_cyclic', category: 'powl', output_type: 'powl', runs: 3, timeout_ms: 120000,
        ...
        {
            id: 'powl_decision_graph_cyclic_strict', category: 'powl', output_type: 'powl', runs: 3, timeout_ms: 120000,
        ...
        {
            id: 'powl_config', category: 'powl', output_type: 'powl', runs: 3, timeout_ms: 60000,
        ...
        // ── Analytics Functions ──
        {
            id: 'event_statistics', category: 'analytics', output_type: 'stats', runs: 5, timeout_ms: 30000,
            description: 'Event Statistics',
            execute: (w, h) => { w.analyze_event_statistics(h); return true; },
        },
        {
            id: 'case_duration', category: 'analytics', output_type: 'stats', runs: 5, timeout_ms: 30000,
            description: 'Case Duration Analysis',
            execute: (w, h) => { w.analyze_case_duration(h); return true; },
        },
        {
            id: 'dotted_chart', category: 'analytics', output_type: 'chart', runs: 5, timeout_ms: 30000,
            description: 'Dotted Chart Analysis',
            execute: (w, h) => { w.analyze_dotted_chart(h); return true; },
        },
        {
            id: 'trace_variants', category: 'analytics', output_type: 'variants', runs: 5, timeout_ms: 30000,
            description: 'Trace Variant Extraction',
            execute: (w, h) => { w.analyze_trace_variants(h, ACTIVITY_KEY); return true; },
        },
        {
            id: 'sequential_patterns', category: 'analytics', output_type: 'patterns', runs: 5, timeout_ms: 30000,
...
function buildConformanceAlgorithms() {
    return [
        {
            id: 'token_replay', category: 'conformance', output_type: 'conformance', runs: 5, timeout_ms: 60000,
    ...
    for (const trace of parsed.traces) {
        for (const rawEvent of trace.events) {
            ...
            if (typeof nameAttr === 'string') {
                ...
                const tagged = nameAttr;
                if (typeof tagged.value === 'string')
                    activitySet.add(tagged.value);
            }
        }
    }
...
}, 60000); // 60s timeout for WASM init + XES load
...
describe('WASM benchmarks: discovery + POWL + analytics on BPI 2020', () => {
    ...
    for (const algo of algorithms) {
        it(`${algo.id}: ${algo.description}`, () => {
            ...
            console.info(`[bench] ${algo.id.padEnd(35)} ${String(result.median_ms).padStart(10)}ms  ` +
...
describe('benchmark report', () => {
    it('writes JSON report to results/', () => {
        ...
        const rows = allResults.map(r => `${r.algorithm},${r.category},${r.output_type},${r.median_ms},${r.min_ms},${r.max_ms},${r.runs},${r.events},${r.traces},${r.events_per_sec},${r.status}`).join('\n');
        ...
        for (const cat of categories) {
            ...
            for (const r of catResults) {
                console.info(r.algorithm.padEnd(36) +
            ...
            console.info(''.padEnd(36) +
                ...
                '─'.repeat(12) + ' ' +
                '─'.repeat(6));
            console.info('CATEGORY MEDIAN'.padEnd(36) +
...
//# sourceMappingURL=15-wasm-benchmarks.js.map