    ...
    let (bucket_samples, mut case_durations) = state.with_object(log_handle, |obj| {
        match obj {
            Some(StoredObject::EventLog(log)) => {
                // Fix A+B: use FxHashMap with integer tuple keys to avoid ~190K String
                ...
                // id_to_activity lets us reconstruct the String key once at the end.
                ...
                // Convert (u32, usize) keys back to "activity|prefix_len" String keys
                // for the serializable model. This happens once at build time, not per
                // event, so there is no hot-path allocation cost.
