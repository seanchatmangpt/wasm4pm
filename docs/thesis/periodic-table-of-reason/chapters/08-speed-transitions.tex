...
This chapter proves that reasoning capability is not a smooth function of latency but
exhibits \emph{phase transitions} at four regime boundaries.  We define a four-regime
latency taxonomy (Table~\ref{tab:latency-regimes}) and prove that a certified reasoning
service admits real-time integration at regime $i$ if and only if its mean service time
$s$ satisfies $s < D_i$ and the queue utilisation $\rho = \lambda \cdot s < 1$ (Little's
Law stability condition \cite{feller1968}).  Three qualitatively distinct capabilities ---
Bayesian sensitivity analysis, Pareto-frontier multi-objective planning, and adversarial
robustness auditing --- emerge discontinuously as $s$ crosses regime boundaries from
above.

%% -----------------------------------------------------------------------
\section{Four-Regime Latency Taxonomy}
\label{sec:latency-regimes}
%% -----------------------------------------------------------------------

A single global speed threshold obscures the qualitatively different demands placed on a
reasoning service by different integration contexts.  We instead define four regimes
parameterised by the deadline $D_i$ and the maximum sustainable request rate
$\lambda_{\max,i}$ implied by the Little's Law stability condition $\rho = \lambda \cdot
s < 1$.

\begin{table}[ht]
\centering
\caption{Four-Regime Latency Taxonomy for Certified Reasoning Services}
\label{tab:latency-regimes}
\begin{tabular}{llll}
\hline
\textbf{Regime} & \textbf{Deadline $D_i$} & \textbf{Utilisation Condition} & \textbf{Application} \\
\hline
$\tau_{\text{batch}}$       & seconds         & low $\lambda$ (offline)           & offline reasoning, batch audit \\
$\tau_{\text{interactive}}$ & $\approx 1$ ms  & $\lambda \leq 10^3$ req/s         & request/response API \\
$\tau_{\text{control}}$     & $\approx 100\,\mu\text{s}$ & $\lambda \leq 10^4$ req/s & control loop integration \\
...
\noindent
\emph{Regime assignment for wasm4pm breeds.}  Measured breed latencies
(\texttt{docs/benchmarks/breed-latency-2026-06-10.md}) place
\texttt{production\_rules} ($\approx 2\,\mu\text{s}$) and
\texttt{soar\_rule\_match} ($\approx 5\,\mu\text{s}$) in the
$\tau_{\text{interrupt}}$ regime, and \texttt{strips\_planning}
($\approx 10\,\mu\text{s}$) at the $\tau_{\text{interrupt}}$ boundary.
A hypothetical breed at $\approx 1\,\mu\text{s}$ would fall \emph{below} all current
regime deadlines and is therefore flagged as \emph{aspirational} --- not yet represented
by a validated wasm4pm breed.

A phase transition occurs when $s$ crosses $D_i$ from above: the service exits the
lower-demand regime and enters the higher-demand regime, gaining (or losing) the
epistemic capabilities associated with that regime.
...
\label{thm:speed-phase-transition}
Let $\Breed_i$ be a breed satisfying the categorical axioms of
Chapter~\ref{chap:breed}.  A certified reasoning service with mean service time $s$
admits real-time integration at regime $i$ (with deadline $D_i$ as defined in
Table~\ref{tab:latency-regimes}) if and only if $s < D_i$ \emph{and}
$\rho = \lambda \cdot s < 1$ (Little's Law stability condition \cite{feller1968}).
A phase transition occurs when $s$ crosses $D_i$ from above.  In particular,
when $s \leq \tau^*_\varepsilon = T_D / k^*_\varepsilon$, three capabilities emerge
that are logically inaccessible when $s > \tau^*_\varepsilon$:

\begin{enumerate}[label=(\roman*)]
  \item \emph{Bayesian sensitivity analysis} (requires $s \leq \tau_{\text{control}}$ or lower).
  \item \emph{Pareto-frontier multi-objective planning} (requires $s \leq \tau_{\text{interrupt}}$).
  \item \emph{Adversarial robustness auditing} (requires $s \leq \tau_{\text{interrupt}}$).
\end{enumerate}
\end{theorem}
...
\begin{remark}[Concrete Berry--Ess\'{e}en instantiation]
With $C \approx 0.5$ (Kolmogorov--Smirnov version) and $\varepsilon = 0.05$, the bound requires $k \geq C^2/\varepsilon^2 = 100$ samples. Under a decision budget $T_D = 100\,\text{ms}$, this yields $\tau^*_\varepsilon = 1\,\text{ms}$; under $T_D = 1\,\text{ms}$, $\tau^*_\varepsilon = 10\,\mu\text{s} = \tau_{\text{interrupt}}$. Decision budgets in the millisecond regime thus admit the normal approximation with 5\% error probability; sub-millisecond budgets require the Poisson limit.
\end{remark}
...
(the $\tau_{\text{interrupt}}$ boundary; see \texttt{docs/benchmarks/breed-latency-2026-06-10.md})
can enumerate the complete Pareto front for this instance in roughly $0.2$ seconds, well
