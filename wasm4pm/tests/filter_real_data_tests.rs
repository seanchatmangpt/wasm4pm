...
fn filter_start_activity_all_roadtraffic_traces_start_with_create_fine() {
    ...
    assert_eq!(
        filtered.len(),
        100,
        "pm4py oracle: filter_start_activities(['Create Fine']) = 100, got {}",
        filtered.len()
    );
}
...
fn filter_end_activity_roadtraffic_payment_matches_pm4py_oracle() {
    ...
    assert_eq!(
        filtered.len(),
        47,
        "pm4py oracle: filter_end_activities(['Payment']) = 47, got {}",
        filtered.len()
    );
}
...
fn filter_end_activity_roadtraffic_credit_collection_matches_pm4py_oracle() {
    ...
    assert_eq!(
        filtered.len(),
        36,
        "pm4py oracle: filter_end_activities(['Send for Credit Collection']) = 36, got {}",
        filtered.len()
    );
}
...
fn filter_case_size_roadtraffic_one_to_three_events_matches_pm4py() {
    ...
    assert_eq!(
        filtered, 43,
        "pm4py oracle: traces with 1–3 events = 43, got {}",
        filtered
    );
}
...
fn filter_case_size_roadtraffic_exactly_three_events() {
    ...
    let filtered = log.traces.iter().filter(|t| t.events.len() == 3).count();

    assert_eq!(
        filtered, 5,
        "pm4py oracle: traces with exactly 3 events = 5, got {}",
        filtered
    );
}
...
fn filter_variants_top_k_roadtraffic_top3_matches_pm4py_oracle() {
    ...
    let top3: std::collections::HashSet<Vec<String>> =
        sorted.iter().take(3).map(|(v, _)| (*v).clone()).collect();
    ...
    assert_eq!(
        filtered, 74,
        "pm4py oracle: filter_variants top-3 = 74 traces, got {}",
        filtered
    );
}
...
fn filter_directly_follows_create_fine_to_send_fine_matches_pm4py() {
    ...
    assert_eq!(
        filtered, 77,
        "pm4py oracle: filter_directly_follows(Create Fine → Send Fine) = 77, got {}",
        filtered
    );
}
...
fn filter_traces_containing_add_penalty_matches_pm4py() {
    ...
    assert_eq!(
        filtered, 57,
        "pm4py oracle: traces containing 'Add penalty' = 57, got {}",
        filtered
    );
}
...
fn filter_traces_excluding_add_penalty_is_complement() {
    ...
    assert_eq!(
        excluded, 43,
        "Excluding 'Add penalty' traces: 100 - 57 = 43, got {}",
        excluded
    );
}
...
fn filter_case_size_bpi2020_large_scale_non_degenerate() {
    ...
    assert!(
        filtered > 0 && filtered < log.traces.len(),
        "BPI 2020 filter(1–10 events) must be non-trivial: got {}/{}",
        filtered,
        log.traces.len()
    );
    assert!(
        filtered < log.traces.len(),
...
fn filter_start_activity_bpi2020_reduces_trace_count() {
    ...
    let start_acts: std::collections::HashSet<&str> = log
        .traces
        .iter()
    ...
    assert!(
        !start_acts.is_empty(),
        "BPI 2020 must have start activities"
    );
...
fn filter_variant_coverage_roadtraffic_top1_is_largest_variant() {
    ...
    assert_eq!(
        top1_count, 36,
        "roadtraffic top-1 variant count must be 36, got {}",
        top1_count
    );
}
