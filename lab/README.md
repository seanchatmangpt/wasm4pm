...
- Simulate what an end user does after `npm install @wasm4pm/cli`
...
```
tests/
  nodejs.test.ts       # WASM library API surface (require('@wasm4pm/cli'))
...
```bash
cd lab
npm install           # installs @wasm4pm/cli from npm (not workspace source)
npm test              # 124 tests across 3 files
```

## Dependency

`lab/` depends on `"@wasm4pm/cli": "^26.4.5"` — the published npm package, not `workspace:*`.
