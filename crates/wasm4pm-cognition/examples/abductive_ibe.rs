...
fn main() {
    let input = BreedInput {
        ...
        facts: vec![
            ...
            Fact { key: "ibe:obs:fever".to_string(),   value: "true".to_string() },
            Fact { key: "ibe:obs:cough".to_string(),   value: "true".to_string() },
            Fact { key: "ibe:obs:fatigue".to_string(), value: "true".to_string() },

            // Hypothesis: influenza — explains all three symptoms, moderate assumption cost
            Fact { key: "ibe:hyp:influenza:covers".to_string(), value: "fever,cough,fatigue".to_string() },
            Fact { key: "ibe:hyp:influenza:cost".to_string(),   value: "3".to_string() },

            // Hypothesis: common_cold — explains cough and fatigue only, low cost
            Fact { key: "ibe:hyp:common_cold:covers".to_string(), value: "cough,fatigue".to_string() },
            Fact { key: "ibe:hyp:common_cold:cost".to_string(),   value: "1".to_string() },

            // Hypothesis: pneumonia — explains fever and fatigue, high cost (requires imaging)
            Fact { key: "ibe:hyp:pneumonia:covers".to_string(), value: "fever,fatigue".to_string() },
            Fact { key: "ibe:hyp:pneumonia:cost".to_string(),   value: "8".to_string() },
        ],
        cases: vec![],
        rules: vec![],
        goals: vec![],
    ...
    match run_breed(&breed, &input) {
        Ok(output) => {
            ...
            println!("abductive_ibe ok — selected={:?}  hash={}", output.selected, &output_hash[..16]);
