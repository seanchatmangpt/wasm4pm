...
describe('ml command', () => {
  ...
  describe('error handling', () => {
    it('exits 1 when no task provided', async () => {
      const result = await pmctl(['ml']);
      assertExitCode(result, EXIT_CODES.CONFIG_ERROR);
    });

    it('exits 2 for invalid task name', async () => {
      const result = await pmctl(['ml', 'nonexistent_task', '-i', RUNNING_EXAMPLE]);
    ...
    it('exits 1 when no input provided', async () => {
      const result = await pmctl(['ml', 'classify']);
      assertExitCode(result, EXIT_CODES.CONFIG_ERROR);
    ...
    for (const task of WORKING_ML_TASKS) {
      describe(`${task} task`, () => {
        it(`pictl ml ${task} exits 0 and returns valid JSON`, async () => {
          const result = await pmctl(['ml', task, '-i', RUNNING_EXAMPLE, '--format', 'json']);
  ...
  describe('PCA task', () => {
    it('exits 3 when running-example.xes has insufficient features for PCA', async () => {
      const result = await pmctl(['ml', PCA_TASK, '-i', RUNNING_EXAMPLE, '--format', 'json']);
  ...
  describe('task-specific data fields', () => {
    it('classify has predictions array', async () => {
      const result = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const json = extractJson(result.stdout) as Record<string, unknown>;
      expect(Array.isArray(json.predictions)).toBe(true);
    });

    it('cluster has assignments array', async () => {
      const result = await pmctl(['ml', 'cluster', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const json = extractJson(result.stdout) as Record<string, unknown>;
    ...
    it('forecast has trend object', async () => {
      const result = await pmctl(['ml', 'forecast', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const json = extractJson(result.stdout) as Record<string, unknown>;
      expect(json.trend).toBeDefined();
    });

    it('anomaly has peakIndices array', async () => {
      const result = await pmctl(['ml', 'anomaly', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const json = extractJson(result.stdout) as Record<string, unknown>;
      expect(Array.isArray(json.peakIndices)).toBe(true);
    });

    it('regress has predictions array with actual/predicted fields', async () => {
      const result = await pmctl(['ml', 'regress', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const json = extractJson(result.stdout) as Record<string, unknown>;
      expect(Array.isArray(json.predictions)).toBe(true);
  ...
  describe('determinism', () => {
    it('classify produces same JSON on two runs', async () => {
      const result1 = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '--format', 'json']);
      const result2 = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '--format', 'json']);
  ...
  it('supports --activity-key flag', async () => {
    const result = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '--activity-key', 'concept:name', '--format', 'json']);
    assertExitCode(result, EXIT_CODES.SUCCESS);
    const json = extractJson(result.stdout) as Record<string, unknown>;
  ...
  it('supports --method flag', async () => {
    const result = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '--method', 'knn', '--format', 'json']);
    assertExitCode(result, EXIT_CODES.SUCCESS);
    const json = extractJson(result.stdout) as Record<string, unknown>;
  ...
  it('supports --k flag', async () => {
    const result = await pmctl(['ml', 'cluster', '-i', RUNNING_EXAMPLE, '--k', '3', '--format', 'json']);
    assertExitCode(result, EXIT_CODES.SUCCESS);
    const json = extractJson(result.stdout) as Record<string, unknown>;
    const modelInfo = json.modelInfo as Record<string, unknown>;
