...
describe('e2e-status: Phase Tracking', () => {
    ...
    it('should track start and end timestamps for phases', async () => {
        ...
        expect(new Date(phase.completedAt).getTime() -
            new Date(phase.startedAt).getTime()).toBe(5000);
    });
    ...
    it('should handle phase transitions correctly', async () => {
        ...
        for (const transition of transitions) {
            ...
            expect(['pending', 'running', 'completed', 'failed']).toContain(phase.status);
        }
    });
});
/**
...
describe('e2e-status: Resource Monitoring', () => {
    ...
    it('should track WASM memory usage', async () => {
        ...
        expect(snapshot.resourceUsage.wasmMemoryMaxMB).toBeGreaterThan(snapshot.resourceUsage.wasmMemoryUsedMB);
    });
    it('should not exceed memory limits', async () => {
        ...
        // Assert
        expect(snapshot.resourceUsage.wasmMemoryUsedMB).toBeLessThanOrEqual(snapshot.resourceUsage.wasmMemoryMaxMB);
    });
    ...
    it('should warn if memory usage increases over time', async () => {
        ...
        expect(snapshots[1].resourceUsage.wasmMemoryUsedMB).toBeGreaterThan(snapshots[0].resourceUsage.wasmMemoryUsedMB);
        expect(snapshots[2].resourceUsage.wasmMemoryUsedMB).toBeGreaterThan(snapshots[1].resourceUsage.wasmMemoryUsedMB);
    });
});
/**
...
describe('e2e-status: Progress Reporting', () => {
    ...
    it('should maintain monotonic progress increase', async () => {
        ...
        for (let i = 1; i < snapshots.length; i++) {
            expect(snapshots[i].progress).toBeGreaterThanOrEqual(snapshots[i - 1].progress);
        }
    });
...
describe('e2e-status: Status Queries', () => {
    ...
    it('should retrieve latest status for run', async () => {
        ...
        await fs.writeFile(path.join(statusDir, '1.json'), JSON.stringify(status1));
        await fs.writeFile(path.join(statusDir, '2.json'), JSON.stringify(status2));
        // Act: Load latest
        const files = await fs.readdir(statusDir);
        const latestFile = files.sort().pop();
        const latest = JSON.parse(await fs.readFile(path.join(statusDir, latestFile), 'utf-8'));
        // Assert
...
//# sourceMappingURL=e2e-status.test.js.map