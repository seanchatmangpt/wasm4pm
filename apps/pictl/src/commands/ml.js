...
export const ml = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            if (!VALID_ML_TASKS.includes(task)) {
            ...
            try {
                const result = await executeMlTask(wasm, task, logHandle, activityKey, {
                    method: ctx.args.method,
                    k: ctx.args.k,
                    targetKey: ctx.args['target-key'],
                    forecastPeriods: ctx.args['forecast-periods'],
                    nComponents: ctx.args['n-components'],
                    eps: ctx.args.eps,
                });
                if (formatter instanceof JSONFormatter) {
                    ...
                    formatMlHumanOutput(formatter, task, result);
                }
                if (!ctx.args['no-save']) {
                    const savedPath = await savePredictionResult(`ml-${task}`, inputPath, activityKey, result);
...
function formatMlHumanOutput(formatter, task, result) {
    switch (task) {
        case 'classify': {
            ...
            if (predictions.length > 10)
                formatter.log(`  ... (${predictions.length - 10} more)`);
            const info = result.modelInfo;
            formatter.log(`  Method: ${result.method}, Traces: ${info?.traceCount}, Features: ${info?.featureCount}`);
            formatter.log('');
            break;
        }
        case 'cluster': {
            ...
            if (assignments.length > 10)
                formatter.log(`  ... (${assignments.length - 10} more)`);
            const info = result.modelInfo;
            formatter.log(`  Method: ${result.method}, Clusters: ${result.clusterCount}, Noise: ${result.noiseCount}`);
            if (info?.inertia !== undefined)
                formatter.log(`  Inertia: ${info.inertia}`);
            formatter.log('');
            break;
        }
        case 'forecast': {
            ...
            formatter.log('');
            formatter.log(`  Trend: ${trend?.direction} (slope: ${(trend?.slope ?? 0).toFixed(4)}, strength: ${(trend?.strength ?? 0).toFixed(2)})`);
            formatter.log(`  Window count: ${result.windowCount}`);
            if (forecast) {
                formatter.log(`  Forecast (${forecast.length} periods): ${forecast.map((v) => v.toFixed(1)).join(', ')}`);
            }
            if (seasonality) {
                formatter.log(`  Seasonality: period=${seasonality.period}, strength=${(seasonality.strength ?? 0).toFixed(2)}`);
            }
            formatter.log('');
            break;
        }
        case 'anomaly': {
            const peakIndices = result.peakIndices;
            const peakValues = result.peakValues;
            const residualPeaks = result.residualPeaks;
            formatter.log('');
        ...
        case 'regress': {
            ...
            formatter.log(`  Slope: ${Number(result.slope ?? 0).toFixed(4)}, Intercept: ${Number(result.intercept ?? 0).toFixed(4)}`);
            formatter.log(`  RMSE: ${Number(result.rmse ?? 0).toFixed(2)}, MAE: ${Number(result.mae ?? 0).toFixed(2)}`);
            formatter.log('');
            break;
        }
        case 'pca': {
            const explainedVariance = result.explainedVariance;
            const transformedData = result.transformedData;
            formatter.log('');
            formatter.log(`  Components: ${result.nComponents} (from ${result.originalFeatureCount} features)`);
            if (explainedVariance) {
                formatter.log(`  Explained variance: ${explainedVariance.map((v) => v.toFixed(4)).join(', ')}`);
            }
