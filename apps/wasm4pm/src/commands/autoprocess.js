...
export const autoprocess = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            const cycleConfig = ctx.args.config || '{}';
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.log(`    Health: ${cycle.perception.health_state} (score ${cycle.perception.health_score})`);
                formatter.log('');
                // Decision
                formatter.log('  Decision:');
                formatter.log(`    Guard: ${cycle.decision.guard_result ? 'PASS' : 'FAIL'}`);
                formatter.log(`    Pattern: ${cycle.decision.pattern_result} (${cycle.decision.pattern_ticks} ticks)`);
                formatter.log('');
                // Protection
                ...
                formatter.log(`    Total: ${timing.total_ns} ns (see benchmarks for nanosecond measurements)`);
                formatter.log('');
                // Success indicator
                if (cycle.success) {
            ...
            let exitCode = EXIT_CODES.execution_error;
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.error(`AutoProcess failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            process.exit(exitCode);
        }
    },
});
//# sourceMappingURL=autoprocess.js.map