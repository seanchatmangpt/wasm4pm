...
pub fn verify_receipt(envelope: &Value) -> (VerificationResult, String, String) {
    let session_id = envelope
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let expected_path_hash = envelope
        .get("expected_path_hash")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let ocel2_batch_hash = envelope
        .get("ocel2_batch_hash")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let receipt_hash = envelope
        .get("receipt_hash")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let admission_status = envelope
    ...
    let ocel2 = match envelope.get("ocel2") {
        ...
        None => {
            return (
                VerificationResult::BoundaryMissing,
                "".to_string(),
                "".to_string(),
            )
        }
    ...
    let computed_batch_hash = blake3::hash(canonical_ocel2.as_bytes())
        .to_hex()
        .to_string();

    if computed_batch_hash != ocel2_batch_hash {
        return (
            VerificationResult::ReceiptForged,
            computed_batch_hash,
            "".to_string(),
        );
    }
    ...
    let receipt_seed = format!(
        "{}:{}:{}",
        session_id, computed_batch_hash, expected_path_hash
    );
    ...
    if admission_status == "ReceiptAdmitted" {
        (
            VerificationResult::ReceiptAdmitted,
            computed_batch_hash,
            computed_receipt_hash,
        )
    } else {
        (
            VerificationResult::InvalidTransition,
            computed_batch_hash,
            computed_receipt_hash,
        )
    }
