...
\textbf{Status:} Signed receipts are \textsc{pending}.  The current receipt
chain uses BLAKE3 content hashing but does not yet attach an asymmetric
signature.  The planned mechanism is \texttt{minisign} over the serialised
receipt JSON.
...
\end{verbatim}
When signatures are wired, \texttt{proof-pack/verify-signatures.sh} will
contain the full \texttt{minisign -V} transcript.
