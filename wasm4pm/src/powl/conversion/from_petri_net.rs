...
impl InternalNet {
    ...
    fn is_silent(&self, name: &str) -> bool {
        self.transitions.get(name).is_some_and(|l| l.is_none())
    }
...
fn translate_partial_order(
    ...
    for &(g1, g2) in &connection_edges {
        if let (Some(&c1), Some(&c2)) = (group_to_child.get(&g1), group_to_child.get(&g2)) {
            let pos1 = children_clone
                .iter()
                .position(|&c| c == c1)
                .expect("invariant: c1 was inserted into children_clone at group build time");
            let pos2 = children_clone
                .iter()
                .position(|&c| c == c2)
                .expect("invariant: c2 was inserted into children_clone at group build time");
