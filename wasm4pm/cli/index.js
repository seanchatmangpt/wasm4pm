...
import { ProcessMiningClient } from '../src/client.js';
const defaultConfig = {
...
class WASM4PMCLI {
    ...
    async discover(filePath, algorithm) {
    ...
    async export(filePath, format) {
        ...
        try {
            ...
            const outputFile = this.config.outputFile || filePath.replace(/\.[^.]+$/, `.exported.${format}`);
