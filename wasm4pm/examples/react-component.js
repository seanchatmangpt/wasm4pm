...
export const ProcessMiningApp = () => {
    const [log, setLog] = useState(null);
    const [loading, setLoading] = useState(false);
    const [results, setResults] = useState([]);
    const [selectedAlgorithm, setSelectedAlgorithm] = useState('dfg');
      ...
      <div style={styles.grid}>
        ...
        <aside style={styles.sidebar}>
          <section style={styles.section}>
            ...
            <input type="file" accept=".xes,.json,.xml" onChange={handleFileUpload} disabled={loading} style={styles.fileInput}/>
          </section>

          {log && (<section style={styles.section}>
              ...
              <div style={{ marginTop: '15px' }}>
                ...
                <div style={styles.tagList}>
                  {log.activities.map(activity => (<span key={activity} style={styles.tag}>
                      {activity}
                    </span>))}
                </div>
              </div>
          ...
          {log && (<section style={styles.section}>
              ...
              <div style={styles.algorithmSelect}>
                {['dfg', 'petri', 'declare'].map(algo => (<label key={algo} style={styles.radioLabel}>
                    <input type="radio" value={algo} checked={selectedAlgorithm === algo} onChange={(e) => setSelectedAlgorithm(e.target.value)}/>
                    {algo.toUpperCase()}
                  </label>))}
              </div>

              {selectedAlgorithm === 'dfg' && (<div style={{ marginTop: '10px' }}>
                  <label>
                    Min Frequency:
                    <input type="number" min="1" value={minFrequency} onChange={(e) => setMinFrequency(parseInt(e.target.value))} style={{ width: '60px', marginLeft: '10px' }}/>
                  </label>
                </div>)}

              <button onClick={runDiscovery} disabled={!log || loading} style={styles.button}>
        ...
        <main style={styles.main}>
          {!log ? (<div style={styles.emptyState}>
              ...
              <section style={styles.section}>
                ...
                {results.length === 0 ? (<p style={{ color: '#999' }}>No discoveries yet. Run an algorithm to get started.</p>) : (<div style={styles.resultsList}>
                    {results.map((result, idx) => (<div key={idx} style={styles.resultCard}>
