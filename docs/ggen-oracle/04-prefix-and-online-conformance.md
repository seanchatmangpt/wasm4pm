...
| `PetriNet` + `FlatIncidenceMatrix` | `crates/wasm4pm-types/src/models.rs:96` (`final_markings`), `:118-128` (`FlatIncidenceMatrix::get`, `places_count`) | Final markings *exist* — the substrate for "reached a terminal state". | Reachability ("does ANY firing sequence from marking `m` reach a final marking") is **not** queryable — see TO BE BUILT #1. |
...
| `ConformanceResult` (algos) | `crates/wasm4pm-algos/src/conformance.rs` + `crates/wasm4pm-types/src/conformance.rs:63-107` (`fitness`, `precision`, `generalization`, `simplicity`, `total_traces`, `fitting_traces`, `deviating_traces`, `conformance_rate()`) | Serde-ready report payload for the *batch* fitness portion of an oracle report. | Whole-log only; says nothing about prefixes or order-law violations. |
...
|---|---|---|
| `OrderingLaw`, `CompiledLaw`, `PrefixOracle`, detectors D1–D8, `check_tape`, `watch_tape` | **`wasm4pm-algos`** new module `prefix_conformance` | Pure Rust, no `wasm-bindgen` — the only link-safe, subprocess-reachable home (`00-MAP` §1; algos is leaf-ish and re-exports `wasm4pm-types`). Keeps the oracle callable both as a library and from `wpm` without the WASM engine. |
