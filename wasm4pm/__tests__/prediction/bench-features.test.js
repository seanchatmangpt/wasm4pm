...
describe('extract_prefix_features_wasm', () => {
    ...
    it('10 000 calls latency', async () => {
        ...
        for (let i = 0; i < 10000; i++)
            wasm.extract_prefix_features_wasm(prefix);
        const perCall = Number(((performance.now() - t) / 10000).toFixed(5));
        rows.push({
...
describe('build_transition_probabilities', () => {
    it('sample — probabilities sum to 1 per source', async () => {
        ...
        for (const e of result.edges)
            bySource.set(e.from, (bySource.get(e.from) ?? 0) + e.probability);
...
//# sourceMappingURL=bench-features.test.js.map