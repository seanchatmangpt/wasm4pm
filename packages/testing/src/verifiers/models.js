 ...
 */
export function verifySoundness(net, initialMarking, finalMarking) {
    const details = [];
    ...
    const incomingArcs = new Map();
    const outgoingArcs = new Map();
    // Initialize arc maps
 ...
 */
function checkBoundedness(net, incomingArcs, outgoingArcs, details) {
 ...
 */
function hasPathFromInitiallyMarked(placeId, initiallyMarked, placeIds, incomingArcs, outgoingArcs, visited = new Set()) {
 ...
 */
function checkDeadlockFreedom(net, initialMarking, incomingArcs, outgoingArcs, details) {
 ...
 */
function canReachFinalMarking(net, initialMarking, finalMarking, incomingArcs, outgoingArcs, details) {
 ...
 */
function isPlaceReachable(targetPlace, sourcePlaces, placeIds, incomingArcs, outgoingArcs, visited = new Set()) {
 ...
 */
export function computeQualityMetrics(model, eventLog, options = { type: 'petrinet' }) {
 ...
 */
function computeFitness(model, eventLog, options) {
    if (options.type === 'dfg') {
        return computeDFGFitness(model, eventLog);
    }
    // Token replay fitness for Petri nets
    const conformance = computeTokenReplayConformance(model, eventLog);
    return conformance.fit;
}
/**
 * Compute fitness for DFG model.
 */
function computeDFGFitness(dfg, eventLog) {
    let totalEdges = 0;
    let matchingEdges = 0;
 ...
 */
function computeTokenReplayConformance(net, eventLog) {
 ...
 */
function computePrecision(model, eventLog, options) {
    if (options.type === 'dfg') {
        return computeDFGPrecision(model, eventLog);
    }
 ...
 */
function computeDFGPrecision(dfg, eventLog) {
    const logEdges = extractLogEdges(eventLog);
 ...
 */
function extractLogEdges(eventLog) {
    const edges = new Set();
    for (const trace of eventLog) {
        for (let i = 0; i < trace.activities.length - 1; i++) {
 ...
 */
function computeSimplicity(model, options) {
    if (options.type === 'dfg') {
        const dfg = model;
 ...
 */
export function validateVerifierDFG(dfg) {
    const errors = [];
    // Check nodes
 ...
 */
export function formatSoundnessResult(result) {
    const lines = [];
 ...
 */
export function formatQualityMetrics(metrics) {
    return [
