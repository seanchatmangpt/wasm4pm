...
pub fn dispatch_breed(breed: &str, input: &BreedInput) -> Result<BreedOutput, String> {
    match breed {
        ...
        "abductive_ibe" => crate::breeds::abductive_ibe::AbductiveIbe.run(input).map_err(|e| format!("{}: {}", e.breed, e.message)),
        "partial_order_plan" => crate::breeds::partial_order_plan::PartialOrderPlan.run(input).map_err(|e| format!("{}: {}", e.breed, e.message)),
        "event_calculus" => crate::breeds::event_calculus::EventCalculus.run(input).map_err(|e| format!("{}: {}", e.breed, e.message)),
