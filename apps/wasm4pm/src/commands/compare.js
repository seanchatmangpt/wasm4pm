...
function extractStats(raw) {
    ...
    if (Array.isArray(obj['places']) && Array.isArray(obj['transitions'])) {
        const places = obj['places'].length;
        const transitions = obj['transitions'].length;
        const arcs = Array.isArray(obj['arcs']) ? obj['arcs'].length : 0;
        return { nodes: places + transitions, edges: arcs };
    }
    // Edge-set shape (genetic / ant-colony): { edges: [{from, to}] }
    if (Array.isArray(obj['edges'])) {
        return { nodes: 0, edges: obj['edges'].length };
    }
 ...
 */
function extractModelMetrics(wasm, logHandle, activityKey) {
    const raw = wasm['compute_model_metrics'](logHandle, activityKey);
    const obj = (typeof raw === 'string' ? JSON.parse(raw) : raw);
    const variants = obj['num_variants'] ?? null;
    const density = obj['density'] ?? null;
    const complexity = obj['complexity_score'] ?? null;
 ...
 */
function sparkBar(value, min, max, width = 8) {
    if (max <= min)
        return '░'.repeat(width);
...
function numCol(n, width, decimals = 0) {
...
export const compare = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            const rawAlgos = ctx.args.algorithms
                .split(/[\s,]+/)
                .map((s) => s.trim().toLowerCase())
                .filter(Boolean);
            // Resolve kernel IDs to CLI aliases, then validate
            const resolved = rawAlgos.map((a) => ALGORITHM_CLI_ALIASES[a] ?? a);
            const invalid = resolved.filter((a) => !ALGORITHMS.includes(a));
            if (invalid.length > 0) {
                formatter.error(`Unknown algorithm(s): ${invalid.join(', ')}. Available: ${Object.keys(ALGORITHM_CLI_ALIASES).join(', ')}`);
            ...
            if (formatter instanceof JSONFormatter) {
                formatter.success('Algorithm comparison', {
                    input: inputPath,
                    activityKey,
                    algorithms: stats,
                });
            ...
            // Table header
            humanFormatter.log(`  ${'Algorithm'.padEnd(20)}  ${'Nodes'.padStart(6)}  ${'Edges'.padStart(6)}  ${'Time(ms)'.padStart(9)}  ${'Nodes'.padEnd(10)}  ${'Edges'.padEnd(10)}  ${'Time'.padEnd(10)}`);
            humanFormatter.log(`  ${'─'.repeat(20)}  ${'─'.repeat(6)}  ${'─'.repeat(6)}  ${'─'.repeat(9)}  ${'(bar)'.padEnd(10)}  ${'(bar)'.padEnd(10)}  ${'(bar)'.padEnd(10)}`);
            for (const s of stats) {
                ...
                humanFormatter.log(`  ${algoCol}  ${nodesStr}  ${edgesStr}  ${timeStr}  ${nodesBar}  ${edgesBar}  ${timeBar}`);
            }
            humanFormatter.log('');
            humanFormatter.log('  Legend: ▓▓▓▓▓▓▓▓ = max  ░░░░░░░░ = min   bars are relative within this comparison');
            humanFormatter.log('');
            ...
            if (formatter instanceof JSONFormatter) {
                formatter.error('Comparison failed', error);
            }
            else {
                formatter.error(`Comparison failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            process.exit(EXIT_CODES.execution_error);
        }
    },
