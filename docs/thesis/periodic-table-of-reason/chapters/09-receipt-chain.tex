...
\]
where $\blake$ denotes the BLAKE3 hash function \cite{blake3_2020}, $\|$ denotes
concatenation after canonical serialisation (JSON with lexicographically sorted keys),
and $\mathtt{breed\_id}(\Breed_i)$ is the stable identifier registered in the kernel registry.
\end{definition}
...
\]
as follows.  On objects: $R_{\Breed}(\Breed_i) = \mathcal{R}_i$, the \emph{receipt type}
of $\Breed_i$, defined as the set of all valid receipts for breed $\Breed_i$
(i.e., all 256-bit digests that can be produced by a lawful terminal consultation of
$\Breed_i$).  On morphisms: if $f : \Breed_i \to \Breed_j$ is a breed morphism (a natural
transformation of the underlying inference functors), then
$R_{\Breed}(f) : \mathcal{R}_i \to \mathcal{R}_j$ is the canonical hash-linking map
between receipt types, sending each $\rho_i \in \mathcal{R}_i$ to the receipt
$\rho_j = \blake(\rho_i \| \mathtt{breed\_id}(\Breed_j) \| \ldots) \in \mathcal{R}_j$,
i.e., every valid receipt of the successor breed includes a valid receipt of the
predecessor as a committed prefix.
\end{definition}
...
of all predecessors.  This is precisely the Kleisli composition \cite{maclane1971,moggi1991monads} in the
writer monad over the free monoid of BLAKE3 digests: composing two breed invocations
appends their receipts, and the identity morphism maps to the empty receipt chain.

\phantomsection\label{sec:trust-properties}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \emph{Tamper-evident content binding (content integrity only, not authority binding).}
...
\paragraph{(ii) Tamper-evident content binding (content integrity only, not authority binding).}
BLAKE3 provides tamper-evident content binding: given the hash, any modification to
content produces a different hash with overwhelming probability.  This is a property of
the hash function, not of the authority that computed it.  Non-repudiation requires
additionally a digital signature where the authority holds the private key.  The receipt
schema (Appendix~\ref{app:receipt-schema}) distinguishes these: \texttt{input\_hash} and
\texttt{output\_hash} establish content binding; \texttt{signature} and
\texttt{public\_key\_id} establish accountable execution binding.  As of v26.6.10,
\texttt{signature} and \texttt{public\_key\_id} are emitted by \texttt{cognition\_run()}
using a deterministic Ed25519 actor keyed from a compile-time constant seed.  The Ed25519 signature with the default actor key provides \emph{process-binding integrity}
for replay verification.  Production deployment requires replacing the default actor with
an institutionally managed keypair to achieve \emph{accountable execution binding} in a
legal or regulatory sense.

Concretely, once a receipt $\rho$ is emitted by \texttt{cognition\_run()} and saved to
\texttt{.wasm4pm/receipts/latest.json}, any post-hoc modification to the committed fields
(\texttt{breed\_id}, \texttt{input\_hash}, rule-firing trace $L$, timestamp $t$) is
detectable by recomputing the hash.  Because determinism is a merge gate in wasm4pm
(same inputs must produce bit-exact output), a verifier can replay the computation and
confirm the receipt.  This establishes content-binding integrity but not cryptographic
process-binding integrity: with the v26.6.10 Ed25519 default actor, a verifier can detect
any post-hoc substitution of the committed fields.  Full non-repudiation (binding the
receipt to a named legal actor) additionally requires replacing the default actor with
an institutionally managed keypair held exclusively by the authorised execution environment.
...
receipt value.  The causal completeness gap is now closed \cite{chatman2025proofbound} by the addition of
...
non-forgeability, process-binding integrity,\footnote{The receipt chain satisfies
process-binding integrity (Ed25519 default actor, v26.6.10) but not non-repudiation in
the legal/regulatory sense, which requires an institutionally managed keypair.}
third-party auditability, and --- following the v26.6.10 \texttt{input\_hash} addition
at the WASM boundary --- causal completeness.  Together,
