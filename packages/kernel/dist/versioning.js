 ...
 */
export function satisfiesMinimum(version, minimum) {
 ...
 */
export function isMajorCompatible(version, target) {
    const v = parseSemVer(version);
 ...
 */
export function assertCompatibility(requiredVersion) {
    const result = checkCompatibility(requiredVersion);
    if (!result.compatible) {
        throw new Error(`Kernel version incompatible: ${result.reason} ` +
