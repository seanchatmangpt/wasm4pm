...
fn main() {
    let input = BreedInput {
        ...
        facts: vec![
            // Initially unlocked
            Fact { key: "ec:initially".to_string(),           value: "unlocked".to_string() },
            // Events
            Fact { key: "ec:happens:2".to_string(),           value: "lock".to_string() },
            Fact { key: "ec:happens:5".to_string(),           value: "unlock".to_string() },
            // Causal laws
            Fact { key: "ec:initiates:lock".to_string(),      value: "locked".to_string() },
            Fact { key: "ec:terminates:lock".to_string(),     value: "unlocked".to_string() },
            Fact { key: "ec:initiates:unlock".to_string(),    value: "unlocked".to_string() },
            Fact { key: "ec:terminates:unlock".to_string(),   value: "locked".to_string() },
        ],
        cases: vec![],
        rules: vec![],
        goals: vec![
            // locked should hold at t=3 (after lock at t=2)
            Goal { id: "q1".to_string(), predicate: "ec:holdsat".to_string(), value: "locked@3".to_string() },
            // unlocked should hold at t=6 (after unlock at t=5)
            Goal { id: "q2".to_string(), predicate: "ec:holdsat".to_string(), value: "unlocked@6".to_string() },
            // unlocked should NOT hold at t=3 (clipped by lock)
            Goal { id: "q3".to_string(), predicate: "ec:holdsat".to_string(), value: "unlocked@3".to_string() },
        ],
        state: vec![],
    };
    ...
    match run_breed(&breed, &input) {
        Ok(output) => {
            ...
            println!("event_calculus ok — selected={:?}  hash={}", output.selected, &output_hash[..16]);
