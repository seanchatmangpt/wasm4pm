...
export const main = defineCommand({
  ...
  subCommands: {
    ...
    // Setup (2)
    init,      // Scaffold wasm4pm.toml + .env.example
...
|---------|-------------|------------|
| `wpm run <log.xes>` | Process discovery with configurable algorithm | 0=success, 1=config, 2=source, 3=execution |
| `wpm compare <algos> -i <log>` | Side-by-side algorithm comparison with ASCII sparklines | 0=success, 1=config, 2=source |
| `wpm diff <log1> <log2>` | Compare two event logs (activities, edges, Jaccard) | 0=success, 1=config, 2=source |

### Prediction Commands (1 with 6 perspectives)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm predict next-activity -i <log> --prefix "A,B"` | Predict next activity from trace prefix | 0=success, 1=config, 2=source, 3=execution |
| `wpm predict remaining-time -i <log> --prefix "A"` | Predict remaining case duration | 0=success, 1=config, 2=source, 3=execution |
| `wpm predict outcome -i <log>` | Predict case outcome (classification) | 0=success, 1=config, 2=source, 3=execution |
| `wpm predict drift -i <log>` | Detect concept drift in process behavior | 0=success, 1=config, 2=source, 3=execution |
| `wpm predict features -i <log>` | Extract features for ML pipeline | 0=success, 1=config, 2=source, 3=execution |
| `wpm predict resource -i <log>` | Predict resource allocation/intervention | 0=success, 1=config, 2=source, 3=execution |

### Conformance & Quality Commands (3)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm conformance -i <log>` | Measure log-to-model fitness and precision | 0=success, 1=config, 2=source, 3=execution |
| `wpm quality -i <log>` | Multi-dimensional quality (fitness, precision, generalization, simplicity) | 0=success, 1=config, 2=source, 3=execution |
| `wpm validate <log.xes>` | Validate log schema, required attributes, data quality | 0=valid, 1=invalid |

### Analysis & Simulation Commands (3)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm temporal -i <log>` | Analyze temporal profiles and performance patterns | 0=success, 1=config, 2=source, 3=execution |
| `wpm social -i <log>` | Mine social networks (handover, working together) | 0=success, 1=config, 2=source, 3=execution |
| `wpm simulate -i <log>` | Monte Carlo simulation and process tree playout | 0=success, 1=config, 2=source, 3=execution |

### Monitoring Commands (1)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm drift-watch -i <log>` | Live EWMA concept drift monitor (Ctrl+C to stop) | 0=success, 1=config, 2=source, 130=SIGINT |

### ML Analysis Commands (1 with 6 tasks)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm ml classify -i <log>` | Classify traces (knn, logistic_regression) | 0=success, 1=config, 2=source, 3=execution |
| `wpm ml cluster -i <log>` | Cluster traces (kmeans, dbscan) | 0=success, 1=config, 2=source, 3=execution |
| `wpm ml forecast -i <log>` | Forecast drift trends | 0=success, 1=config, 2=source, 3=execution |
| `wpm ml anomaly -i <log>` | Detect anomalies in drift signal | 0=success, 1=config, 2=source, 3=execution |
| `wpm ml regress -i <log>` | Regress remaining time prediction | 0=success, 1=config, 2=source, 3=execution |
| `wpm ml pca -i <log>` | PCA dimensionality reduction | 0=success, 1=config, 2=source, 3=execution |

### POWL Commands (1 with sub-commands)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm powl construct -i <log>` | Construct POWL model from event log | 0=success, 1=config, 2=source, 3=execution |
| `wpm powl replay -i <log>` | Replay log against POWL model | 0=success, 1=config, 2=source, 3=execution |

### Results & Health Commands (4)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm results` | View all saved discovery & prediction results | 0=success, 1=no_results |
| `wpm results --last` | Print the most recent result | 0=success, 1=no_results |
| `wpm doctor` | 17-check environment diagnostic | 0=all_ok, 1=some_failed |
| `wpm status` | WASM module status and memory usage | 0=success |

### Setup Commands (2)

| Command | Description | Exit Codes |
|---------|-------------|------------|
| `wpm init` | Scaffold wasm4pm.toml + .env.example in current dir | 0=success, 1=exists |
| `wpm watch` | Config file watcher — re-run on change | 0=success, 1=config, 130=SIGINT |
| `wpm explain [algorithm]` | Human/academic algorithm explanations | 0=success |
