...
describe('Pm4wasmBackend', () => {
    ...
    beforeEach(() => {
        ...
        mockWasmModule = {
            ...
            discover_dfg: vi.fn(async (handle) => JSON.stringify({
                nodes: [
                    { id: 'A', label: 'A', type: 'activity' },
                    { id: 'B', label: 'B', type: 'activity' },
                ],
            ...
            discover_inductive_miner: vi.fn(async (handle) => JSON.stringify({
                nodes: [
                    { id: 'p0', label: 'Place 0', type: 'place' },
            ...
            discover_genetic_algorithm: vi.fn(async (handle) => JSON.stringify({
                nodes: [
                    { id: 'A', label: 'A', type: 'activity' },
                    { id: 'B', label: 'B', type: 'activity' },
                ],
            ...
            eventlog_from_json: vi.fn(async (json) => `log_handle_test`),
            model_from_json: vi.fn(async (json) => `model_handle_test`),
            token_replay_pure: vi.fn(async (logHandle, modelHandle) => JSON.stringify({
                fitness: 0.85,
                precision: 0.80,
                generalization: 0.75,
                simplicity: 100,
            })),
            compute_optimal_alignments: vi.fn(async (logHandle, modelHandle) => JSON.stringify({
                fitness: 0.90,
                precision: 0.85,
                generalization: 0.80,
                simplicity: 100,
    ...
    describe('budget timeout enforcement', () => {
        it('should handle timeout for slow operation', async () => {
            ...
            const slowWasmModule = {
                ...mockWasmModule,
                discover_dfg: vi.fn(() => new Promise((resolve) => setTimeout(() => resolve(JSON.stringify({ nodes: [], edges: [] })), 200))),
    ...
    describe('analyze()', () => {
        it('should return not-supported error for analysis tasks', async () => {
            const result = await backend.analyze(SAMPLE_LOG, { task_type: 'bottleneck_analysis' }, BUDGET_ONLINE);
            expect(result.status).toBe('failed');
            expect(result.error).toContain('not supported by Pm4wasmBackend');
        });
    });
...
//# sourceMappingURL=pm4wasm-backend.test.js.map