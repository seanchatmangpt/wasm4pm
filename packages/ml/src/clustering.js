...
// ---------------------------------------------------------------------------
function kmeansCore(data, k, maxIter = 100) {
    ...
    for (let c = 1; c < k; c++) {
        ...
        for (let j = 0; j < d; j++)
            centCols[j][c] = cols[j][chosen];
    }
    const assignments = new Int32Array(n);
    const counts = new Int32Array(k);
    const sumCols = [];
    for (let j = 0; j < d; j++)
        sumCols.push(new Float64Array(k));
    let iterations = 0;
...
// ---------------------------------------------------------------------------
function dbscanCore(data, eps, minPoints) {
    const col = toColumnar(data);
    const { cols, n, d } = col;
    const epsSq = eps * eps;
    ...
    function regionQueryCountAndFill(idx) {
        let count = 0;
        for (let i = 0; i < n; i++) {
            let ss = 0;
            for (let j = 0; j < d; j++) {
 ...
 */
export async function clusterTraces(featuresJson, options = {}) {
    ...
    if (method === 'dbscan') {
        ...
        for (let i = 0; i < labels.length; i++) {
            if (labels[i] >= 0)
                nClusters = Math.max(nClusters, labels[i] + 1);
            if (labels[i] < 0)
                nNoise++;
        }
        return {
