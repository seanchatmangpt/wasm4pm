...
export class AlgorithmRegistry {
    constructor() {
        ...
        this.deploymentProfileMap = new Map();
    ...
    registerAllAlgorithms() {
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Advanced Alpha+++ algorithm. Extends the original Alpha algorithm with explicit handling of length-1 loops, length-2 loops, and parallel short-loop pairs. Produces a proper Petri net with source/sink places.',
            outputType: 'petrinet',
            complexity: 'O(n²)',
            speedTier: 20,
            qualityTier: 50,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Region-based Petri net discovery. Finds causal place candidates (1-to-1, AND-splits, AND-joins) validated by token replay, with greedy minimization. Produces precise Petri nets with explicit parallel-join/split structure.',
            outputType: 'petrinet',
            complexity: 'NP-Hard',
            speedTier: 80,
            qualityTier: 90,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'SIMD-accelerated streaming directly-follows graph discovery. Approximately 500x faster than standard DFG via vectorized event processing.',
            outputType: 'dfg',
            complexity: 'O(n)',
            speedTier: 1,
            qualityTier: 30,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Hierarchical chunking DFG for massive event logs. Scales to 100B+ events via divide-and-conquer with bounded memory.',
            outputType: 'dfg',
            complexity: 'O(n)',
            speedTier: 5,
            qualityTier: 30,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Probabilistic streaming event log processor. Stateful handle-based API. Use streaming_log_create(), streaming_log_add_trace(), streaming_log_estimate_dfg(), streaming_log_free().',
        ...
        this.registerWithInferredProfiles({
            id: 'smart_engine',
            name: 'Smart Engine',
            description: 'Smart execution engine with adaptive algorithm selection, result caching, and early termination. Output type varies based on log characteristics.',
            outputType: 'dfg',
            complexity: 'O(n)',
            speedTier: 3,
            qualityTier: 45,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Forecast future process throughput using linear trend, autocorrelation seasonality, and optional exponential overlay.',
            outputType: 'ml_result',
            complexity: 'O(n)',
            speedTier: 25,
            qualityTier: 50,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Predict remaining case cycle time using linear, polynomial, or exponential regression on trace features.',
            outputType: 'ml_result',
            complexity: 'O(n)',
            speedTier: 25,
            qualityTier: 50,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Reduce trace feature dimensionality using Principal Component Analysis (Jacobi eigendecomposition).',
            outputType: 'ml_result',
            complexity: 'O(n³)',
            speedTier: 25,
            qualityTier: 55,
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Mine organisational handover-of-work networks from event logs (van der Aalst social network mining). ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Mine working-together social networks: edges represent resources that handled the same case. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Discover an aggregate Object-Centric Directly-Follows Graph (OC-DFG) across all object types. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Discover per-object-type Directly-Follows Graphs from an OCEL. Returns a map from ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Discover an Object-Centric Petri Net (OC-Petri net) from an OCEL. The OC-Petri net captures ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Encode an OCEL as a compact human-readable text representation suitable for LLM context, ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Discover Object-Centric Language Abstraction (OCLA) from an OCEL. Captures the language ' +
        ...
        this.registerWithInferredProfiles({
            id: 'ocel_oc_declare',
            name: 'OC-Declare',
            description: 'Discover Object-Centric Declare constraints from an OCEL. Identifies temporal constraints ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Predict the most likely next activity in a process using n-gram (Markov chain) models. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Predict remaining time to case completion using statistical bucket models and Weibull distribution. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Predict case outcome (success/anomaly) using anomaly scoring against DFG model and boundary coverage analysis. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Detect concept drift in a process by comparing activity distributions across sliding windows. ' +
        ...
        this.registerWithInferredProfiles({
            id: 'compute_ewma',
            name: 'EWMA Smoothing',
            description: 'Compute Exponentially Weighted Moving Average (EWMA) for a series of values. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Measure variant entropy and diversity in the event log. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Compute activity transition matrix (Markov chain) for the process. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Identify where process accelerates/decelerates over time using timestamp deltas. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Compute pairwise trace similarity matrix using Levenshtein distance on activity sequences. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Auto-optimize classification model (RF/XGB) for trace outcome prediction. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Auto-optimize time-series forecasting model for process throughput. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'Agentic discovery engine with LRU caching, convergence monitoring, and fused multi-pass execution. ' +
        ...
        this.registerWithInferredProfiles({
            ...
            description: 'End-to-end agentic lifecycle: perception, decision, protection, and Bellman-optimized policy. ' +
     ...
     */
    registerWithInferredProfiles(metadata) {
     ...
     */
    inferDeploymentProfiles(profiles) {
    ...
    getBestAlgorithmForLogSize(logSize) {
        ...
        if (profile === 'quality') {
            if (traces > 10000)
                return 'heuristic_miner'; // speed guard — genetic too slow
        ...
        if (traces > 50000 || activities > 200)
            return 'dfg';
        if (traces > 10000)
            return 'heuristic_miner';
        if (activities < 20 && traces < 5000)
     ...
     */
    suggestByLogCharacteristics(logStats, profile) {
        ...
        const scored = candidates
            .map((algo) => {
            let score = 0;
            if (!algo.logCharacteristics)
                return { id: algo.id, score: 0 }; // 0 score = no metadata
 ...
 */
export function algorithmToJsonSchema(metadata) {
    const properties = {};
    const required = [];
 ...
 */
export function registryToJsonSchema() {
