 ...
 */
export function topologicalSortPlan(plan: ExecutionPlan): string[] {
 ...
 */
export function createStepDispatcher(handlers: Map<string, StepHandler>): StepDispatcher {
  return {
    async dispatch(step: PlanStep, context: ExecutionContext): Promise<StepResult> {
