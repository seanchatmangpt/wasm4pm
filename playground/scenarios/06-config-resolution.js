...
function printProvenance(label, prov) {
    const table = Object.fromEntries(Object.entries(prov).map(([k, v]) => [k, `${v.source}${v.path ? ` (${path.basename(v.path)})` : ''}`]));
...
describe('config resolution: provenance across all sources', () => {
    it('three-way mix: TOML + ENV + CLI each win for different fields', async () => {
        await fs.writeFile(path.join(tmpDir, 'wasm4pm.toml'), ['[source]', 'kind = "file"', '[execution]', 'profile = "quality"', '[algorithm]', 'name = "heuristic_miner"', '[algorithm.parameters]'].join('\n'));
        const cfg = await resolveConfig({
            cliOverrides: { profile: 'fast' },
            configSearchPaths: [tmpDir],
...
describe('config resolution: Zod validation', () => {
    it('rejects profile "ultra-fast" (not in enum)', async () => {
        await expect(resolveConfig({ configSearchPaths: [tmpDir], env: { WASM4PM_PROFILE: 'ultra-fast' } })).rejects.toThrow(/validation|invalid/i);
    });
    it('rejects profile "turbo" from TOML', async () => {
        await fs.writeFile(path.join(tmpDir, 'wasm4pm.toml'), `[source]\nkind = "file"\n[execution]\nprofile = "turbo"\n`);
        await expect(resolveConfig({ configSearchPaths: [tmpDir] })).rejects.toThrow(/validation|invalid/i);
    });
...
//# sourceMappingURL=06-config-resolution.js.map