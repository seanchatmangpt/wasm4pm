...
describe('build_remaining_time_model', () => {
  it('sample — build latency', async () => {
    ...
    rows.push({ algorithm: 'build_remaining_time_model', dataset: 'sample', traces: SAMPLE_TRACES, durationMs: dur });
  ...
  it('BPI 2020 — build latency', async () => {
    ...
    wasm.build_remaining_time_model(log, 'concept:name', 'time:timestamp');
    const dur = Number((performance.now() - t).toFixed(3));
    rows.push({ algorithm: 'build_remaining_time_model', dataset: 'BPI2020', traces: BPI_TRACES, durationMs: dur });
...
describe('predict_case_duration', () => {
  it('sample — two-activity prefix', async () => {
    ...
    const t = performance.now();
    const result = JSON.parse(wasm.predict_case_duration(model, JSON.stringify(['Request', 'Review'])));
    const dur = Number((performance.now() - t).toFixed(3));
    expect(result).toHaveProperty('remaining_ms');
    expect(result).toHaveProperty('confidence');
    expect(result.remaining_ms).toBeGreaterThanOrEqual(0);
    rows.push({ algorithm: 'predict_case_duration', dataset: 'sample', traces: SAMPLE_TRACES, durationMs: dur, note: `method=${result.method}` });
  });
  ...
  it('sample — 1 000 calls latency', async () => {
    ...
    rows.push({ algorithm: 'predict_case_duration(1k)', dataset: 'sample', traces: SAMPLE_TRACES, durationMs: perCall, note: 'ms/call' });
  ...
  it('BPI 2020 — real prefix remaining time', async () => {
    ...
    rows.push({ algorithm: 'predict_case_duration', dataset: 'BPI2020', traces: BPI_TRACES, durationMs: dur, note: `${remainingH.toFixed(1)}h remaining` });
    expect(result.remaining_ms).toBeGreaterThanOrEqual(0);
  });
});
...
describe('predict_hazard_rate', () => {
  it('sample — Weibull h(t) at 2h elapsed', async () => {
    ...
    rows.push({ algorithm: 'predict_hazard_rate', dataset: 'sample', traces: SAMPLE_TRACES, durationMs: dur, note: `h(2h)=${result.hazard_rate?.toFixed(6)}` });
  });
  ...
  it('sample — 1 000 hazard evaluations latency', async () => {
    ...
    const perCall = Number(((performance.now() - t) / 1000).toFixed(4));
    rows.push({ algorithm: 'predict_hazard_rate(1k)', dataset: 'sample', traces: SAMPLE_TRACES, durationMs: perCall, note: 'ms/call' });
    expect(perCall).toBeLessThan(5);
  });
});
