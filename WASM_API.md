...
| **POWL** | 8+ | Partial-order workflow parsing, simplification, conversion, complexity analysis (`->` sequence, `+` parallel) |
...
| **TrueX** | 1 | OCEL 2.0 execution receipt verification (BLAKE3 + JCS-OCEL canonicalization) |
...
When converting POWL to a process tree (via `powl_to_process_tree`), nodes use the following operator notation:

| Operator | Symbol | Meaning |
|----------|--------|---------|
| Sequence | `->` | Activities execute in strict order |
| Parallel | `+` | Activities execute concurrently (any order) |
| XOR choice | `X` | Exactly one branch executes |
| Loop | `*` | Body executes one or more times |

Example process tree string: `->( A, +( B, C ), D )` — A runs first, then B and C in parallel, then D.
...
| `powl_to_process_tree(powl_handle)` | `Result<JsValue, JsValue>` | Convert to process tree (operators: `->` sequence, `+` parallel) |
...
## TrueX — OCEL 2.0 Trust Layer (feature: cloud — fog/browser profiles only)

WASM-based BLAKE3 verification of OCEL 2.0 execution receipts. Gated on `#[cfg(feature = "cloud")]`.

| Function | Parameters | Returns | Description |
|----------|-----------|---------|-------------|
| `truex_verify_receipt(envelope_json)` | `envelope_json: string` | `Result<String, JsValue>` | Verify a TrueX envelope — returns structured refusal or admission verdict |
...
```json
{
  ...
  "computed_batch_hash": "<BLAKE3 hex>",
  "computed_receipt_hash": "<BLAKE3 hex>"
}
```

**Status values (refusal taxonomy):**

| Status | Meaning |
|--------|---------|
| `ReceiptAdmitted` | Envelope passes all integrity checks |
| `ReceiptForged` | BLAKE3 digest mismatch — payload tampered |
| `ReceiptLaundered` | Receipt fields structurally sound but canonicalization fails |
| `BoundaryMissing` | Required OCEL 2.0 object boundaries absent |
| `SummaryOnlyProof` | Receipt is a summary stub — full proof chain missing |
| `CanonicalizationMismatch` | JCS-OCEL canonical form differs from declared hash |
| `ReplayDetected` | Receipt already admitted in a prior session |
| `InvalidTransition` | Object lifecycle transition is not lawful |
| `IncompletePath` | Route missing required stages |
| `VerifierMismatch` | Receipt was generated by a different verifier version |
...
const envelope = require('fs').readFileSync('envelope.json', 'utf-8');
const result = JSON.parse(wasm.truex_verify_receipt(envelope));

if (result.status === 'ReceiptAdmitted') {
  console.log('Admitted — equivalence class:', result.equivalence_class);
} else {
  console.error('Refused:', result.status);
}
```

**CLI integration:** `wpm truex verify <envelope.json>`  
**Supabase ingest after admission:** `wpm truex verify <envelope.json> --ingest`

**TypeScript `@wasm4pm/supabase` API** (not WASM — pure TypeScript):

| Function | Description |
|----------|-------------|
| `parseTruexEnvelope(raw)` | Parse a raw envelope JSON object into a typed `TruexEnvelope` |
| `assertAdmittedEnvelope(envelope)` | Throw `SupabaseIntegrationError` if admission_status ≠ `ReceiptAdmitted` |
| `ingestTruexEnvelope({ config, envelope })` | Persist an admitted envelope to Supabase via Edge Function or direct upsert |
| `envelopePayloadHash(envelope)` | Compute BLAKE3 hex digest of the canonical envelope payload |

Import: `import { parseTruexEnvelope, ingestTruexEnvelope } from '@wasm4pm/supabase'`
