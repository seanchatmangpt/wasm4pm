...
import { createRequiredFields, validateRequiredFields, REQUIRED_FIELD_NAMES, } from '../src/fields.js';
...
describe('Required fields', () => {
    ...
    it('all required fields appear on every exported span', async () => {
        ...
        for (const span of spans) {
            const attrMap = new Map(span.attributes.map((a) => [a.key, a.value]));
    ...
    it('required fields values match the configured values', async () => {
        ...
        const attrMap = new Map(span.attributes.map((a) => [
            a.key,
            a.value.stringValue,
        ]));
