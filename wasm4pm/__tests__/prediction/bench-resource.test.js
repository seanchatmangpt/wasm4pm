...
import { ms, printTable } from './bench-helpers.js';
let wasm;
const rows = [];
beforeAll(async () => {
...
describe('estimate_queue_delay', () => {
    ...
    it('1 000 000 evaluations — nanosecond latency', () => {
        ...
        const perCall = Number(((performance.now() - t) / 1000000).toFixed(6));
        rows.push({
...
describe('rank_interventions', () => {
    ...
    it('100 000 rankings — latency', () => {
        ...
        const perCall = Number(((performance.now() - t) / 100000).toFixed(5));
        rows.push({
...
//# sourceMappingURL=bench-resource.test.js.map