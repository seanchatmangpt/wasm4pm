...
import { 
// Receipt
isReceipt, ReceiptBuilder, validateReceipt, 
// Errors
createError, validateErrorSystem, TYPED_ERROR_CODES, 
// Plan
validatePlanDAG, 
// Result
ok, err, error as structuredError, isOk, isErr, isError, unwrap, unwrapOr, getExitCode, } from '@wasm4pm/contracts';
// ── Receipt shape ─────────────────────────────────────────────────────────────
...
describe('contracts: Receipt shape', () => {
    ...
    it('all four hash fields match BLAKE3 hex-64 pattern', () => {
        ...
        for (const field of ['config_hash', 'input_hash', 'plan_hash', 'output_hash']) {
    ...
    it('validateReceipt returns valid:true on well-formed receipt', () => {
        ...
        if (result.warnings?.length > 0)
            console.info('[contracts] receipt warnings:', result.warnings);
    });
...
describe('contracts: ErrorInfo exit code ranges', () => {
    it('validateErrorSystem reports no mapping gaps', () => {
        ...
        if (issues.length > 0)
            console.error('[contracts] error system issues:', issues);
...
describe('contracts: Plan DAG invariants', () => {
    ...
    it('plan without source node is rejected', () => {
        const plan = makePlan([{ id: 'algo', kind: 'algorithm', label: 'Algo', config: {}, version: '1.0' }, { id: 'snk', kind: 'sink', label: 'Sink', config: {}, version: '1.0' }], [{ from: 'algo', to: 'snk' }]);
        expect(validatePlanDAG(plan).some(e => /source/i.test(e))).toBe(true);
    });
    it('plan without sink node is rejected', () => {
        const plan = makePlan([{ id: 'src', kind: 'source', label: 'Source', config: {}, version: '1.0' }, { id: 'algo', kind: 'algorithm', label: 'Algo', config: {}, version: '1.0' }], [{ from: 'src', to: 'algo' }]);
    ...
    it('edge referencing unknown node is rejected', () => {
        const plan = makePlan([{ id: 'src', kind: 'source', label: 'Source', config: {}, version: '1.0' }, { id: 'snk', kind: 'sink', label: 'Sink', config: {}, version: '1.0' }], [{ from: 'src', to: 'ghost' }]);
        const errors = validatePlanDAG(plan);
...
//# sourceMappingURL=07-contract-shapes.js.map