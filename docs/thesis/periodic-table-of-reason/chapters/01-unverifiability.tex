...
produces outputs via the Certainty Factor calculus (Definition~\ref{def:cf-calculus}
in Chapter~\ref{chap:mycin-breed}), but the binary itself emits no certificate
...
process model using \texttt{pm4py}.  This check detects deviations between the
