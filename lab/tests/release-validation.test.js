...
beforeAll(() => {
    try {
        ...
        console.info('[release] wasm4pm loaded, version:', typeof wasm['get_version'] === 'function' ? wasm['get_version']() : 'unknown');
    }
    catch (e) {
...
describe('3. End-to-End: XES → DFG', () => {
    ...
    it('3.1 load_eventlog_from_xes(XES_SAMPLE) returns a non-empty string handle', () => {
        expect(wasm).not.toBeNull();
        const result = wasm['load_eventlog_from_xes'](XES_SAMPLE);
        ...
        if (fs.existsSync(XES_FIXTURE)) {
            ...
            handle = wasm['load_eventlog_from_xes'](fixtureXes);
        }
        else {
            handle = result;
        }
    ...
    it('3.2 discover_dfg(handle, "concept:name") returns object with nodes and edges arrays', () => {
        expect(wasm).not.toBeNull();
        if (!handle) {
            console.warn('[release] skipping DFG — no handle');
            return;
        }
        ...
        const typedDfg = dfg;
        const nodes = typedDfg['nodes'];
        const edges = typedDfg['edges'];
        console.info('[release] DFG nodes:', nodes?.length ?? 0, 'edges:', edges?.length ?? 0);
    });
    it('3.3 DFG nodes array is non-empty', () => {
        expect(wasm).not.toBeNull();
        if (!handle) {
            console.warn('[release] skipping activity check — no handle');
            return;
        }
        const raw = wasm['discover_dfg'](handle, 'concept:name');
        let dfg = raw;
        ...
        const labels = nodes.map(n => {
            if (n instanceof Map)
                return String(n.get('label') ?? n.get('id') ?? '');
            const obj = n;
    ...
    it('3.4 DFG edges array is non-empty', () => {
        expect(wasm).not.toBeNull();
        if (!handle) {
            console.warn('[release] skipping edges check — no handle');
            return;
        }
        const raw = wasm['discover_dfg'](handle, 'concept:name');
        let dfg = raw;
    ...
    it('3.5 delete_object(handle) does not throw', () => {
        expect(wasm).not.toBeNull();
        if (!handle) {
            console.warn('[release] skipping cleanup — no handle');
            return;
        }
        expect(() => {
...
describe('4. All 14 Algorithm IDs Reachable', () => {
    ...
    it('4.2 available_discovery_algorithms() returns a Map with an "algorithms" key', () => {
        ...
        const result = wasm['available_discovery_algorithms']();
        expect(result).toBeTruthy();
...
describe('5. Error Messages Are Human-Readable', () => {
    it('5.1 load_eventlog_from_xes with invalid XML does not throw a WASM RuntimeError', () => {
        ...
        try {
            result = wasm['load_eventlog_from_xes']('this is not valid XML at all <unclosed');
        }
        catch (e) {
            thrownError = e;
        }
    ...
    it('5.2 discover_dfg with an invalid handle returns null or throws a plain Error (not WASM panic)', () => {
        expect(wasm).not.toBeNull();
        try {
            const result = wasm['discover_dfg']('definitely-not-a-valid-handle-xyz', 'concept:name');
...
//# sourceMappingURL=release-validation.test.js.map