...
fn predict_model(
    ...
    match model.model_type.as_str() {
        ...
        "NeuralNet" => {
            ...
            let mut predictions = Vec::with_capacity(n_samples);
            for i in 0..n_samples {
                let x = &X[i * n_features..(i + 1).saturating_mul(n_features).min(X.len())];
                        ...
                        for out in 0..n_outputs {
                            ...
                            for inp in 0..n_inputs {
                                ...
                                if w_idx < weights.len() {
                                    val += weights[w_idx] * activations[inp];
                                }
                            }
...
mod tests {
    ...
    fn test_predict_model_random_forest_with_tree_predictions() {
        ...
        let X = vec![1.0, 2.0, 1.0, 2.0, 1.0, 2.0]; // 3 samples × 2 features
        let result = predict_model(&model, &X, 3, 2);
    ...
    fn test_predict_model_random_forest_fallback_weights() {
        ...
        let X = vec![3.0, 4.0,  // sample 0: 3*1 + 4*2 = 11
                     1.0, 0.5]; // sample 1: 1*1 + 0.5*2 = 2
        let result = predict_model(&model, &X, 2, 2);
        assert!(result.is_ok());
        let preds = result.unwrap();
    ...
    fn test_predict_model_gradient_boosting_fallback_uniform_weights() {
        ...
        let X = vec![4.0, 4.0]; // sample 0: 4 * 0.5 + 4 * 0.5 = 4.0
        let result = predict_model(&model, &X, 1, 2);
        assert!(result.is_ok());
        let preds = result.unwrap();
    ...
    fn test_predict_model_neural_net_relu_activation() {
        ...
        let X = vec![2.0]; // output before relu = -2.0
        let result = predict_model(&model, &X, 1, 1);
        assert!(result.is_ok());
        let preds = result.unwrap();
