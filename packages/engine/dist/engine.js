...
export class Engine {
    ...
    handleEngineError(err, code, suggestion, fallbackState = 'failed') {
        const error = {
            code,
        ...
        if (this.requiredOtelAttrs) {
            const errorEvent = Instrumentation.createErrorEvent(this.traceId, error.code, error.message, this.requiredOtelAttrs, { severity: error.severity });
    ...
    withTimeout(work, timeoutMs, timeoutCode) {
        return Promise.race([
            work,
            new Promise((_, reject) => setTimeout(() => reject(new Error(`${timeoutCode} after ${timeoutMs}ms`)), timeoutMs)),
        ]);
    }
    // ── Lifecycle methods ────────────────────────────────────────────────────────
    /**
    ...
    async bootstrap(timeoutMs = 30000) {
        ...
        try {
            ...
            if (!this.stateMachine.canTransition('bootstrapping')) {
                throw new Error(`Cannot bootstrap from state: ${this.state()}. ` +
            ...
            const stateChangeReady = Instrumentation.createStateChangeEvent(this.traceId, 'bootstrapping', 'ready', this.requiredOtelAttrs, { reason: 'WASM and kernel initialized successfully' });
    ...
    async plan(config, timeoutMs = 10000) {
        ...
        try {
            // Validate state
            if (this.state() !== 'ready') {
                throw new Error(`Cannot plan in state: ${this.state()}. Engine must be ready. ` +
            ...
            const planGenerated = Instrumentation.createPlanGeneratedEvent(this.traceId, plan.planId, planHash, plan.totalSteps, this.requiredOtelAttrs, { estimatedDurationMs: plan.estimatedDurationMs });
            ...
            const stateChangeReady = Instrumentation.createStateChangeEvent(this.traceId, 'planning', 'ready', this.requiredOtelAttrs, { reason: 'Plan generated successfully' });
    ...
    async run(plan, timeoutMs = 300000) {
        ...
        try {
            ...
            const receipt = await this.withTimeout(this.executor.run(plan), timeoutMs, 'EXECUTION_TIMEOUT').catch((err) => {
            ...
            const stateChangeReady = Instrumentation.createStateChangeEvent(this.traceId, 'running', 'ready', this.requiredOtelAttrs, { reason: 'Execution completed successfully' });
    ...
    async *watch(plan) {
        try {
            ...
            this.watchSession.start((heartbeat) => {
                this.observability.emitJsonSafe({
    ...
    async recover(options) {
        ...
        try {
            ...
            const recoveryStartedSpan = Instrumentation.createRecoveryStartedEvent(this.traceId, 'soft', previousState, this.requiredOtelAttrs);
            ...
            this.observability.emitOtelSafe(Instrumentation.createRecoveryCompletedEvent(this.traceId, recoveryStartedSpan.event.spanId, 'soft', previousState, this.requiredOtelAttrs, { durationMs: recoveryDuration, mttrMs: this.stateMachine.getMTTR() }));
            this.observability.emitJsonSafe({
                timestamp: new Date().toISOString(),
                level: 'info',
                component: 'engine',
    ...
    async fastRecoverFromFailed() {
        ...
        const recoveryStartedSpan = Instrumentation.createRecoveryStartedEvent(this.traceId, recoveryType, 'failed', this.requiredOtelAttrs);
        this.observability.emitOtelSafe(recoveryStartedSpan.otelEvent);
        this.observability.emitJsonSafe({
            timestamp: new Date().toISOString(),
            level: 'info',
        ...
        try {
            ...
            if (!this.wasmLoader.isInitialized()) {
                ...
                const recoveryDuration = Date.now() - recoveryStart;
                this.stateMachine.recordRecovery(recoveryDuration);
                this.observability.emitOtelSafe(Instrumentation.createRecoveryCompletedEvent(this.traceId, recoveryStartedSpan.event.spanId, 'full', 'failed', this.requiredOtelAttrs, { durationMs: recoveryDuration, mttrMs: this.stateMachine.getMTTR() }));
                this.observability.emitJsonSafe({
                    timestamp: new Date().toISOString(),
                    level: 'info',
                    component: 'engine',
            ...
            const recoveryDuration = Date.now() - recoveryStart;
            this.stateMachine.recordRecovery(recoveryDuration);
            this.observability.emitOtelSafe(Instrumentation.createRecoveryCompletedEvent(this.traceId, recoveryStartedSpan.event.spanId, 'fast', 'failed', this.requiredOtelAttrs, { durationMs: recoveryDuration, mttrMs: this.stateMachine.getMTTR() }));
            this.observability.emitJsonSafe({
                timestamp: new Date().toISOString(),
                level: 'info',
                component: 'engine',
            ...
            const recoveryDuration = Date.now() - recoveryStart;
            this.observability.emitOtelSafe(Instrumentation.createRecoveryCompletedEvent(this.traceId, recoveryStartedSpan.event.spanId, recoveryType, 'failed', this.requiredOtelAttrs, {
    ...
    onStateTransition(event) {
        ...
        console.debug(`[Engine] State transition: ${event.fromState} -> ${event.toState} (${duration}ms)`, event.reason ? `- ${event.reason}` : '');
    }
    /**
 ...
 */
export function createSimpleEngine(kernel, wasmConfig, obsConfig) {
 ...
 */
export function createFullEngine(kernel, planner, executor, wasmConfig, obsConfig, watchConfig) {
