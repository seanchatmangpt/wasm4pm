...
export class Engine {
    ...
    async bootstrap(timeoutMs = 30000) {
        ...
        try {
            ...
            // Emit error event
            if (this.requiredOtelAttrs) {
                const errorEvent = Instrumentation.createErrorEvent(this.traceId, error.code, error.message, this.requiredOtelAttrs, { severity: error.severity, context: error.context });
                this.observability.emitOtelSafe(errorEvent.otelEvent);
                this.observability.emitJsonSafe(errorEvent.jsonEvent);
            }
