    ...
    pub fn create_from_ocel(ocel: &OCEL) -> Self {
        ...
        for (idx, event) in ocel.events.iter().enumerate() {
            for obj_id in event.all_object_ids() {
                object_traces.entry(obj_id.to_string()).or_default().push(idx);
            }
        }
        ...
        for (obj_id, event_indices) in object_traces {
            if let Some(obj_type) = obj_to_type.get(&obj_id) {
                ...
                abstraction.start_ev_types.entry(obj_type.clone()).or_default().insert(first_ev.clone());
                abstraction.end_ev_types.entry(obj_type.clone()).or_default().insert(last_ev.clone());

                let df_set = abstraction.directly_follows.entry(obj_type.clone()).or_default();
