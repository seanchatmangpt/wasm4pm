 ...
 */
class MockBackend implements MiningBackend {
  constructor(
    readonly id: string,
  ...
  supportsQualityFloor(floor: string): boolean {
    const floorOrder = { fast: 0, balanced: 1, quality: 2, research: 3 };
    return (floorOrder[floor as keyof typeof floorOrder] || 0) <= floorOrder[this.maxQualityFloor];
  }

  canRun(concurrencyLevel: number): boolean {
...
class MockBackendRegistry {
  ...
  private backendInstances: Map<string, MiningBackend> = new Map();

  private backendStates: Map<string, 'ready' | 'evicted'> = new Map();
  ...
  setBackendState(id: string, state: 'ready' | 'evicted'): void {
    this.backendStates.set(id, state);
  }

  getBackend(id: string): MiningBackend | undefined {
 ...
 * Test event logs
 */
function createSmallEventLog(): EventLogIR {
  return {
...
function createMediumEventLog(): EventLogIR {
  return {
...
function createLargeEventLog(): EventLogIR {
  return {
 ...
 */
function createSubMsBudget(): BudgetEnvelope {
  return {
...
function createHighMsBudget(): BudgetEnvelope {
  return {
...
function createSecondsBudget(): BudgetEnvelope {
  return {
...
function createBrowserBudget(): BudgetEnvelope {
  return {
    latencyBudget: 'high_ms',
    memoryBudget: 0,
    qualityFloor: 'balanced',
...
describe('FederationController with pm4wasm backend', () => {
  ...
  let wasmBackend: MockBackend;
  let pm4wasmBackend: MockBackend;
  let mlBackend: MockBackend;

  beforeEach(async () => {
    // Setup registry with 3 backends
    registry = new MockBackendRegistry();

    wasmBackend = new MockBackend(
      'wasm',
      false,
    ...
    (registry as any).setBackendState('wasm', 'ready');
    (registry as any).setBackendState('pm4wasm', 'ready');
    (registry as any).setBackendState('ml', 'ready');
  });
  ...
  describe('Rule 3: Latency Budget Gate', () => {
    ...
    it('Latency tier ordering: sub_ms(0) < low_ms(1) < high_ms(2) < seconds(3) < minutes(4)', () => {
      const tiers = ['sub_ms', 'low_ms', 'high_ms', 'seconds', 'minutes'] as const;
      const tierMap = {
        sub_ms: 0,
        low_ms: 1,
        high_ms: 2,
  ...
  describe('Rule 4: Quality Floor Gate', () => {
    it('Test: qualityFloor="quality" → all backends PASS', async () => {
      const log = createSmallEventLog();
      const budget = { ...createHighMsBudget(), qualityFloor: 'quality' as const };
    ...
    it('Test: qualityFloor="research" → WASM FAIL, pm4wasm FAIL, ML PASS', async () => {
      const log = createSmallEventLog();
      const budget = { ...createHighMsBudget(), qualityFloor: 'research' as const };
    ...
    it('Test: qualityFloor="fast" → all backends PASS', async () => {
      const log = createSmallEventLog();
      const budget = { ...createHighMsBudget(), qualityFloor: 'fast' as const };
  ...
  describe('Rule 5: Health State Mapping', () => {
    ...
    it('Test: healthLevel=2 (degraded) → pm4wasm marked degraded, WASM + ML available', async () => {
      ...
      (registry as any).setBackendState('pm4wasm', 'evicted');
    ...
    it('Circuit breaker: Open → HalfOpen → Closed (recovery)', async () => {
      ...
      const now = Date.now();
      breaker.lastOpenedAt = now - 40_000; // 40s in past (timeout is 30s)
  ...
  describe('Rule 7: RL Tiebreaker', () => {
    ...
    it('Test: pm4wasm, ML both pass Rules 1-6 → pm4wasm selected over ML', async () => {
      ...
      (registry as any).setBackendState('wasm', 'evicted');
    ...
    it('RL weight ordering: WASM > pm4wasm > pm4py > ML', () => {
      ...
      for (let i = 0; i < backends.length - 1; i++) {
        const id1 = backends[i].id as keyof typeof idPriority;
        const id2 = backends[i + 1].id as keyof typeof idPriority;
