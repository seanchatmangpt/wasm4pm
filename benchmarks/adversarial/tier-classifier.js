 ...
 */
export function classifyAlgorithm(result, registryMetadata) {
    const reasons = [];
    ...
    if (result.outputType !== registryMetadata.outputType) {
        reasons.push(`Output type mismatch: registry says ${registryMetadata.outputType}, actual is ${result.outputType}`);
    ...
    if (registryMetadata.fitnessCapable && result.quality.fitness < 0.85) {
        reasons.push(`Fitness too low for fitness-capable algorithm: ${result.quality.fitness.toFixed(3)} < 0.85`);
    }
    ...
    if (result.latencyMs > budgetMs) {
        reasons.push(`Latency exceeds budget: ${result.latencyMs.toFixed(1)}ms > ${budgetMs}ms at 500K events`);
    }
 ...
 */
export function classifyAll(results, registryMetadata) {
 ...
 * Summarize tiers.
 */
export function summarizeTiers(classifications) {
 ...
 */
export function printTierSummary(summary) {
    const lines = [];
    lines.push('');
    lines.push('╔════════════════════════════════════════════╗');
