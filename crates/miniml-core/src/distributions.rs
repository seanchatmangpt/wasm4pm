...
fn log_binomial_coeff(n: i64, k: i64) -> f64 {
    ...
    let mut log_c = 0.0;
    for i in 0..k { log_c += ((n - i) as f64).ln(); }
    for i in 1..=k { log_c -= (i as f64).ln(); }
    log_c
}
...
fn normal_ppf_impl(p: f64, mean: f64, std: f64) -> f64 {
    ...
    const A: [f64; 6] = [-3.969683028665376e+01, 2.209460984245205e+02, -2.759285104469687e+02, 1.38357751867269e2, -3.066479806614716e+01, 2.506628277459239e+00];
    ...
    let q = if p < p_low {
        ...
        let q = 1.0 - p;
        let q = (((-((q * C[5] + C[4]) * q + C[3]) * q + C[2]) * q + C[1]) * q + C[0]) / ((((q * D[3] + D[2]) * q + D[1]) * q + D[0]) * q + 1.0);
        -q
    };
...
fn normal_sample_impl(n: usize, mean: f64, std: f64, seed: u64) -> Result<Vec<f64>, MlError> {
    ...
    for _ in 0..pairs {
        ...
        result.push(mean + z0);
        if result.len() < n { result.push(mean + z1); }
    }
    Ok(result)
}
...
fn binomial_cdf_impl(k: i64, n: i64, p: f64) -> f64 {
    ...
    let mut sum = 0.0;
    for i in 0..=k { sum += binomial_pmf_impl(i, n, p); }
    sum
}
...
fn binomial_sample_impl(n_samples: usize, n_trials: i64, p: f64, seed: u64) -> Result<Vec<f64>, MlError> {
    ...
    for _ in 0..n_samples {
        let mut count = 0i64;
        for _ in 0..n_trials { if rng.next_f64() < p { count += 1; } }
...
fn poisson_cdf_impl(k: i64, lambda: f64) -> f64 {
    let mut sum = 0.0;
    for i in 0..=k { sum += poisson_pmf_impl(i, lambda); }
    sum
}
...
fn chi_squared_pdf_impl(x: f64, k: f64) -> f64 {
    if x <= 0.0 { return 0.0; }
    let half_k = k / 2.0;
    let log_pdf = (half_k - 1.0) * x.ln() - x / 2.0 - half_k * 2.0_f64.ln() - log_gamma_impl(half_k);
    log_pdf.exp()
}
...
fn t_ppf_impl(p: f64, df: f64) -> f64 {
    ...
    for _ in 0..100 {
        ...
        if t_cdf_impl(mid, df) < p { lo = mid; } else { hi = mid; }
    }
    0.5 * (lo + hi)
}
...
pub fn gamma_function(x: f64) -> f64 { gamma_function_impl(x) }

#[wasm_bindgen(js_name = "logGamma")]
pub fn log_gamma(x: f64) -> f64 { log_gamma_impl(x) }

#[wasm_bindgen(js_name = "betaFunction")]
pub fn beta_function(a: f64, b: f64) -> f64 { beta_function_impl(a, b) }

#[wasm_bindgen(js_name = "erf")]
pub fn erf(x: f64) -> f64 { erf_impl(x) }

#[wasm_bindgen(js_name = "normalPdf")]
pub fn normal_pdf(x: f64, mean: f64, std: f64) -> f64 { normal_pdf_impl(x, mean, std) }

#[wasm_bindgen(js_name = "normalCdf")]
pub fn normal_cdf(x: f64, mean: f64, std: f64) -> f64 { normal_cdf_impl(x, mean, std) }

#[wasm_bindgen(js_name = "normalPpf")]
pub fn normal_ppf(p: f64, mean: f64, std: f64) -> f64 { normal_ppf_impl(p, mean, std) }
...
pub fn binomial_pmf(k: i64, n: i64, p: f64) -> f64 { binomial_pmf_impl(k, n, p) }

#[wasm_bindgen(js_name = "binomialCdf")]
pub fn binomial_cdf(k: i64, n: i64, p: f64) -> f64 { binomial_cdf_impl(k, n, p) }
...
pub fn poisson_pmf(k: i64, lambda: f64) -> f64 { poisson_pmf_impl(k, lambda) }

#[wasm_bindgen(js_name = "poissonCdf")]
pub fn poisson_cdf(k: i64, lambda: f64) -> f64 { poisson_cdf_impl(k, lambda) }
...
pub fn exponential_pdf(x: f64, lambda: f64) -> f64 { exponential_pdf_impl(x, lambda) }

#[wasm_bindgen(js_name = "exponentialCdf")]
pub fn exponential_cdf(x: f64, lambda: f64) -> f64 { exponential_cdf_impl(x, lambda) }
...
pub fn chi_squared_pdf(x: f64, k: f64) -> f64 { chi_squared_pdf_impl(x, k) }

#[wasm_bindgen(js_name = "chiSquaredCdf")]
pub fn chi_squared_cdf(x: f64, k: f64) -> f64 { chi_squared_cdf_impl(x, k) }
...
pub fn t_pdf(x: f64, df: f64) -> f64 { t_pdf_impl(x, df) }

#[wasm_bindgen(js_name = "tCdf")]
pub fn t_cdf(x: f64, df: f64) -> f64 { t_cdf_impl(x, df) }

#[wasm_bindgen(js_name = "tPpf")]
pub fn t_ppf(p: f64, df: f64) -> f64 { t_ppf_impl(p, df) }

#[wasm_bindgen(js_name = "fPdf")]
pub fn f_pdf(x: f64, d1: f64, d2: f64) -> f64 { f_pdf_impl(x, d1, d2) }

#[wasm_bindgen(js_name = "fCdf")]
pub fn f_cdf(x: f64, d1: f64, d2: f64) -> f64 { f_cdf_impl(x, d1, d2) }
...
mod tests {
    ...
    #[test]
    fn test_gamma() { assert!(approx_eq(gamma_function_impl(5.0), 24.0, 1e-8)); }
    #[test]
    fn test_erf() { assert!(approx_eq(erf_impl(1.0), 0.8427, 1e-3)); }
}
