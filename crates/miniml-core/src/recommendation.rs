...
pub fn matrix_factorization_impl(
    ...
    if config.n_users == 0 || config.n_items == 0 {
        return Err(MlError::new("n_users and n_items must be > 0"));
    }
    if config.n_factors == 0 {
        return Err(MlError::new("n_factors must be > 0"));
    }
    if ratings.len() != config.n_users * config.n_items {
        return Err(MlError::new("ratings length must equal n_users * n_items"));
    }
    ...
    for v in user_factors.iter_mut() {
        *v = (rng.next_f64() - 0.5) * 2.0 * init_scale;
    }
    for v in item_factors.iter_mut() {
        *v = (rng.next_f64() - 0.5) * 2.0 * init_scale;
    }
    let (sum_ratings, count_ratings): (f64, usize) = ratings
        .iter()
        .filter(|&&r| r != 0.0)
        .fold((0.0, 0), |(s, c), &r| (s + r, c + 1));
    let global_mean = if count_ratings > 0 {
        sum_ratings / count_ratings as f64
    } else {
        0.0
    };
    let known_ratings: Vec<(usize, usize, f64)> = (0..config.n_users)
        .flat_map(|u| {
            (0..config.n_items).filter_map(move |i| {
                let r = ratings[u * config.n_items + i];
                if r != 0.0 {
                    Some((u, i, r))
                } else {
                    None
        ...
        .collect();
    if known_ratings.is_empty() {
        return Err(MlError::new(
            "ratings must contain at least one non-zero entry",
        ));
    }
    for _iter in 0..config.max_iter {
        ...
        for &ri in &indices {
            ...
            let u_off = user_idx * config.n_factors;
            let i_off = item_idx * config.n_factors;
            let mut dot = 0.0;
            for k in 0..config.n_factors {
                dot += user_factors[u_off + k] * item_factors[i_off + k];
            }
            let error = actual_rating - (global_mean + dot);
            for k in 0..config.n_factors {
                let uf = user_factors[u_off + k];
                let i_f = item_factors[i_off + k];
...
pub fn matrix_factorization(
    ...
    let mut out = vec![
        ...
        model.n_items as f64,
        model.n_factors as f64,
    ];
    out.extend(&model.user_factors);
    out.extend(&model.item_factors);
    Ok(JsValue::from(out))
}

pub fn matrix_factorization_predict_impl(
    ...
    for &item_id in item_ids {
        if user_id >= model.n_users || item_id >= model.n_items {
            predictions.push(model.global_mean);
            continue;
        }
        let mut dot = 0.0;
        let u_off = user_id * model.n_factors;
        let i_off = item_id * model.n_factors;
        for k in 0..model.n_factors {
            dot += model.user_factors[u_off + k] * model.item_factors[i_off + k];
        }
...
pub fn matrix_factorization_predict(model: &[f64], user_id: usize, item_ids: &[f64]) -> Vec<f64> {
    ...
    let global_mean = model[0];
    let n_users = model[1] as usize;
    let n_items = model[2] as usize;
    let n_factors = model[3] as usize;
    let header_len = 4;
    let expected = header_len + n_users * n_factors + n_items * n_factors;
    if model.len() < expected {
        return vec![];
    }
    let m = MatrixFactorizationModel {
        global_mean,
        n_users,
        n_items,
        n_factors,
        user_factors: model[header_len..header_len + n_users * n_factors].to_vec(),
        item_factors: model[header_len + n_users * n_factors..expected].to_vec(),
    };
...
pub fn user_user_collaborative_impl(
    ratings: &[f64],
    n_users: usize,
    n_items: usize,
    user_id: usize,
    ...
    for other in 0..n_users {
        ...
        let mut norm_b = 0.0;
        for j in 0..n_items {
            let a = target_ratings[j];
            let b = other_ratings[j];
            if a > 0.0 && b > 0.0 {
                dot += a * b;
                norm_a += a * a;
                norm_b += b * b;
        ...
        if norm_a > 0.0 && norm_b > 0.0 && dot > 0.0 {
            similarities.push((other, dot / (norm_a.sqrt() * norm_b.sqrt())));
        }
    }
    similarities.sort_unstable_by(|a, b| b.1.total_cmp(&a.1));
    let neighbors: Vec<(usize, f64)> = similarities.into_iter().take(k_neighbors).collect();
    let mut predictions = vec![0.0; n_items];
    for item in 0..n_items {
        if target_ratings[item] > 0.0 {
            predictions[item] = target_ratings[item];
            continue;
        }
        let mut sum_sim = 0.0;
        ...
        for &(neighbor, sim) in &neighbors {
            ...
            sum_sim += sim;
            sum_weighted += sim * (nr - nm);
        }
        predictions[item] = if sum_sim > 0.0 {
            (target_mean + sum_weighted / sum_sim).max(0.0)
        } else {
            target_mean
        };
    }
...
pub fn user_user_collaborative(
    ratings: &[f64],
    n_users: usize,
    n_items: usize,
    user_id: usize,
...
mod tests {
    ...
    fn test_matrix_factorization_simple() {
        ...
        assert!(pred[0] > 0.0);
        assert!((model.global_mean - 3.444).abs() < 0.01);
    }
    #[test]
    ...
    #[test]
    fn test_mean_of_nonzero() {
        assert_eq!(mean_of_nonzero(&[3.0, 0.0, 5.0]), 4.0);
    }
}
