        ...
        const initWasm = async () => {
            try {
                ...
                const mockModule = {
                    ...
                    get_version: () => '0.5.4',
                    load_eventlog_from_xes: (content) => 'handle_' + Math.random(),
                    load_ocel_from_json: (content) => 'handle_' + Math.random(),
                    analyze_event_statistics: (handle) => JSON.stringify({
                        num_traces: 3,
                        num_events: 14,
                        num_activities: 5,
                    }),
                    analyze_case_duration: (handle) => JSON.stringify({
                        min_duration: 3600000,
                        max_duration: 7200000,
                        mean_duration: 5400000,
                    }),
                    analyze_dotted_chart: (handle) => JSON.stringify({ dotted_chart: [] }),
                    discover_dfg: (handle) => JSON.stringify({ nodes: [], edges: [] }),
                    discover_alpha_plus_plus: (handle, threshold) => JSON.stringify({ places: [], transitions: [] }),
                    discover_declare: (handle) => JSON.stringify({ constraints: [] }),
                    object_count: () => 0,
                    clear_all_objects: () => { },
                };
                setPm(mockModule);
    ...
    return (<div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          <h3>XES File</h3>
          <textarea value={xesInput} onChange={(e) => setXesInput(e.target.value)} placeholder="Paste XES content or load sample" style={{
            width: '100%',
            height: '150px',
            padding: '10px',
        ...
        <div style={{ marginBottom: '10px' }}>
          <h3>OCEL File</h3>
          <textarea value={ocelInput} onChange={(e) => setOcelInput(e.target.value)} placeholder="Paste OCEL JSON content" style={{
            width: '100%',
            height: '150px',
            padding: '10px',
            fontFamily: 'monospace',
        ...
        <div style={{ marginBottom: '10px' }}>
          <h3>Upload File</h3>
          <input ref={fileInputRef} type="file" accept=".xes,.json,.xml" onChange={handleFileUpload}/>
        </div>
      </div>

      {eventLog && (<div style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#f0f0f0' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          <button onClick={() => runAnalysis('statistics')} disabled={!eventLog || isLoading}>
            Event Statistics
          </button>
          <button onClick={() => runAnalysis('duration')} disabled={!eventLog || isLoading} style={{ marginLeft: '10px' }}>
            Case Duration
          </button>
        </div>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          <button onClick={() => runAnalysis('dfg')} disabled={!eventLog || isLoading}>
            Discover DFG
          </button>
          <button onClick={() => runAnalysis('alpha')} disabled={!eventLog || isLoading} style={{ marginLeft: '10px' }}>
            Discover Alpha++
          </button>
          <button onClick={() => runAnalysis('declare')} disabled={!eventLog || isLoading} style={{ marginLeft: '10px' }}>
            Discover DECLARE
          </button>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        {error && (<div style={{ padding: '10px', backgroundColor: '#ffcccc', color: 'red', marginBottom: '10px' }}>
            {error}
          </div>)}
        ...
        <div>
          {analysisResults.map((result, idx) => (<div key={idx} style={{
                marginBottom: '10px',
                padding: '10px',
