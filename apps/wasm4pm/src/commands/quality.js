...
export const quality = defineCommand({
    ...
    async run(ctx) {
        ...
        try {
            ...
            const metricsRaw = ctx.args.metrics || 'fitness,precision,generalization,simplicity';
            ...
            if (invalidMetrics.length > 0) {
                formatter.error(`Invalid metric(s): ${invalidMetrics.join(', ')}. Valid: ${validMetrics.join(', ')}`);
            ...
            if (requestedMetrics.includes('fitness')) {
                try {
                    ...
                    const traces = alignResult.traces;
                    if (traces && traces.length > 0) {
                        ...
                        qualityScores.fitness = alignResult.fitness ?? 1.0;
                    }
                }
                catch {
            ...
            if (requestedMetrics.includes('precision')) {
                try {
                    ...
                    qualityScores.precision = precResult.precision
                        ?? precResult.value
                        ?? 0.5;
                }
                catch {
            ...
            if (requestedMetrics.includes('generalization')) {
                try {
                    ...
                    qualityScores.generalization = genResult.generalization
                        ?? genResult.value
                        ?? 0.5;
                }
                catch {
            ...
            if (requestedMetrics.includes('simplicity')) {
                try {
                    const numPlaces = modelInfo.places?.length ?? 0;
                    const numTransitions = modelInfo.transitions?.length ?? 0;
                    const numArcs = modelInfo.arcs?.length ?? 0;
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                printHumanQuality(formatter, result);
            }
            ...
            if (formatter instanceof JSONFormatter) {
                ...
                formatter.error(`Quality assessment failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            process.exit(EXIT_CODES.execution_error);
        }
    },
});
function printHumanQuality(formatter, result) {
    const scores = result.scores;
    const aggregate = result.aggregate;
    const modelInfo = result.model;
    formatter.log('');
    formatter.success(`Quality Assessment — ${result.input}`);
    formatter.log(`  Activity key: ${result.activityKey}`);
    formatter.log(`  Model: ${modelInfo.type} (${modelInfo.nodes} nodes, ${modelInfo.edges} edges)`);
    formatter.log('');
    // ASCII bar chart for quality scores
    const sparkBar = (value, width = 20) => {
