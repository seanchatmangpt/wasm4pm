...
import { z } from 'zod';
import { spawnWorker, getWorker, listWorkers, storeResult, getResult, dissolveWorkers, getSwarmId, getEpisodeCount, incrementEpisodeCount, setWorkerStatus, } from './worker-registry.js';
...
const CheckConvergenceInput = z.object({
    algorithm: z.string(),
    threshold: z.number().min(0).max(1).default(1.0).describe('Agreement fraction required (1.0 = unanimous, 0.8 = 80% quorum)'),
    worker_ids: z.array(z.string()).optional(),
});
...
const GetSwarmStatusInput = z.object({
    include_results: z.boolean().default(false).describe('Include full result payloads; false returns hashes only'),
});
...
async function runAlgorithmOnWorker(workerId, algorithm, params = {}) {
...
const server = new Server({ name: 'wasm4pm-swarm', version: '26.4.6' }, { capabilities: { tools: {} } });
        ...
        switch (name) {
            ...
            case 'run_all': {
                ...
                const workers = input.worker_ids
                    ...
                    : listWorkers();
                const results = await Promise.all(workers.map(w => runAlgorithmOnWorker(w.workerId, input.algorithm, input.params ?? {})));
            ...
            case 'get_worker_result': {
                ...
                if (!raw)
                    return err(`No results for worker ${input.worker_id}`);
