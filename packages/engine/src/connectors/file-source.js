...
class FileEventStream {
    ...
    async next() {
        ...
        if (this.skippedLines > 0) {
            console.warn(`[FileSource] ${this.skippedLines} invalid JSON line(s) skipped so far (partial data — check your input file)`);
        }
...
export class FileSourceAdapter {
    ...
    async validate() {
        try {
            ...
            if (!stats.isFile()) {
                return error(createError('SOURCE_INVALID', `Not a file: ${this.config.filePath}`, { path: this.config.filePath, isDirectory: stats.isDirectory() }));
            }
            ...
            if (!format) {
                return error(createError('SOURCE_INVALID', `Unrecognized file format: ${this.config.filePath}. Supported: XES, JSON, OCEL`, { path: this.config.filePath }));
            }
            return ok(undefined);
        }
        catch (e) {
            if (e.code === 'ENOENT') {
                return error(createError('SOURCE_NOT_FOUND', `File not found: ${this.config.filePath}`, { path: this.config.filePath }));
            }
            if (e.code === 'EACCES') {
                return error(createError('SOURCE_PERMISSION', `Permission denied reading: ${this.config.filePath}`, { path: this.config.filePath }));
            }
    ...
    async open() {
        ...
        if (lastError.code === 'ENOENT') {
            return error(createError('SOURCE_NOT_FOUND', `File not found after ${this.retry.maxAttempts} retries: ${this.config.filePath}`, { path: this.config.filePath, attempts: this.retry.maxAttempts }));
        }
        if (lastError.code === 'EACCES') {
            return error(createError('SOURCE_PERMISSION', `Permission denied after ${this.retry.maxAttempts} retries: ${this.config.filePath}`, { path: this.config.filePath }));
        }
