...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    vision.  We demonstrate with the \code{wasm4pm}/\code{bcinr} arc, where an
...
Now consider the \code{wasm4pm} algorithm registry.
...
types, APIs, registries, and configuration schemas.  The \code{wasm4pm} registry
...
When the \code{wasm4pm} arc reached the conclusion that 0 of 41 algorithms were
...
Consider the \code{wasm4pm} inductive miner task.  As a ticket, it reads:
...
In the \code{wasm4pm} arc, sequential debugging would have found:
\begin{itemize}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{The wasm4pm Arc as Working Backwards in Practice}
% ─────────────────────────────────────────────────────────────────────────────

\subsection{The Press Release: 41 Algorithms}

The \code{wasm4pm} registry was a press release written in TypeScript.  It
...
The result was not ``wasm4pm failed.''  The result was:
...
The \code{wasm4pm} architectural finding---\code{RefCell<Mutex>} incompatible
...
When the \code{wasm4pm} audit concluded that the Tier 2 gap was not worth
...
The \code{wasm4pm} registry was a press release written in code.  The 41
...
The \code{wasm4pm} registry and the \code{bcinr} ontology are both press
...
The \code{wasm4pm} registry was published to npm as \code{wasm4pm@26.4.18} on April 17, 2026.
...
doctrine that served as the primary oracle throughout the \code{wasm4pm} arc; and
