...
import { KernelError, isKernelError, classifyRustError, toTypedError, wrapKernelCall, } from '../src/errors';
...
describe('classifyRustError', () => {
    ...
    it('should handle empty/null input', () => {
        ...
        expect(classifyRustError(null)).toBe('ALGORITHM_FAILED');
    });
});
...
describe('wrapKernelCall', () => {
    ...
    it('should convert errors to KernelError', async () => {
        await expect(wrapKernelCall(async () => {
            throw new Error('Handle not found: obj_1');
        }, { algorithm: 'dfg' })).rejects.toThrow(KernelError);
    });
