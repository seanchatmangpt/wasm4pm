...
describe('OTEL failure handling', () => {
    ...
    it('logs warning and continues when required=false and fetch fails', async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('OTEL export failed (non-blocking)'));
        warnSpy.mockRestore();
    });
    ...
    it('does NOT block execution when exporter is unavailable', async () => {
        vi.stubGlobal('fetch', vi.fn().mockImplementation(() => new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 500))));
