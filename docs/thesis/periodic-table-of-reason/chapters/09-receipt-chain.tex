...
using a deterministic Ed25519 actor keyed from a compile-time constant seed.  This
provides \emph{process-binding integrity}: a verifier can confirm the receipt was produced
by a runtime possessing that seed.  Production deployment requires replacing the default
actor with an institutionally managed keypair to achieve \emph{accountable execution
binding} (non-repudiation in the legal sense).
