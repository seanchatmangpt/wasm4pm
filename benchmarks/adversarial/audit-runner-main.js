...
// Imports
import { generateScaleSeries } from './synthetic-log-gen.js';
...
async function main() {
    ...
    for (const [scale, stat] of stats) {
        console.log(`   ${scale}: ${stat.totalCases} cases, ${stat.totalEvents} events, fitness=1.0`);
    }
    // Step 2: Load WASM
    ...
    try {
        ...
        console.error('Please build WASM first: cd wasm4pm && npm run build');
    ...
    console.log(printTierSummary(auditResult.classifications.reduce((acc, c) => {
        if (c.tier === 0)
            acc.tier0.push(c.algorithm);
        if (c.tier === 1)
            acc.tier1.push(c.algorithm);
        if (c.tier === 2)
            acc.tier2.push(c.algorithm);
        if (c.tier === 3)
            acc.tier3.push(c.algorithm);
        acc.totalAlgorithms++;
        return acc;
    ...
    console.log('\n💡 Step 6: Recommendations');
    const tier3 = auditResult.classifications.filter((c) => c.tier === 3);
    if (tier3.length > 0) {
    ...
    const tier2 = auditResult.classifications.filter((c) => c.tier === 2);
    if (tier2.length > 0) {
    ...
    const tier1 = auditResult.classifications.filter((c) => c.tier === 1);
    if (tier1.length > 0) {
    ...
    const tier0 = auditResult.classifications.filter((c) => c.tier === 0);
    if (tier0.length > 0) {
...
//# sourceMappingURL=audit-runner-main.js.map