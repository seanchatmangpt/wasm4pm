...
// ────────────────────────────────────────────────────────────────────────────
async function resolveWasmPkgDir() {
    let dir = process.cwd();
    for (let i = 0; i < 10; i++) {
...
async function checkSimdSupport() {
    try {
        ...
        const compile = globalThis.WebAssembly?.compile;
        if (!compile) {
            return {
                name: 'WASM SIMD',
                status: 'warn',
...
async function checkGitHooks() {
    ...
    for (let i = 0; i < 10; i++) {
        if (existsSync(path.join(gitDir, '.git')))
            break;
...
// ────────────────────────────────────────────────────────────────────────────
async function checkResultsDir() {
    const rootDir = await resolveWorkspaceRoot();
    if (!rootDir) {
...
// ────────────────────────────────────────────────────────────────────────────
async function checkAlgorithmRegistry() {
    const wasmPkgDir = await resolveWasmPkgDir();
    if (!wasmPkgDir) {
...
let _cachedRoot;
function getCachedWorkspaceRoot() {
...
async function checkStepTypeSync() {
    if (!hasSourceAccess()) {
    ...
    for (const m of enumMatch[1].matchAll(/'([^']+)'/g))
        enumValues.add(m[1]);
    ...
    if (inEnumNotArray.length > 0)
        details.push(`${inEnumNotArray.length} in enum but not array: ${inEnumNotArray.slice(0, 3).join(', ')}`);
    if (inArrayNotEnum.length > 0)
        details.push(`${inArrayNotEnum.length} in array but not enum: ${inArrayNotEnum.slice(0, 3).join(', ')}`);
    return {
...
async function checkRegistryConsistency() {
    if (!hasSourceAccess()) {
    ...
    const ids = new Set();
    for (const m of idsMatch[1].matchAll(/'([^']+)'/g))
        ids.add(m[1]);
    const stepTypeKeys = new Set();
    for (const m of stepTypeMatch[1].matchAll(/(\w+)\s*:/g))
        stepTypeKeys.add(m[1]);
    const displayKeys = new Set();
    for (const m of displayMatch[1].matchAll(/(\w+)\s*:/g))
        displayKeys.add(m[1]);
    const outputKeys = new Set();
    for (const m of outputMatch[1].matchAll(/(\w+)\s*:/g))
        outputKeys.add(m[1]);
    const issues = [];
...
async function checkStateMachineIntegrity() {
    if (!hasSourceAccess()) {
    ...
    for (const s of stateValues) {
        if (!transitionKeys.has(s))
            issues.push(`EngineState '${s}' missing from VALID_TRANSITIONS`);
    }
    // VALID_TRANSITIONS keys not in EngineState
    for (const k of transitionKeys) {
        if (!stateValues.has(k))
            issues.push(`VALID_TRANSITIONS key '${k}' not in EngineState`);
    }
    // Transition targets not in EngineState
    for (const t of allTargets) {
        if (!stateValues.has(t))
            issues.push(`Transition target '${t}' not a valid EngineState`);
    }
...
async function checkProfileCoverage() {
    if (!hasSourceAccess()) {
...
async function checkCanonicalNaming() {
    if (!hasSourceAccess()) {
...
async function checkStepTypeCoverage() {
    if (!hasSourceAccess()) {
...
async function checkStateMachineCompleteness() {
    if (!hasSourceAccess()) {
