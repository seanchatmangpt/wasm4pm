...
import { pmctl, extractJson, resolveRepo, EXIT_CODES, assertExitCode } from '../helpers/cli.js';
...
describe('ML correctness validation', () => {
  ...
  describe('classify task — correctness', () => {
    ...
    it('classify results are deterministic (same k, same output)', async () => {
      const result1 = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '-k', '3', '--format', 'json']);
      const result2 = await pmctl(['ml', 'classify', '-i', RUNNING_EXAMPLE, '-k', '3', '--format', 'json']);
  ...
  describe('cluster task — correctness', () => {
    ...
    it('cluster assignments are deterministic with same k', async () => {
      const result1 = await pmctl(['ml', 'cluster', '-i', RUNNING_EXAMPLE, '-k', '2', '--format', 'json']);
      const result2 = await pmctl(['ml', 'cluster', '-i', RUNNING_EXAMPLE, '-k', '2', '--format', 'json']);
