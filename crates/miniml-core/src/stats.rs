...
impl AnovaResult {
    ...
    #[wasm_bindgen(getter)]
    pub fn between_groups_ss(&self) -> f64 {
        self.between_groups_ss
    }
    #[wasm_bindgen(getter)]
    pub fn within_groups_ss(&self) -> f64 {
        self.within_groups_ss
    }
    #[wasm_bindgen(getter)]
    pub fn between_groups_df(&self) -> f64 {
        self.between_groups_df
    }
    #[wasm_bindgen(getter)]
    pub fn within_groups_df(&self) -> f64 {
        self.within_groups_df
    }
}
...
pub fn t_test_one_sample_impl(
    ...
    let n = data.len();
    if n < 2 {
        return Err(MlError::new("Need at least 2 observations for a t-test"));
    }
...
pub fn t_test_two_sample_impl(
    ...
    let n2 = data2.len();
    if n1 < 2 || n2 < 2 {
        return Err(MlError::new("Each sample needs at least 2 observations"));
    }
    if alpha <= 0.0 || alpha >= 1.0 {
        return Err(MlError::new("alpha must be between 0 and 1"));
    }
...
pub fn t_test_paired_impl(
    ...
    if data1.len() != data2.len() {
        return Err(MlError::new("Paired samples must have the same length"));
    }
    if data1.len() < 2 {
        return Err(MlError::new("Need at least 2 paired observations"));
    }
    if alpha <= 0.0 || alpha >= 1.0 {
        return Err(MlError::new("alpha must be between 0 and 1"));
    }
...
pub fn welch_t_test_impl(data1: &[f64], data2: &[f64], alpha: f64) -> Result<TTestResult, MlError> {
    let n1 = data1.len();
    let n2 = data2.len();
    if n1 < 2 || n2 < 2 {
        return Err(MlError::new("Each sample needs at least 2 observations"));
...
pub fn mann_whitney_u_impl(data1: &[f64], data2: &[f64]) -> Result<MannWhitneyResult, MlError> {
    let n1 = data1.len();
    let n2 = data2.len();
    if n1 == 0 || n2 == 0 {
        return Err(MlError::new("Both samples must be non-empty"));
    }
    ...
    let rank_counts: Vec<usize> = {
        ...
        while i < sorted.len() {
            let mut j = i + 1;
            while j < sorted.len() && (sorted[j] - sorted[i]).abs() < 1e-15 {
                j += 1;
            }
            counts.push(j - i);
            i = j;
    ...
    let tie_correction: f64 = rank_counts
        .iter()
...
pub fn wilcoxon_signed_rank_impl(
    ...
    let n = diffs.len();
    if n == 0 {
        return Err(MlError::new("No non-zero differences found"));
    }
...
pub fn ks_test_impl(data: &[f64]) -> Result<KSTestResult, MlError> {
    let n = data.len();
    if n < 2 {
        return Err(MlError::new("Need at least 2 observations for KS test"));
    }
...
pub fn chi_square_test_impl(
    ...
    if observed.is_empty() {
        return Err(MlError::new("Data must be non-empty"));
    }
    for &e in expected {
        if e <= 0.0 {
            return Err(MlError::new("Expected frequencies must be positive"));
        }
    }
...
pub fn chi_square_independence_impl(
    ...
    if contingency.len() != n_rows * n_cols {
        return Err(MlError::new(
            "Contingency table size does not match n_rows * n_cols",
        ));
    }
    ...
    if grand_total <= 0.0 {
        return Err(MlError::new("Grand total must be positive"));
    }
...
pub fn one_way_anova_impl(groups: &[f64], group_sizes: &[usize]) -> Result<AnovaResult, MlError> {
    if group_sizes.is_empty() {
        return Err(MlError::new("Need at least one group"));
    }
    if group_sizes.len() < 2 {
        return Err(MlError::new("Need at least 2 groups for ANOVA"));
    }

    let k = group_sizes.len();
    let total_n: usize = group_sizes.iter().sum();
    if groups.len() != total_n {
        return Err(MlError::new(
            "Total group sizes must equal length of groups data",
        ));
    }
    if total_n < k + 1 {
        return Err(MlError::new("Not enough observations for ANOVA"));
    }
    ...
    for &size in group_sizes {
        if size == 0 {
            return Err(MlError::new("Group sizes must be positive"));
        }
        ...
        for &val in group {
            within_ss += (val - group_mean) * (val - group_mean);
        }
        offset += size;
    }
    ...
    Ok(AnovaResult {
        f_statistic,
        p_value,
        between_groups_ss: between_ss,
        within_groups_ss: within_ss,
        between_groups_df: between_df,
        within_groups_df: within_df,
    })
}
...
pub fn confidence_interval_mean_impl(
    ...
    let n = data.len();
    if n < 2 {
        return Err(MlError::new("Need at least 2 observations"));
    }
    if alpha <= 0.0 || alpha >= 1.0 {
        return Err(MlError::new("alpha must be between 0 and 1"));
    }
...
pub fn confidence_interval_proportion_impl(
    ...
    if total <= 0 {
        return Err(MlError::new("Total must be positive"));
    }
    if successes < 0 || successes > total {
        return Err(MlError::new("Successes must be between 0 and total"));
    }
    if alpha <= 0.0 || alpha >= 1.0 {
        return Err(MlError::new("alpha must be between 0 and 1"));
    }
...
mod tests {
    ...
    fn test_t_test_one_sample_known() {
        ...
        assert!(
            approx_eq(result.statistic, 0.0, 1e-10),
            "Expected t=0, got {}",
            result.statistic
        );
        assert!(
    ...
    fn test_t_test_two_sample_identical() {
        ...
        assert!(
            approx_eq(result.statistic, 0.0, 1e-10),
            "Expected t=0 for identical samples, got {}",
            result.statistic
        );
        assert!(
            result.p_value > 0.99,
            "Expected p~1 for identical samples, got {}",
            result.p_value
        );
    }
    ...
    fn test_t_test_paired() {
        ...
        assert!(
            approx_eq(result.mean_diff, -1.0, 1e-10),
            "Expected mean_diff=-1, got {}",
            result.mean_diff
        );
        assert!(approx_eq(result.df, 4.0, 1e-10));
    }
    ...
    fn test_mann_whitney_separated() {
        ...
        assert!(
            result.p_value < 0.05,
            "Expected p < 0.05 for separated groups, got {}",
            result.p_value
        );
        // With perfect separation, min(U1, U2) = 0 which is correct
        assert!(
            result.u_statistic == 0.0,
            "Expected U=0 for perfectly separated groups, got {}",
            result.u_statistic
        );
    }
    ...
    fn test_chi_square_good_fit() {
        ...
        assert!(
            approx_eq(result.statistic, 10.0, 1e-6),
            "Expected chi2=10, got {}",
            result.statistic
        );
        assert!(
            result.p_value < 0.05,
    ...
    fn test_chi_square_independence_2x2() {
        ...
        assert!(
            approx_eq(result.statistic, 100.0 / 15.0, 1e-4),
            "Expected chi2=6.667, got {}",
            result.statistic
        );
    ...
    fn test_anova_different_groups() {
        ...
        assert!(
            ...
            "Expected very small p-value, got {}",
            result.p_value
        );
        assert!(
            result.f_statistic > 100.0,
            "Expected large F statistic, got {}",
            result.f_statistic
        );
    ...
    fn test_confidence_interval_mean() {
        ...
        assert!(
            approx_eq(result.point_estimate, 6.0, 1e-10),
            "Expected point_estimate=6, got {}",
            result.point_estimate
        );
        assert!(
            result.lower <= 6.0 && result.upper >= 6.0,
            "CI [{}, {}] does not contain mean 6.0",
            result.lower,
            result.upper
        );
    ...
    fn test_describe_basic() {
        ...
        assert!(
            approx_eq(result.mean, 3.0, 1e-10),
            "Expected mean=3, got {}",
            result.mean
        );
        assert!(
            approx_eq(result.median, 3.0, 1e-10),
            "Expected median=3, got {}",
            result.median
        );
        assert!(
            approx_eq(result.std, 2.5_f64.sqrt(), 1e-6),
            "Expected std~1.5811, got {}",
            result.std
        );
        ...
        assert!(
            ...
            "Expected skewness~0 for symmetric data, got {}",
            result.skewness
        );
    }
    ...
    fn test_ks_test_normal_data() {
        let data = vec![
            -0.5, 0.2, -0.1, 0.8, -0.3, 0.1, -0.7, 0.4, -0.2, 0.6, -0.4, 0.3, -0.6, 0.0, -0.8, 0.5,
            -0.9, 0.7, -0.15, 0.35,
        ];
        let result = ks_test_impl(&data).unwrap();
        assert!(
            result.p_value > 0.05,
            "Expected p > 0.05 for normal-like data, got p={}",
            result.p_value
        );
    ...
    fn test_confidence_interval_proportion() {
        ...
        assert!(
            approx_eq(result.point_estimate, 0.5, 1e-10),
            "Expected p_hat=0.5, got {}",
            result.point_estimate
        );
        assert!(
            result.lower > 0.39 && result.lower < 0.41,
            "CI lower out of range: {}",
            result.lower
        );
        assert!(
            result.upper > 0.59 && result.upper < 0.61,
            "CI upper out of range: {}",
            result.upper
        );
    }
    ...
    fn test_anova_same_groups() {
        ...
        let result = one_way_anova_impl(&groups, &group_sizes).unwrap();
        assert!(
            approx_eq(result.f_statistic, 0.0, 1e-6),
            "Expected F~0 for identical groups, got {}",
            result.f_statistic
        );
        assert!(
            result.p_value > 0.95,
            "Expected p~1 for identical groups, got {}",
            result.p_value
        );
    }
