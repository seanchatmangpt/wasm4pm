...
fn test_full_ensemble_consistency() {
    ...
    // The ensemble must have collected at least two object-level reports for
    // meta_reasoning to arbitrate (its documented minimum).
    assert!(
        contributing >= 2,
        "expected >=2 contributing breeds, got {} (ensemble path would be vacuous)",
        contributing
    );
