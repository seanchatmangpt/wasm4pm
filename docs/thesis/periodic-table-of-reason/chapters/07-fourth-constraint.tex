...
under native Rust DFA replay via \texttt{validate\_ocel\_alignment()} (\texttt{ocel.rs}) against the declared lifecycle OCPN---deterministic, reproducible, no external process-mining framework. A pipeline that passes code-level tests but fails
...
\label{def:fourth-constraint-ucq}
...
\section{The Four-Constraint Universal Equation}
...
\begin{itemize}
  ...
  \item $C_2$ — process conformance ($\phi \geq \phi^*$ under native Rust DFA replay via \texttt{validate\_ocel\_alignment()} (\texttt{ocel.rs}) against the declared lifecycle OCPN---deterministic, reproducible, no external process-mining framework).
...
\begin{proposition}[BVC as a Sheaf Condition \cite{maclane1992sheaves}]
...
receipt. We equip $G$ with the Alexandrov topology induced by the stage partial order: opens are down-closed sets of stages. Under the discrete topology the gluing condition trivializes; the Alexandrov topology is the substantive choice that makes locality and gluing non-trivial. Define a presheaf $\mathcal{F}$ on $G$ by assigning to each node $v$ the set
