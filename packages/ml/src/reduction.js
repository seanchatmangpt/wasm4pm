...
function minMaxNormalize(col) {
    ...
    for (let j = 0; j < d; j++) {
        ...
        for (let i = 0; i < n; i++)
            c[i] = (c[i] - min) * invRange;
    }
}
// ---------------------------------------------------------------------------
// Covariance matrix (direct computation, no transpose)
// ---------------------------------------------------------------------------
function covarianceMatrix(col) {
    const { cols, n, d } = col;
    const invN = 1 / (n - 1);
...
// ---------------------------------------------------------------------------
function eigenSymmetric(cov, maxIter = 200) {
    const n = cov.length;
    // Work on a copy
    const a = [];
    for (let i = 0; i < n; i++)
        a.push(new Float64Array(cov[i]));
 ...
 */
export async function reduceFeaturesPCA(featuresJson, options = {}) {
