...
function createTestEventLog(
  traceCount: number = 10,
  eventsPerTrace: number = 5,
  sourceFormat: 'xes' | 'ocel' | 'json' | 'csv' = 'xes'
): EventLogIR {
 ...
 */
function createTestDFGModel(
  nodeCount: number = 5,
  algorithmId: string = 'dfg'
): ModelIR {
  const nodes = [];
 ...
 */
function createTestPetriNetModel(
  placeCount: number = 5,
  transitionCount: number = 4
): ModelIR {
  const nodes = [];
 ...
 */
function createTestPOWLModel(): ModelIR {
  return {
    format_version: '1.0',
    model_type: 'powl',
...
describe('Suite 1: EventLogIR Round-Trip Serialization', () => {
  it('should serialize and deserialize EventLogIR with identical hash', () => {
    ...
    // Deserialize
    const deserialized: EventLogIR = JSON.parse(serialized);
...
describe('Suite 2: ModelIR Bidirectional Conversions', () => {
  ...
  it('should preserve DFG node frequencies through round-trip', () => {
    ...
    const serialized = JSON.stringify(original);
    const deserialized: ModelIR = JSON.parse(serialized);
...
describe('Suite 3: ProvenanceChain Audit Trail Propagation', () => {
  ...
  it('should have distinct combined_hash for different inputs', () => {
    const hash1 = computeCombinedHash(
      'aaaa' + 'a'.repeat(60),
      'bbbb' + 'b'.repeat(60),
      'cccc' + 'c'.repeat(60),
      'dddd' + 'd'.repeat(60)
    ...
    const hash2 = computeCombinedHash(
      'xxxx' + 'x'.repeat(60),
      'yyyy' + 'y'.repeat(60),
      'zzzz' + 'z'.repeat(60),
      'wwww' + 'w'.repeat(60)
...
describe('Suite 4: FederationController Backend Selection', () => {
  ...
  it('should apply budget latency gate: exclude backends exceeding budget tier', () => {
    ...
    const budgetMs = latencyTiers[latencyBudget as keyof typeof latencyTiers];
  ...
  it('should apply concurrency gate: exclude backends at maxConcurrentInvocations', () => {
    ...
    const available = backends.filter(
      b => b.concurrent < b.maxConcurrent
    );
...
describe('Suite 7: Determinism Verification', () => {
  ...
  it('should produce identical combined_hash across 5 runs', () => {
    ...
    for (let i = 0; i < 5; i++) {
      const combined = computeCombinedHash(
        'i'.repeat(64),
        'o'.repeat(64),
        'c'.repeat(64),
        'p'.repeat(64)
...
describe('Suite 7b: OTEL Span Structure Validation', () => {
  ...
  it('should reject span with missing status field', () => {
    ...
    expect((invalidSpan as any).status).toBeUndefined();
  });
...
describe('Suite 9: Cross-Format Model Conversions', () => {
  it('should convert DFG to canonical JSON and back', () => {
    ...
    const restored: ModelIR = JSON.parse(json);
...
describe('Suite 10: Regression Prevention', () => {
  ...
  it('should not regress: Receipt has all required fields', () => {
    ...
    for (const field of requiredFields) {
      expect((receipt as any)[field]).toBeDefined();
    }
  });
});
